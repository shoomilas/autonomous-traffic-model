    ...
    public static class RootUtils {
        ...
        public static float Ease(float _t, float _k1, float _k2) {
            ...
            if (_t < _k1)
                ...
                s = 2 * _k1 / Mathf.PI + _k2 - _k1 +
                    (1 - _k2) * (2 / Mathf.PI) * Mathf.Sin((_t - _k2) / (1.0f - _k2) * Mathf.PI / 2);
        ...
        public static bool Intersects2D(ref Vector2 _line1S, ref Vector2 _line1E, ref Vector2 _line2S,
            ref Vector2 _line2E, out Vector2 _intersectionPoint) {
            ...
            if (s >= 0 && s <= 1 && t >= 0 && t <= 1) {
                var intersectionPointX = _line1S.x + t * firstLineSlopeX;
                var intersectionPointY = _line1S.y + t * firstLineSlopeY;
        ...
        public static string GetPrefabString(GameObject _object) {
            ...
            if (_object != null) {
                ...
                if (path == null || path.Length < 1) {
                    ...
                    parentObject = PrefabUtility.GetCorrespondingObjectFromSource(_object);
#else
...
#if UNITY_EDITOR
            GC.Collect();
            if (_isWait) GC.WaitForPendingFinalizers();
        ...
        public static void SetupUniqueIdentifier(ref string _uID) {
            if (_uID == null || _uID.Length < 4) _uID = Guid.NewGuid().ToString();
        }
        ...
        public static void CreateXML<T>(ref string _path, object _object) {
            ...
            var xmlTextWriter = new XmlTextWriter(fs, Encoding.UTF8);
        ...
        public static string GetString<T>(object _object) {
            var data = SerializeObject<T>(ref _object);
            return data;
        }
        ...
        public static T LoadXML<T>(ref string _path) {
            ...
            var _info = reader.ReadToEnd();
            reader.Close();
            var loadedObject = DeserializeObject<T>(_info);
            return loadedObject;
        }
        ...
        private static T DeserializeObject<T>(string _xmlString) {
            var xs = new XmlSerializer(typeof(T));
            var memoryStream = new MemoryStream(StringToUTF8ByteArray(_xmlString));
        ...
        private static string UTF8ByteArrayToString(byte[] _characters) {
            ...
            var constructedString = encoding.GetString(_characters);
        ...
        private static byte[] StringToUTF8ByteArray(string _xmlString) {
            var encoding = new UTF8Encoding();
            var byteArray = encoding.GetBytes(_xmlString);
            return byteArray;
        }
        ...
        public static Vector4[] ProcessTangents(int[] _tris, Vector3[] _normals, Vector2[] _uvs, Vector3[] _verts) {
            ...
            var triangleCount = _tris.Length;
            ...
            Vector3 n;
            Vector3 t;
            for (var index = 0; index < MVL; index++) {
                n = _normals[index];
                t = tan1[index];
        ...
        public static string GetDirLibraryBase() {
            var libraryPath = Path.Combine(Path.Combine(RoadEditorUtility.GetBasePathForIO(), "Editor"), "Library");
            return libraryPath;
        }
        ...
        public static void CheckCreateSpecialLibraryDirs() {
            var libraryPath = GetDirLibraryBase();
            var directoryPath = Path.Combine(libraryPath, "EdgeObjects");
        ...
        public static void StartProfiling(Road _road, string _profileName) {
            if (_road.isProfiling) Profiler.BeginSample(_profileName);
        }
        ...
        public static void EndProfiling(Road _road) {
            if (_road.isProfiling) Profiler.EndSample();
        }
        ...
        public static void EndStartProfiling(Road _road, string _profileName) {
            if (_road.isProfiling) {
                Profiler.EndSample();
                Profiler.BeginSample(_profileName);
            }
        }
