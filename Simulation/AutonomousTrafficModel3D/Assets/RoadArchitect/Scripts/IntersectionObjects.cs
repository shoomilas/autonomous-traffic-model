...
namespace RoadArchitect {
    public static class IntersectionObjects {
        ...
        public static void CleanupIntersectionObjects(GameObject _masterGameObj) {
            var childCount = _masterGameObj.transform.childCount;
            ...
            for (var index = objectsToDelete.Count - 1; index >= 0; index--)
                Object.DestroyImmediate(objectsToDelete[index]);
        }


        public static void GetFourPoints(RoadIntersection _roadIntersection, out Vector3 _posRR, out Vector3 _posRL,
            out Vector3 _posLL, out Vector3 _posLR, float _distFromCorner) {
            GetFourPointsDo(ref _roadIntersection, out _posRR, out _posRL, out _posLL, out _posLR, _distFromCorner);
        }


        private static void GetFourPointsDo(ref RoadIntersection _roadIntersection, out Vector3 _posRR,
            out Vector3 _posRL, out Vector3 _posLL, out Vector3 _posLR, float _distFromCorner) {
            //Get four points:
            var tPos1 = 0f;
            var tPos2 = 0f;
            ...
            var Node1Width = -1f;
            var Node2Width = -1f;
            var tVectRR = _roadIntersection.cornerRR;
            var tVectRL = _roadIntersection.cornerRL;
            var tVectLR = _roadIntersection.cornerLR;
            var tVectLL = _roadIntersection.cornerLL;
            var tDir = default(Vector3);
            var ShoulderWidth1 = _roadIntersection.node1.spline.road.shoulderWidth;
            var ShoulderWidth2 = _roadIntersection.node2.spline.road.shoulderWidth;

            if (!_roadIntersection.isFlipped) {
                //RR:
                Node1Width =
                    (Vector3.Distance(_roadIntersection.cornerRR, _roadIntersection.node1.pos) + ShoulderWidth1) /
                    _roadIntersection.node1.spline.distance;
                Node2Width =
                    (Vector3.Distance(_roadIntersection.cornerRR, _roadIntersection.node2.pos) + ShoulderWidth2) /
                    _roadIntersection.node2.spline.distance;
                ...
                //RL:
                Node1Width =
                    (Vector3.Distance(_roadIntersection.cornerRL, _roadIntersection.node1.pos) + ShoulderWidth1) /
                    _roadIntersection.node1.spline.distance;
                Node2Width =
                    (Vector3.Distance(_roadIntersection.cornerRL, _roadIntersection.node2.pos) + ShoulderWidth2) /
                    _roadIntersection.node2.spline.distance;
                tPos1 = _roadIntersection.node1.time + Node1Width;
                ...
                //LL:
                Node1Width =
                    (Vector3.Distance(_roadIntersection.cornerLL, _roadIntersection.node1.pos) + ShoulderWidth1) /
                    _roadIntersection.node1.spline.distance;
                Node2Width =
                    (Vector3.Distance(_roadIntersection.cornerLL, _roadIntersection.node2.pos) + ShoulderWidth2) /
                    _roadIntersection.node2.spline.distance;
                tPos1 = _roadIntersection.node1.time + Node1Width;
                if (_roadIntersection.intersectionType == RoadIntersection.IntersectionTypeEnum.ThreeWay)
                    tPos1 = _roadIntersection.node1.time;
                ...
                //LR:
                Node1Width =
                    (Vector3.Distance(_roadIntersection.cornerLR, _roadIntersection.node1.pos) + ShoulderWidth1) /
                    _roadIntersection.node1.spline.distance;
                Node2Width =
                    (Vector3.Distance(_roadIntersection.cornerLR, _roadIntersection.node2.pos) + ShoulderWidth2) /
                    _roadIntersection.node2.spline.distance;
                tPos1 = _roadIntersection.node1.time - Node1Width;
                tTan1 = _roadIntersection.node1.spline.GetSplineValue(tPos1, true) * -1f;
                tPos2 = _roadIntersection.node2.time - Node2Width;
        ...
        private static void DebugFourPoints(Vector3 _posRR, Vector3 _posRL, Vector3 _posLL, Vector3 _posLR) {
            var tObj = GameObject.CreatePrimitive(PrimitiveType.Cube);
        ...
        private static void AddRigidbodyToSign(GameObject _obj, bool _isRB) {
            if (_isRB) {
                var RB = _obj.AddComponent<Rigidbody>();
                RB.mass = 100f;
        ...
        private static void CreateStopSignsAllWayDo(ref GameObject _masterGameObj, bool _isRB) {
...
#if UNITY_EDITOR
            prefab = AssetDatabase.LoadAssetAtPath<GameObject>(RoadEditorUtility.GetBasePath() +
            ...
            var roadIntersection = _masterGameObj.GetComponent<RoadIntersection>();
            var spline = roadIntersection.node1.spline;
            ...
            var ShoulderWidth = spline.road.shoulderWidth;

            //Cleanup:
            CleanupIntersectionObjects(_masterGameObj);

            //Get four points:
            var DistFromCorner = ShoulderWidth * 0.45f;
            ...
            if (roadIntersection.ignoreCorner == 0) Object.DestroyImmediate(tObj);
            ...
            if (roadIntersection.ignoreCorner == 2) Object.DestroyImmediate(tObj);
            ...
            if (roadIntersection.ignoreCorner == 1) Object.DestroyImmediate(tObj);
            ...
            if (roadIntersection.ignoreCorner == 3) Object.DestroyImmediate(tObj);
        }
        ...
        private static Vector3 StopSignGetRotRR(RoadIntersection _intersection, SplineC _spline) {
            var tDist = (Vector3.Distance(_intersection.cornerRL, _intersection.cornerRR) / 2f +
                         0.025f * Vector3.Distance(_intersection.cornerLL, _intersection.cornerRR)) / _spline.distance;
            var p = Mathf.Clamp(_intersection.node1.time - tDist, 0f, 1f);
            var rotation = _spline.GetSplineValue(p, true);
            return rotation * -1;
        }
        ...
        private static Vector3 StopSignGetRotLL(RoadIntersection _intersection, SplineC _spline) {
            var tDist = (Vector3.Distance(_intersection.cornerLR, _intersection.cornerLL) / 2f +
                         0.025f * Vector3.Distance(_intersection.cornerLL, _intersection.cornerRR)) / _spline.distance;
            var p = Mathf.Clamp(_intersection.node1.time + tDist, 0f, 1f);
            var rotation = _spline.GetSplineValue(p, true);
            return rotation;
        ...
        private static Vector3 StopSignGetRotRL(RoadIntersection _intersetion, SplineC _spline) {
            var tDist = (Vector3.Distance(_intersetion.cornerLL, _intersetion.cornerRL) / 2f +
                         0.025f * Vector3.Distance(_intersetion.cornerLR, _intersetion.cornerRL)) / _spline.distance;
            var p = -1f;
        ...
        private static Vector3 StopSignGetRotLR(RoadIntersection _intersection, SplineC _spline) {
            var tDist = (Vector3.Distance(_intersection.cornerRR, _intersection.cornerLR) / 2f +
                         0.025f * Vector3.Distance(_intersection.cornerLR, _intersection.cornerRL)) / _spline.distance;
            var p = -1f;
        ...
        public static void CreateTrafficLightBases(GameObject _masterGameObj, bool _isTrafficLight1 = true) {
            var intersection = _masterGameObj.GetComponent<RoadIntersection>();
            var spline = intersection.node1.spline;
            var isRB = true;

            //float RoadWidth = spline.road.RoadWidth();
            var LaneWidth = spline.road.laneWidth;
            var ShoulderWidth = spline.road.shoulderWidth;

            var Lanes = spline.road.laneAmount;
            var LanesHalf = Lanes / 2;
            ...
            var DistFromCorner = ShoulderWidth * 0.45f;
            var TLDistance = LanesForInter * LaneWidth + DistFromCorner;
            ...
            var MaxDistanceStart = Mathf.Max(tempDistances);
            var OrigPoleAlignment = intersection.isRegularPoleAlignment;

            //Node1:
            //RL:
            tObjRL = CreateTrafficLight(TLDistance, true, true, MaxDistanceStart, intersection.isTrafficPoleStreetLight,
                spline.road.isSavingMeshes);
            ...
            if (!intersection.isRegularPoleAlignment && intersection.ContainsLine(StartVec, EndVec)) {
                ...
                tObjRL = CreateTrafficLight(TLDistance, true, true, MaxDistanceStart,
                    intersection.isTrafficPoleStreetLight, spline.road.isSavingMeshes);
                //xDir = (intersection.cornerRL - intersection.transform.position).normalized;
                tDir = TrafficLightBaseGetRotRL(intersection, spline, DistFromCorner);
                if (tDir == zeroVect) tDir = new Vector3(0f, 0.0001f, 0f);
                tObjRL.transform.rotation = Quaternion.LookRotation(tDir) * Quaternion.Euler(-90f, -180f, 0f);
            ...
            //LR:
            tObjLR = CreateTrafficLight(TLDistance, true, true, MaxDistanceStart, intersection.isTrafficPoleStreetLight,
                spline.road.isSavingMeshes);
            ...
            if (!intersection.isRegularPoleAlignment && intersection.ContainsLine(StartVec, EndVec)) {
                ...
                tObjLR = CreateTrafficLight(TLDistance, true, true, MaxDistanceStart,
                    intersection.isTrafficPoleStreetLight, spline.road.isSavingMeshes);
                //xDir = (intersection.cornerLR - intersection.transform.position).normalized;
                tDir = TrafficLightBaseGetRotLR(intersection, spline, DistFromCorner);
                if (tDir == zeroVect) tDir = new Vector3(0f, 0.0001f, 0f);
            ...
            //Node2:
            //RR:
            tObjRR = CreateTrafficLight(TLDistance, true, true, MaxDistanceStart, intersection.isTrafficPoleStreetLight,
                spline.road.isSavingMeshes);
            ...
            if (!intersection.isRegularPoleAlignment && intersection.ContainsLine(StartVec, EndVec)) {
                ...
                tObjRR = CreateTrafficLight(TLDistance, true, true, MaxDistanceStart,
                    intersection.isTrafficPoleStreetLight, spline.road.isSavingMeshes);
                //xDir = (intersection.cornerRR - intersection.transform.position).normalized;
                tDir = TrafficLightBaseGetRotRR(intersection, spline, DistFromCorner);
                if (tDir == zeroVect) tDir = new Vector3(0f, 0.0001f, 0f);
            ...
            //LL:
            tObjLL = CreateTrafficLight(TLDistance, true, true, MaxDistanceStart, intersection.isTrafficPoleStreetLight,
                spline.road.isSavingMeshes);
            ...
            if (!intersection.isRegularPoleAlignment && intersection.ContainsLine(StartVec, EndVec)) {
                ...
                tObjLL = CreateTrafficLight(TLDistance, true, true, MaxDistanceStart,
                    intersection.isTrafficPoleStreetLight, spline.road.isSavingMeshes);
                //xDir = (intersection.cornerLL - intersection.transform.position).normalized;
                tDir = TrafficLightBaseGetRotLL(intersection, spline, DistFromCorner);
                if (tDir == zeroVect) tDir = new Vector3(0f, 0.0001f, 0f);
        ...
        private static bool IsTrafficLightBaseInIntersection(RoadIntersection _intersection, ref Vector3 _startVec,
            ref Vector3 _endVec) {
            return _intersection.ContainsLine(_startVec, _endVec);
        }


        private static GameObject CreateTrafficLight(float _distance, bool _isTrafficLight1, bool _isOptionalCollider,
            float _distanceX, bool _isLight, bool _isSavingAsset) {
            GameObject tObj = null;
            var tTrafficLightNumber = "1";
            if (!_isTrafficLight1) tTrafficLightNumber = "2";
            ...
            var assetNameAsset = "InterTLB" + tTrafficLightNumber + "_" + _distance.ToString("F0") + "_" +
                                 _distanceX.ToString("F0") + ".asset";
            var BackupFBX = "InterTLB" + tTrafficLightNumber + ".FBX";
            var tMod = _distance / 5f;
            var hMod = _distance / 10f * 0.7f;
            var xMod = (_distanceX / 20f + 2f) * 0.3334f;
            xMod = Mathf.Clamp(xMod, 1f, 20f);
            tMod = Mathf.Clamp(tMod, 1f, 20f);
            hMod = Mathf.Clamp(hMod, 1f, 20f);

            var bXMod = false;
            if (!RootUtils.IsApproximately(xMod, 1f, 0.0001f)) bXMod = true;
...
#if UNITY_EDITOR
            xMesh = AssetDatabase.LoadAssetAtPath<Mesh>(basePath + "/Mesh/Signs/TrafficLightBases/" + assetNameAsset);
            if (xMesh == null) {
                xMesh = AssetDatabase.LoadAssetAtPath<Mesh>(basePath + "/Mesh/Signs/TrafficLightBases/" + BackupFBX);
                bDoCustom = true;
            }
#else
            ...
            RoadEditorUtility.SetRoadMaterial(basePath + "/Materials/Signs/InterTLB" + tTrafficLightNumber + ".mat",
                MR);

            if (!bDoCustom) MF.sharedMesh = xMesh;

            var tempMaxHeight = 7.6f * hMod;
            var xValue = tempMaxHeight - 7.6f;
            if (bDoCustom) {
                var tMesh = new Mesh();
                ...
                xValue = xMod * 6f - 6f;
                if (xMod * 6f > tempMaxHeight - 1f) xValue = tempMaxHeight - 1f - 6f;

                var bIgnoreMe = false;

                var mCount = tVerts.Length;
                var uv = tMesh.uv;
                //List<int> tUVInts = new List<int>();
                for (var index = 0; index < mCount; index++) {
                    bIgnoreMe = false;
                    // Scale width of traffic light
                    if (RootUtils.IsApproximately(tVerts[index].y, 5f)) {
                        tVerts[index].y = _distance;
                        if (uv[index].y > 3.5f) uv[index].y *= tMod;
                        bIgnoreMe = true;
                    }
                    ...
                    if (!bIgnoreMe && tVerts[index].z > 7.5f) {
                        tVerts[index].z *= hMod;
                        if (uv[index].y > 3.8f) uv[index].y *= hMod;
                    }
                ...
                //Save:
                if (_isSavingAsset)
                    AssetDatabase.CreateAsset(tMesh, basePath + "/Mesh/Signs/TrafficLightBases/" + assetNameAsset);
#endif
            }

            var BC = tObj.AddComponent<BoxCollider>();
            var MaxHeight = MF.sharedMesh.bounds.size.z;
            ...
            */
...
#if UNITY_EDITOR
                yObj = AssetDatabase.LoadAssetAtPath<GameObject>(basePath + "/Prefabs/Signs/TrafficLight.prefab");
#else
                // Load your GameObject in another runtime way
                yObj = null;
#endif

                var kObj = Object.Instantiate(yObj);
            ...
            for (var index = 0; index < tMeshes.Length; index++) tMeshes[index].sharedMesh.RecalculateBounds();

            return tObj;
        }


        private static Vector3 TrafficLightBaseGetRotRL(RoadIntersection _intersection, SplineC _spline,
            float _distFromCorner, bool _isOverridingRegular = false) {
            ...
            if (!_intersection.isRegularPoleAlignment && !_isOverridingRegular) {
                ...
                var p = Mathf.Clamp(_intersection.node1.time, 0f, 1f);
        ...
        private static Vector3 TrafficLightBaseGetRotLR(RoadIntersection _intersection, SplineC _spline,
            float _distFromCorner, bool _isOverridingRegular = false) {
            var rotation = default(Vector3);
            if (!_intersection.isRegularPoleAlignment && !_isOverridingRegular) {
        ...
        private static Vector3 TrafficLightBaseGetRotRR(RoadIntersection _intersection, SplineC _spline,
            float _distFromCorner, bool _isOverridingRegular = false) {
            var rotation = default(Vector3);
            if (!_intersection.isRegularPoleAlignment && !_isOverridingRegular) {
                //float dist = ((Vector3.Distance(_intersection.cornerRR, _intersection.cornerLR) / 2f) + _distFromCorner) / _spline.distance;
                var p = Mathf.Clamp(_intersection.node2.time, 0f, 1f);
                rotation = _spline.GetSplineValue(p, true);
                rotation = Vector3.Cross(rotation, Vector3.up);
                if (_intersection.isFlipped) rotation = rotation * -1;
            }
            else {
        ...
        private static Vector3 TrafficLightBaseGetRotLL(RoadIntersection _intersection, SplineC _spline,
            float _distFromCorner, bool _isOverridingRegular = false) {
            var rotation = default(Vector3);
            if (!_intersection.isRegularPoleAlignment && !_isOverridingRegular) {
        ...
        private static void CreateTrafficLightMains(GameObject _masterGameObj, GameObject _RR, GameObject _RL,
            GameObject _LL, GameObject _LR) {
            var roadIntersection = _masterGameObj.GetComponent<RoadIntersection>();
            var tSpline = roadIntersection.node1.spline;

            var tDist = Vector3.Distance(roadIntersection.cornerRL, roadIntersection.cornerRR) / 2f / tSpline.distance;
            var tan = tSpline.GetSplineValue(roadIntersection.node1.time + tDist, true);
            ProcessPole(_masterGameObj, _RL, tan * -1, 1,
                Vector3.Distance(roadIntersection.cornerRL, roadIntersection.cornerRR));
            ...
            ProcessPole(_masterGameObj, _LR, tan, 3,
                Vector3.Distance(roadIntersection.cornerLR, roadIntersection.cornerLL));


            var InterDist = Vector3.Distance(roadIntersection.cornerRL, roadIntersection.cornerLL);
            tDist = InterDist / 2f / tSpline.distance;
            tan = tSpline.GetSplineValue(roadIntersection.node1.time + tDist, true);

            var fTime1 = roadIntersection.node2.time + tDist;
            var fTime2neg = roadIntersection.node2.time - tDist;
            ...
            if (roadIntersection.ignoreCorner == 0) {
                if (_RR != null) Object.DestroyImmediate(_RR);
            }
            else if (roadIntersection.ignoreCorner == 1) {
                if (_RL != null) Object.DestroyImmediate(_RL);
            }
            else if (roadIntersection.ignoreCorner == 2) {
                if (_LL != null) Object.DestroyImmediate(_LL);
            }
            else if (roadIntersection.ignoreCorner == 3) {
                if (_LR != null) Object.DestroyImmediate(_LR);
            }
        }
        ...
        private static void AdjustLightPrefab(GameObject _light) {
            ...
            foreach (var light in _light.GetComponentsInChildren<Light>())
                if (light.type == LightType.Point) {
#if UNITY_EDITOR
                    light.flare = AssetDatabase.LoadAssetAtPath<Flare>(basePath + "/Flares/SodiumBulb.flare");
#endif
                }
        }
        ...
        private static void ProcessPole(GameObject _masterGameObj, GameObject _obj, Vector3 _tan, int _corner,
            float _interDist) {
            var intersection = _masterGameObj.GetComponent<RoadIntersection>();
            var spline = intersection.node1.spline;
            ...
            if (intersection.roadType == RoadIntersection.RoadTypeEnum.BothTurnLanes)
                ...
                LanesForInter = LanesHalf;
            var DistFromCorner = ShoulderWidth * 0.35f;
            var TLDistance = LanesForInter * LaneWidth + DistFromCorner;

            var MF = _obj.GetComponent<MeshFilter>();
            var tMesh = MF.sharedMesh;
            var tVerts = tMesh.vertices;
            var StartVec = new Vector3(0f, 0.5f, 5.8f);
            var EndVec = new Vector3(0f, tMesh.bounds.extents.y * 2, 5.8f);


            StartVec = (EndVec - StartVec) * (DistFromCorner / TLDistance) + StartVec;
            var tLanePosR = StartVec - new Vector3(0f, 1.6f, 0.5f);

            var SmallerDist = Vector3.Distance(StartVec, EndVec);
            ...
            for (var index = 0; index < LanesHalf; index++)
                tLanePos[index] = (EndVec - StartVec) * ((LaneWidth * 0.5f + index * LaneWidth) / SmallerDist) +
                                  StartVec;
            ...
            var mCount = tLanePos.Length;
            var mDistance = -50000f;
            var tDistance = 0f;
            for (var index = 0; index < mCount; index++) {
            ...
            var tScaleSense = (200f - intersection.scalingSense) / 200f * 200f;
            tScaleSense = Mathf.Clamp(tScaleSense * 0.1f, 0f, 20f);
            var ScaleMod = (mDistance / 17f + tScaleSense) * (1f / (tScaleSense + 1f));
            if (RootUtils.IsApproximately(tScaleSense, 20f, 0.05f)) ScaleMod = 1f;
            ScaleMod = Mathf.Clamp(ScaleMod, 1f, 1.5f);
            var tScale = new Vector3(ScaleMod, ScaleMod, ScaleMod);
            var bScale = true;
            if (RootUtils.IsApproximately(ScaleMod, 1f, 0.001f)) bScale = false;
            ...
            if (intersection.roadType != RoadIntersection.RoadTypeEnum.NoTurnLane) {
#if UNITY_EDITOR
                if (intersection.isLeftTurnYieldOnGreen)
                    prefab = AssetDatabase.LoadAssetAtPath<GameObject>(basePath +
                ...
                tLeft = (GameObject)Object.Instantiate(prefab, Vector3.zero, Quaternion.identity);
                ...
                if (intersection.isLeftTurnYieldOnGreen) {
                    ...
                    tLeft_Sign = (GameObject)Object.Instantiate(prefab, Vector3.zero, Quaternion.identity);
            ...
            if (intersection.roadType == RoadIntersection.RoadTypeEnum.BothTurnLanes) {
                ...
                tRight = (GameObject)Object.Instantiate(prefab, Vector3.zero, Quaternion.identity);
...
#if UNITY_EDITOR
            prefab = AssetDatabase.LoadAssetAtPath<GameObject>(basePath + "/Prefabs/Signs/TrafficLightMain.prefab");
#endif
            for (var index = 0; index < LanesHalf; index++) {
                tLanes[index] = (GameObject)Object.Instantiate(prefab, Vector3.zero, Quaternion.identity);
                ...
                tLanes[index].transform.name = "Light" + index;
                if (bScale) tLanes[index].transform.localScale = tScale;
            ...
            var LM = new TrafficLightController(ref tLeft, ref tRight, ref tLanes, ref MR_Left, ref MR_Right,
                ref MR_Mains);
            if (_corner == 0)
            ...
            else if (_corner == 3) intersection.lightsLR = LM;
        }
