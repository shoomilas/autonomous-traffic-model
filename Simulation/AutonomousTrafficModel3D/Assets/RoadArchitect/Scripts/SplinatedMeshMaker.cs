    ...
    public class SplinatedMeshMaker {
        ...
        public SplinatedMeshMaker Copy() {
            ...
            if (verticalCurve != null && verticalCurve.keys.Length > 0)
                for (var i = 0; i < verticalCurve.keys.Length; i++)
        ...
        public void SaveToLibrary(string _name = "", bool _isDefault = false) {
            ...
            var filePath = Path.Combine(libraryPath, "ESO" + objectName + ".rao");
            if (_name.Length > 0) {
                if (_isDefault)
        ...
        public void LoadFromLibrary(string _name, bool _isQuickAdd = false) {
            ...
            var SLM = RootUtils.LoadXML<SplinatedMeshLibraryMaker>(ref filePath);
            SLM.LoadToSMM(this);
        ...
        public void LoadFromLibraryWizard(string _name) {
            RootUtils.CheckCreateSpecialLibraryDirs();
            var libraryPath = RootUtils.GetDirLibrary();
            var filePath = Path.Combine(Path.Combine(libraryPath, "W"), _name + ".rao");
            var SLM = RootUtils.LoadXML<SplinatedMeshLibraryMaker>(ref filePath);
            SLM.LoadToSMM(this);
            isRequiringUpdate = true;
        }
        ...
        public static SplinatedMeshLibraryMaker SLMFromData(string _data) {
            try {
                var SLM = RootUtils.LoadData<SplinatedMeshLibraryMaker>(ref _data);
                return SLM;
            }
            catch {
                return null;
        ...
        public void Kill() {
            ...
            if (EndCapStartOutput != null) Object.DestroyImmediate(EndCapStartOutput);
            if (EndCapEndOutput != null) Object.DestroyImmediate(EndCapEndOutput);
        }
        ...
        private void SplinateMeshDo(bool _isGettingStrings, ref GameObject[] _errorObj, bool _isCollecting) {
            ...
            if (_isGettingStrings) {
                ...
                if (EndCapStart != null) EndCapStartString = RootUtils.GetPrefabString(EndCapStart);
                if (EndCapEnd != null) EndCapEndString = RootUtils.GetPrefabString(EndCapEnd);

#if UNITY_EDITOR
                if (SplinatedMaterial1 != null)
                    SplinatedMaterial1String = AssetDatabase.GetAssetPath(SplinatedMaterial1);
                if (SplinatedMaterial2 != null)
                    SplinatedMaterial2String = AssetDatabase.GetAssetPath(SplinatedMaterial2);
#endif
            }

            if (currentSplination == null) return;
            var tObj = Object.Instantiate(currentSplination);
            _errorObj[0] = tObj;

            GameObject EndCapStartObj = null;
            GameObject EndCapEndObj = null;
            if (EndCapStart != null) {
                EndCapStartObj = Object.Instantiate(EndCapStart);
                _errorObj[1] = EndCapStartObj;
            }

            if (EndCapEnd != null) {
                EndCapEndObj = Object.Instantiate(EndCapEnd);
            ...
            if (isFlippingRotation) {
                if (currentSplinationCap2 != null) {
                    Cap1 = Object.Instantiate(currentSplinationCap2);
                    _errorObj[3] = Cap1;
                }

                if (currentSplinationCap1 != null) {
                    Cap2 = Object.Instantiate(currentSplinationCap1);
                    _errorObj[4] = Cap2;
                }
            }
            else {
                if (currentSplinationCap1 != null) {
                    Cap1 = Object.Instantiate(currentSplinationCap1);
                    _errorObj[3] = Cap1;
                }

                if (currentSplinationCap2 != null) {
                    Cap2 = Object.Instantiate(currentSplinationCap2);
                    _errorObj[4] = Cap2;
                }
            }
            ...
            var OrigMVL = OrigVerts.Length;

            //Transform vertices:
            var OrigNormals = mesh.normals;
            var bCheckingNormal = true;
            ...
            var CapTriCount2 = 0;
            if (CapMesh1 != null) {
                for (var index = 0; index < CapOrigMVL1; index++)
            ...
            if (CapMesh2 != null) {
                for (var index = 0; index < CapOrigMVL2; index++)
                ...
                var oMinX = Mathf.Min(oMinMaxX);
                //				float oMaxX = Mathf.Max(oMinMaxX);
                //				float oMinY = Mathf.Min(oMinMaxY);
                //				float oMaxY = Mathf.Max(oMinMaxY);
                var oMinZ = Mathf.Min(oMinMaxZ);
            ...
            var OrigTriCount = OrigTris.Length;
            var OrigUV = mesh.uv;
            ...
            if (RepeatUVType != RepeatUVTypeEnum.None) tMinMaxUV = new float[OrigMVL];
            for (var i = 0; i < OrigMVL; i++) {
                ...
                else if (RepeatUVType == RepeatUVTypeEnum.Y) tMinMaxUV[i] = OrigUV[i].y;
            }

            var mMax = Mathf.Max(tMinMax);
            var mMin = Mathf.Min(tMinMax);
            var mMaxX = Mathf.Max(tMinMaxX);
            var mMinX = Mathf.Min(tMinMaxX);
            var mMaxY = Mathf.Max(tMinMaxY);
            var mMinY = Mathf.Min(tMinMaxY);
            var mMaxZ = Mathf.Max(tMinMaxZ);
            var mMinZ = Mathf.Min(tMinMaxZ);
            var mMinUV = -1f;
            var mMaxUV = -1f;
            var mUVDiff = -1f;
            ...
            var mMaxDiff = mMax - mMin;
            var mMaxHeight = mMaxY - mMinY;
            var mMaxThreshold = mMax - minMaxMod;
            var mMinThreshold = mMin + minMaxMod;
            ...
            var MiddleCount = MiddleVectorIndicies.Count;
            ...
            var tCount1 = MaxVectorIndices.Count;
            var tCount2 = MinVectorIndices.Count;
            var tIntBuffer1 = -1;
            var tIntBuffer2 = -1;
            var tIntBuffer3 = -1;
            var tIntBuffer4 = -1;
            ...
            for (var index = 0; index < tCount1; index++) {
                tIntBuffer1 = MaxVectorIndices[index];
                tVect1 = OrigVerts[tIntBuffer1];

                var bAdded = false;
                for (var j = 0; j < OrigTriCount; j += 3) {
            ...
            if (MatchingIndices.Count < MaxVectorIndices.Count) {
                var bIsZAxis = Axis == AxisTypeEnum.Z;
                for (var index = 0; index < tCount1; index++) {
                    tIntBuffer1 = MaxVectorIndices[index];
                    ...
                    if (Axis == AxisTypeEnum.Z)
                        for (var j = 0; j < tCount2; j++) {
                            ...
                            if (!AlreadyAddedList.Contains(tIntBuffer2)) {
                                ...
                                if (IsApproxExtruded(ref tVect1, ref tVect2, bIsZAxis) &&
                                    V3EqualNormal(OrigNormals[tIntBuffer1], OrigNormals[tIntBuffer2])) {
                                    MatchingIndices.Add(tIntBuffer1, tIntBuffer2);
                                    AlreadyAddedList.Add(tIntBuffer2);
                                    MatchingIndices_Min.Add(tIntBuffer2, tIntBuffer1);
                                    break;
            ...
            if (CapMesh1 != null) {
                var bDidAdd = false;
                foreach (var KVP in MatchingIndices_Min) {
                    var tList = new List<int>();
                    tVect1 = OrigVerts[KVP.Key];
                    for (var index = 0; index < CapOrigMVL1; index++)
                        if (tCapMatchIndices1.Contains(index) && IsApproxTwoThirds(ref tVect1, CapOrigVerts1[index],
                ...
                if (!bDidAdd) {
                    Debug.LogWarning(
                        "Start cap error (still processing extrusion, ignoring start cap). No matching vertices found for start cap. Most likely the cap mesh is aligned improperly or along the wrong axis relative to the main mesh.");
            ...
            if (CapMesh2 != null) {
                var bDidAdd = false;
                foreach (var KVP in MatchingIndices) {
                    var tList = new List<int>();
                    tVect1 = OrigVerts[KVP.Key];
                    for (var index = 0; index < CapOrigMVL2; index++)
                        if (tCapMatchIndices2.Contains(index) && IsApproxTwoThirds(ref tVect1, CapOrigVerts2[index],
                            vertexMatchingPrecision)) {
                            tList.Add(index);
                            bDidAdd = true;
                        }
                ...
                if (!bDidAdd) {
                    Debug.LogError(
                        "End cap error (still processing extrusion, ignoring end cap). No matching vertices found for end cap. Most likely the cap mesh is aligned improperly or along the wrong axis relative to the main mesh.");
            ...
            if (isMatchingRoadDefinition) {
                var RoadDefStart = spline.road.roadDefinition / 2f * -1;
                var UVChange = spline.road.roadDefinition / mMaxDiff;
                foreach (var KVP in MatchingIndices) {
                    //Vertex change:
                    if (Axis == AxisTypeEnum.X) {
                    ...
                    else if (RepeatUVType == RepeatUVTypeEnum.Y) OrigUV[KVP.Key].y *= UVChange;
                }
                ...
                for (var index = 0; index < OrigMVL; index++) {
                    ...
                    else if (RepeatUVType == RepeatUVTypeEnum.Y) tMinMaxUV[index] = OrigUV[index].y;
                }
            ...
            var VertCutTriIndex1 = -1;
            var VertCutTriIndex2 = -1;
            if (isVerticalMeshCutoffOppositeDir) {
                ...
                var tMatchingMaxY_f = Mathf.Max(tMatchingMaxY);
                foreach (var KVP in MatchingIndices)
            ...
            var pDiffTime = EndTime - StartTime;
            var CurrentH = 0f;
            var fHeight = 0f;
            ...
            while (cTime < EndTime && SpamGuardCounter < SpamGuard) {
                ...
                cTime = spline.GetClosestParam(xVect);
            ...
            if (isTrimStart)
                tTimes.RemoveAt(0);
            else if (isTrimEnd) tTimes.RemoveAt(tTimes.Count - 1);
            ...
            var tStartTime = tTimes[0];
            var tEndTime = tTimes[vSeriesCount - 1];
            //			float tDiffTime = tEndTime - tStartTime;
            //			float cDiff = 0f;

            var jDistance = 0f;
            var jStartDistance = spline.TranslateParamToDist(tStartTime);
            var jEndDistance = spline.TranslateParamToDist(tEndTime);
            var jDistanceDiff = jEndDistance - jStartDistance;
            //			float jLastTime = 0f;
            var jCurrTime = 0f;
            ...
            for (var i = 0; i < vSeriesCount; i++) {
                ...
                tIntStrength = spline.IntersectionStrength(ref tVect1, ref tIntHeight, ref roadIntersection,
                    ref bIsPastInter, ref cTime, ref xNode);
            ...
            var MeshCount = vSeriesCount - 1;

            //			float yDiff = 0f;
            //			float tDistance = 0f;
            var MVL = MeshCount * OrigMVL;
#if UNITY_2017_3_OR_NEWER
            if (MVL > 4000000)
                throw new Exception(
                    "Over 4000000 vertices detected, exiting extrusion. Try switching splination axis and make sure your imported FBX file has proper import scale. Make sure the mesh isn't too small and make sure the distance isn't too large.");
#else
			if(MVL > 64900)
            {
...
#endif
            var MaxCount = MaxVectorIndices.Count;
            var MinCount = MinVectorIndices.Count;
            var TriCount = MeshCount * OrigTriCount;
            ...
            var cCount = -1;
            var cTriCount = -1;
            var bSimpleCollisionOn = false;
            var tOrigHeightBuffer = 0f;
            var tFloat5 = 0f;
            ...
            var tOrigHeightBuffer_Orig = 0f;
            //			if(bVerticalMeshCutoff_OppositeDir){
            //				tTriList = new List<RoadUtility.Construction3DTri>();
            //			}

            if (isStretched) {
                DoStretch(ref OrigVerts, ref OrigUV, ref OrigTris, ref MaxVectorIndices, ref MinVectorIndices, mMaxDiff,
                    out tVerts, out tUV, out tNormals, out tTris);
                goto StretchSkip;
            }
            ...
            for (var j = 0; j < MeshCount; j++) {
                        ...
                        if (MiddleVectorIndicies.Contains(index)) {
                            ...
                            if (isVerticalCutoffDownwards) {
                                if (isVerticalCutoffMatchingZero) {
                                    ...
                                    if (tFloat5 < tOrigHeightBuffer) tVerts[vManuver + index].y = tOrigHeightBuffer;
                                }
                ...
                if (j > 0) {
                    ...
                    for (var g = 0; g < MinVectorIndices.Count; g++)
                ...
                //Triangles:
                for (var index = 0; index < OrigTriCount; index++)
                ...
                if (isVerticalCutoff)
                    if (isVerticalMeshCutoffOppositeDir) {
                        ...
                        for (var index = 0; index < MiddleCount; index++) {
                            ...
                            if (!isVerticalCutoffDownwards) {
                                tBuffer = GetVHeightAtXY(ref VertCutBuffer1, ref VertCutBuffer2, ref VertCutBuffer3) +
                                          VerticalMeshCutoffOffset;
                ...
                if (isStartDown) {
                    ...
                    if (isStartTypeDownOverriden) tFloat1 = startTypeDownOverride;
                    if (j == 0) {
                        for (var index = 0; index < MinCount; index++) {
                            tIntBuffer1 = MinVectorIndices[index];
                        ...
                        for (var index = 0; index < MiddleCount; index++) {
                            ...
                            var tDistTo1 = Vector3.Distance(tVerts[vManuver + tIntBuffer1], pVect1);
                ...
                if (isEndDown) {
                    tFloat1 = mMaxHeight * 1.05f;
                    if (isEndTypeDownOverriden) tFloat1 = endTypeDownOverride;
                    if (j == MeshCount - 1) {
                        for (var index = 0; index < MaxCount; index++) {
                            tIntBuffer1 = MaxVectorIndices[index];
                            tVerts[vManuver + tIntBuffer1].y -= tFloat1;
                        ...
                        for (var i = 0; i < MiddleCount; i++) {
                            ...
                            var tDistTo1 = Vector3.Distance(tVerts[vManuver + tIntBuffer1], pVect2);
                            tVerts[vManuver + tIntBuffer1].y -= tFloat1 * (tDistTo1 / tTotalDistDown);
                        }
                ...
                if (j == 0 && EndCapStartObj != null) {
                    if (isEndCapCustomMatchStart && MinVectorIndices.Count > 0) {
                        ...
                        var tVect6 = spline.GetSplineValue(spline.GetClosestParam(tVect5));
                        tVect5.y = tVect6.y;
                    ...
                    if (isEndObjectsMatchingGround) {
                        ...
                        if (tRayHit.Length > 0) {
                            tRayYs = new float[tRayHit.Length];
                            for (var g = 0; g < tRayHit.Length; g++)
                                if (g == 0) {
                    ...
                    if (isEndCapCustomMatchStart && MaxVectorIndices.Count > 0) {
                        ...
                        for (var g = 0; g < MaxVectorIndices.Count; g++)
                            bVerts[g] = tVerts[vManuver + MaxVectorIndices[g]];
                        var tVect5 = GetVector3Average(bVerts);
                        var tVect6 = spline.GetSplineValue(spline.GetClosestParam(tVect5));
                        if (!float.IsNaN(tVect6.y)) tVect5.y = tVect6.y;
            ...
            if (isStretched) {
                ...
                if (EndCapStartObj != null) {
                    ...
                    tVect2 = spline.GetSplineValue(tFloat1);
                    tVect1.y = tVect2.y;
            ...
            if (CapMesh1 != null) {
                ...
                Array.Copy(CapOrigVerts1, cap1_verts, CapOrigMVL1);
                var cap1_tris = new int[CapTriCount1];
                Array.Copy(CapOrigTris1, cap1_tris, CapTriCount1);
                var cap1_uv = new Vector2[CapOrigMVL1];
                Array.Copy(CapOrigUV1, cap1_uv, CapOrigMVL1);
                var cap1_normals = new Vector3[CapOrigMVL1];
                Array.Copy(CapOrigNormals1, cap1_normals, CapOrigMVL1);
                var cap1_hit = new bool[CapOrigMVL1];
                var bcapstart = true;
                var tHeight = 0f;

                foreach (var KVP in MatchingIndices_Min_Cap) {
                    var wCount = KVP.Value.Count;
                    for (var index = 0; index < wCount; index++) {
                        if (bcapstart) tVect1 = cap1_verts[KVP.Value[index]] - tVerts[KVP.Key];
                        cap1_verts[KVP.Value[index]] = tVerts[KVP.Key];
                        cap1_hit[KVP.Value[index]] = true;
                        if (bcapstart) {
                            tHeight = spline.GetSplineValue(spline.GetClosestParam(cap1_verts[KVP.Value[index]])).y;
                            bcapstart = false;
                        }
                    }
                }
                ...
                for (var index = 0; index < CapOrigMVL1; index++)
                    if (!cap1_hit[index]) {
                        ...
                        tVect2 = spline.GetSplineValue(tParam);
                ...
                var OldTriCount = tTris.Length;
                var OldMVL = tVerts.Length;

                Array.Copy(cap1_verts, nVerts, CapOrigMVL1);
                Array.Copy(cap1_normals, nNormals, CapOrigMVL1);
                Array.Copy(cap1_tris, nTris, CapTriCount1);
                Array.Copy(cap1_uv, nUV, CapOrigMVL1);

                Array.Copy(tVerts, 0, nVerts, CapOrigMVL1, OldMVL);
                Array.Copy(tNormals, 0, nNormals, CapOrigMVL1, OldMVL);
                Array.Copy(tTris, 0, nTris, CapTriCount1, OldTriCount);
                Array.Copy(tUV, 0, nUV, CapOrigMVL1, OldMVL);

                for (var index = CapTriCount1; index < CapTriCount1 + OldTriCount; index++) nTris[index] += CapOrigMVL1;
            ...
            if (CapMesh2 != null) {
                ...
                Array.Copy(CapOrigVerts2, cap2_verts, CapOrigMVL2);
                var cap2_tris = new int[CapTriCount2];
                Array.Copy(CapOrigTris2, cap2_tris, CapTriCount2);
                var cap2_uv = new Vector2[CapOrigMVL2];
                Array.Copy(CapOrigUV2, cap2_uv, CapOrigMVL2);
                var cap2_normals = new Vector3[CapOrigMVL2];
                Array.Copy(CapOrigNormals2, cap2_normals, CapOrigMVL2);
                var cap2_hit = new bool[CapOrigMVL2];
                var bcapstart = true;
                var tHeight = 0f;

                foreach (var KVP in MatchingIndices_Max_Cap) {
                    var wCount = KVP.Value.Count;
                    for (var index = 0; index < wCount; index++) {
                        if (bcapstart) tVect1 = cap2_verts[KVP.Value[index]] - tVerts[vManuver + KVP.Key + CapOrigMVL1];
                        cap2_verts[KVP.Value[index]] = tVerts[vManuver + KVP.Key + CapOrigMVL1];
                        cap2_hit[KVP.Value[index]] = true;

                        if (bcapstart) {
                            tHeight = spline.GetSplineValue(spline.GetClosestParam(cap2_verts[KVP.Value[index]])).y;
                            bcapstart = false;
                        }
                    }
                }
                ...
                Array.Copy(tVerts, 0, nVerts, 0, OldMVL);
                Array.Copy(tNormals, 0, nNormals, 0, OldMVL);
                Array.Copy(tTris, 0, nTris, 0, OldTriCount);
                Array.Copy(tUV, 0, nUV, 0, OldMVL);

                Array.Copy(cap2_verts, 0, nVerts, OldMVL, CapOrigMVL2);
                Array.Copy(cap2_normals, 0, nNormals, OldMVL, CapOrigMVL2);
                Array.Copy(cap2_tris, 0, nTris, OldTriCount, CapTriCount2);
                Array.Copy(cap2_uv, 0, nUV, OldMVL, CapOrigMVL2);

                for (var index = OldTriCount; index < nTris.Length; index++) nTris[index] += OldMVL;
            ...
            var tVertCount = tVerts.Length;
            for (var index = 0; index < tVertCount; index++) tVerts[index] -= node.pos;
            if (cVerts != null) {
                var cVertCount = cVerts.Length;
                for (var index = 0; index < cVertCount; index++) cVerts[index] -= node.pos;
            }
            ...
            tIntBuffer1 = 0;
            if (!isStretched) {
                for (var j = 1; j < MeshCount; j++) {
                    vManuver = j * OrigMVL;
                    vManuver_Prev = (j - 1) * OrigMVL;
                    if (CapMesh1 != null) tIntBuffer1 = CapOrigMVL1;
                    foreach (var KVP in MatchingIndices_Min) {
                        tAvgNormal = (tNormals[tIntBuffer1 + vManuver + KVP.Key] +
                                      tNormals[tIntBuffer1 + vManuver_Prev + KVP.Value]) * 0.5f;
            ...
            if (objectName == null || objectName.Length < 1) objectName = "ExtrudedMesh";
            ...
            if (node.spline.road.isSavingMeshes) SaveMesh(ref xMesh, false);
            ...
            if (CollisionType == CollisionTypeEnum.SimpleMeshTriangle) {
                ...
                if (isBoxColliderFlippedOnX) BCCenter.z *= -1f;
                if (isBoxColliderFlippedOnZ) BCCenter.x *= -1f;
            ...
            if (SplinatedMaterial1 == null && !isMaterialOverriden) {
                ...
                if (PrefabMR != null && PrefabMR.sharedMaterials != null) MR.materials = PrefabMR.sharedMaterials;
            }
            else {
                ...
                if (SplinatedMaterial1 != null) {
                    tIntBuffer1 += 1;
                    if (SplinatedMaterial2 != null) tIntBuffer1 += 1;
                }
                ...
                if (tIntBuffer1 > 0) {
                    ...
                    if (SplinatedMaterial1 != null) {
                        ...
                        if (SplinatedMaterial2 != null) tMats[1] = SplinatedMaterial2;
                    }
            ...
            if (EndCapStartObj != null) {
                ...
                if (eMR.sharedMaterials == null || eMR.sharedMaterial != null &&
                    eMR.sharedMaterial.name.ToLower().Contains("default-diffuse")) eMR.sharedMaterials = fMats;
            }
        ...
        private void SaveMesh(ref Mesh _mesh, bool _isCollider) {
            ...
            var savingPath = Path.GetDirectoryName(Application.dataPath);
        ...
        private void DoStretch(ref Vector3[] _origVerts, ref Vector2[] _origUV, ref int[] _origTris,
            ref List<int> _maxVectorIndices, ref List<int> _minVectorIndices, float _maxDiff, out Vector3[] _verts,
            out Vector2[] _UV, out Vector3[] _normals, out int[] _tris) {
            var tVect1 = node.pos;
            var tVect2 = default(Vector3);
            ...
            var OrigMVL = _origVerts.Length;

            _verts = new Vector3[OrigMVL];
            _UV = new Vector2[OrigMVL];
            _normals = new Vector3[OrigMVL];
            _tris = new int[_origTris.Length];
            Array.Copy(_origVerts, _verts, OrigMVL);
            Array.Copy(_origTris, _tris, _origTris.Length);
            Array.Copy(_origUV, _UV, OrigMVL);
            ...
            //Rotate and set vertex positions:
            var NewDiff = Vector3.Distance(tVect1, tVect2);
            var UVMod = NewDiff / _maxDiff;
            var xVect = default(Vector3);
            for (var index = 0; index < OrigMVL; index++) {
                ...
                if (RepeatUVType == RepeatUVTypeEnum.X) {
                    if (_origUV[index].x > stretchUVThreshold) _UV[index].x = _origUV[index].x * UVMod;
                }
                else {
                    if (_origUV[index].y > stretchUVThreshold) _UV[index].y = _origUV[index].y * UVMod;
                }
            }
        }
        ...
        private Vector3 GetAverageNormalToGround(GameObject _object) {
            ...
            var bounds = _object.GetComponent<MeshFilter>().sharedMesh.bounds;
            ...
            for (var index = 0; index < xVects.Count; index++) hitNormal += xVects[index];
        ...
        public class SplinatedMeshLibraryMaker {
            ...
            public void LoadToSMM(SplinatedMeshMaker _SMM) {
...
#if UNITY_EDITOR
                _SMM.currentSplination = AssetDatabase.LoadAssetAtPath<GameObject>(CurrentSplinationString);
                if (CurrentSplinationCap1String != null && CurrentSplinationCap1String.Length > 1)
                    _SMM.currentSplinationCap1 = AssetDatabase.LoadAssetAtPath<GameObject>(CurrentSplinationCap1String);
                if (CurrentSplinationCap2String != null && CurrentSplinationCap2String.Length > 1)
                    _SMM.currentSplinationCap2 = AssetDatabase.LoadAssetAtPath<GameObject>(CurrentSplinationCap2String);
                if (isMaterialOverriden) {
                    if (SplinatedMaterial1String != null && SplinatedMaterial1String.Length > 0)
                        _SMM.SplinatedMaterial1 = AssetDatabase.LoadAssetAtPath<Material>(SplinatedMaterial1String);
                    if (SplinatedMaterial2String != null && SplinatedMaterial2String.Length > 0)
                        _SMM.SplinatedMaterial2 = AssetDatabase.LoadAssetAtPath<Material>(SplinatedMaterial2String);
                }
#endif
                ...
                if (EndCapStartString != null && EndCapStartString.Length > 0)
                    _SMM.EndCapStart = AssetDatabase.LoadAssetAtPath<GameObject>(EndCapStartString);
                if (EndCapEndString != null && EndCapEndString.Length > 0)
                    _SMM.EndCapEnd = AssetDatabase.LoadAssetAtPath<GameObject>(EndCapEndString);
#endif
            ...
            public float CapHeightOffset1;
            public float CapHeightOffset2;

            [FormerlySerializedAs("bExactSplination")]
            public bool isExactSplination;

            [FormerlySerializedAs("bMatchRoadDefinition")]
            public bool isMatchingRoadDefinition;

            [FormerlySerializedAs("bMatchRoadIncrements")]
            public bool isMatchingRoadIncrements = true;

            [FormerlySerializedAs("bTrimStart")] public bool isTrimStart;

            [FormerlySerializedAs("bTrimEnd")] public bool isTrimEnd;

            [FormerlySerializedAs("bToggle")] public bool isToggled;

            [FormerlySerializedAs("bMatchTerrain")]
            public bool isMatchingTerrain;

            public float MinMaxMod = 0.002f;
            public float VertexMatchingPrecision = 0.005f;

            [FormerlySerializedAs("bIsStretch")] public bool isStretched;

            [FormerlySerializedAs("bStretchLocOffset")]
            public bool isStretchedLocOffset;

            [FormerlySerializedAs("bStretchSize")] public bool isStretchedSize;

            [FormerlySerializedAs("StretchBC_LocOffset")]
            public Vector3 boxColliderOffset;

            [FormerlySerializedAs("StretchBC_Size")]
            public Vector3 boxColliderSize;

            [FormerlySerializedAs("Stretch_UVThreshold")]
            public float stretchUVThreshold = 0.05f;

            [FormerlySerializedAs("bStraightLineMatchStartEnd")]
            public bool isStraightLineMatchingStartEnd;

            [FormerlySerializedAs("bBCFlipX")] public bool isBoxColliderFlippedOnX;

            [FormerlySerializedAs("bBCFlipZ")] public bool isBoxColliderFlippedOnZ;
            ...
            //Vertical cutoff:
            public float VerticalCutoff;

            [FormerlySerializedAs("bVerticalCutoff")]
            public bool isVerticalCutoff;

            [FormerlySerializedAs("bVerticalCutoffDownwards")]
            public bool isVerticalCutoffDownwards;

            [FormerlySerializedAs("bVerticalMeshCutoff_OppositeDir")]
            public bool isVerticalMeshCutoffOppositeDir;

            public float VerticalMeshCutoffOffset = 0.04f;

            [FormerlySerializedAs("bVerticalCutoff_MatchZero")]
            public bool isVerticalCutoffMatchingZero;

            public float RoadRaise;
            public Vector3 CustomRotation;

            [FormerlySerializedAs("bFlipRotation")]
            public bool isFlippedRotation;
            ...
            public int RepeatUVType;

            [FormerlySerializedAs("bNoCenterMode")]
            ...
            [FormerlySerializedAs("bEndCapCustomMatchStart")]
            public bool isEndCapCustomMatchStart = true;

            public Vector3 EndCapCustomOffsetStart;
            public Vector3 EndCapCustomOffsetEnd;
            public Vector3 EndCapCustomRotOffsetStart;
            public Vector3 EndCapCustomRotOffsetEnd;

            [FormerlySerializedAs("bEndObjectsMatchGround")]
            public bool isEndObjectsMatchingGround;

            //Endings down:
            [FormerlySerializedAs("bStartDown")] public bool isStartDown;

            [FormerlySerializedAs("bStartTypeDownOverride")]
            public bool isStartTypeDownOverriden;

            public float StartTypeDownOverride;

            [FormerlySerializedAs("bEndDown")] public bool isEndDown;

            [FormerlySerializedAs("bEndTypeDownOverride")]
            public bool isEndTypeDownOverriden;

            public float EndTypeDownOverride;

            //Collision:
            public int CollisionType;

            [FormerlySerializedAs("bCollisionConvex")]
            public bool isCollisionConvex;

            [FormerlySerializedAs("bSimpleCollisionAutomatic")]
            public bool isSimpleCollisionAutomatic = true;

            [FormerlySerializedAs("bCollisionTrigger")]
            public bool isCollisionTrigger;

            public Vector3 CollisionBoxBL;
            public Vector3 CollisionBoxBR;
            public Vector3 CollisionBoxTL;
            public Vector3 CollisionBoxTR;

            public Vector3 CollisionTriBL;
            public Vector3 CollisionTriBR;
            public Vector3 CollisionTriT;

            [FormerlySerializedAs("tName")] public string objectName = "ExtrudedObject";
        ...
        public class SplinatedMeshEditorMaker {
            ...
            public bool IsEqualToSMM(SplinatedMeshMaker _SMM) {
                if (_SMM.currentSplination != CurrentSplination) return false;
                if (_SMM.currentSplinationCap1 != CurrentSplinationCap1) return false;
                if (_SMM.currentSplinationCap2 != CurrentSplinationCap2) return false;

                if (!RootUtils.IsApproximately(_SMM.capHeightOffset1, CapHeightOffset1, 0.0001f)) return false;
                if (!RootUtils.IsApproximately(_SMM.capHeightOffset2, CapHeightOffset2, 0.0001f)) return false;

                if (_SMM.isMaterialOverriden != isMaterialOverriden) return false;

                if (_SMM.SplinatedMaterial1 != SplinatedMaterial1) return false;
                if (_SMM.SplinatedMaterial2 != SplinatedMaterial2) return false;

                if (_SMM.isExactSplination != isExactSplination) return false;
                if (_SMM.isMatchingRoadDefinition != isMatchingRoadDefinition) return false;
                if (_SMM.isMatchingRoadIncrements != isMatchingRoadIncrements) return false;
                if (_SMM.isTrimStart != isTrimStart) return false;
                if (_SMM.isTrimEnd != isTrimEnd) return false;
                if (_SMM.isMatchingTerrain != isMatchingTerrain) return false;
                if (!RootUtils.IsApproximately(_SMM.minMaxMod, MinMaxMod, 0.0001f)) return false;
                if (_SMM.isBridge != isBridge) return false;
                if (!RootUtils.IsApproximately(_SMM.vertexMatchingPrecision, VertexMatchingPrecision, 0.0001f))
                    return false;

                if (_SMM.isStretched != isStretched) return false;
                if (_SMM.isStretchedOffset != isStretchedLocOffset) return false;
                if (_SMM.isStretchedSize != isStretchedSize) return false;
                if (_SMM.boxColliderOffset != stretchedBCLocOffset) return false;
                if (_SMM.boxColliderSize != stretchedBCSize) return false;
                if (!RootUtils.IsApproximately(_SMM.stretchUVThreshold, stretchedUVThreshold, 0.0001f)) return false;
                if (_SMM.isStraightLineMatchStartEnd != isStraightLineMatchingStartEnd) return false;
                if (_SMM.isBoxColliderFlippedOnX != isBCFlippedX) return false;
                if (_SMM.isBoxColliderFlippedOnZ != isBCFlippedZ) return false;

                //Horizontal offsets:
                if (!RootUtils.IsApproximately(_SMM.horizontalSep, HorizontalSep, 0.0001f)) return false;
                if (_SMM.horizontalCurve != HorizontalCurve) return false;
                //Vertical offset:
                if (!RootUtils.IsApproximately(_SMM.verticalRaise, VerticalRaise, 0.0001f)) return false;
                if (_SMM.verticalCurve != VerticalCurve) return false;
                //Vertical cutoff:
                if (!RootUtils.IsApproximately(_SMM.VerticalCutoff, VerticalCutoff, 0.0001f)) return false;
                if (_SMM.isVerticalCutoff != isVerticalCutoff) return false;
                if (_SMM.isVerticalCutoffDownwards != isVerticalCutoffDownwards) return false;
                if (_SMM.isVerticalMeshCutoffOppositeDir != isVerticalMeshCutoffOppositeDir) return false;
                if (!RootUtils.IsApproximately(_SMM.VerticalMeshCutoffOffset, VerticalMeshCutoffOffset, 0.0001f))
                    return false;
                if (_SMM.isVerticalCutoffMatchingZero != isVerticalCutoffMatchingZero) return false;

                if (!RootUtils.IsApproximately(_SMM.RoadRaise, RoadRaise, 0.0001f)) return false;
                if (_SMM.CustomRotation != CustomRotation) return false;
                if (_SMM.isFlippingRotation != isFlippedRotation) return false;
                if (_SMM.isStatic != isStatic) return false;
                if (!RootUtils.IsApproximately(_SMM.StartTime, StartTime, 0.0001f)) return false;
                if (!RootUtils.IsApproximately(_SMM.EndTime, EndTime, 0.0001f)) return false;
                if (_SMM.Axis != Axis) return false;

                if (_SMM.RepeatUVType != repeatUVType) return false;

                //Ending objects:
                if (_SMM.EndCapStart != EndCapStart) return false;
                if (_SMM.EndCapEnd != EndCapEnd) return false;

                if (_SMM.isEndCapCustomMatchStart != isEndCapCustomMatchingStart) return false;
                if (_SMM.EndCapCustomOffsetStart != EndCapCustomOffsetStart) return false;
                if (_SMM.EndCapCustomOffsetEnd != EndCapCustomOffsetEnd) return false;
                if (_SMM.EndCapCustomRotOffsetStart != EndCapCustomRotOffsetStart) return false;
                if (_SMM.EndCapCustomRotOffsetEnd != EndCapCustomRotOffsetEnd) return false;
                if (_SMM.isEndObjectsMatchingGround != isEndObjectsMatchingGround) return false;

                //Endings down:
                if (_SMM.isStartDown != isStartDown) return false;
                if (_SMM.isStartTypeDownOverriden != isStartTypeDownOverriden) return false;
                if (!RootUtils.IsApproximately(_SMM.startTypeDownOverride, startTypeDownOverriden, 0.0001f))
                    return false;
                if (_SMM.isEndDown != isEndDown) return false;
                if (_SMM.isEndTypeDownOverriden != isEndTypeDownOverriden) return false;
                if (!RootUtils.IsApproximately(_SMM.endTypeDownOverride, endTypeDownOverriden, 0.0001f)) return false;

                //Collision:
                if (_SMM.CollisionType != collisionType) return false;
                if (_SMM.isCollisionConvex != isCollisionConvex) return false;
                if (_SMM.isSimpleCollisionAutomatic != isSimpleCollisionAutomatic) return false;
                if (_SMM.isCollisionTrigger != isCollisionTriggered) return false;

                if (_SMM.CollisionBoxBL != CollisionBoxBL) return false;
                if (_SMM.CollisionBoxBR != CollisionBoxBR) return false;
                if (_SMM.CollisionBoxTL != CollisionBoxTL) return false;
                if (_SMM.CollisionBoxTR != CollisionBoxTR) return false;

                if (_SMM.CollisionTriBL != CollisionTriBL) return false;
                if (_SMM.CollisionTriBR != CollisionTriBR) return false;
                if (_SMM.CollisionTriT != CollisionTriT) return false;

                if (string.CompareOrdinal(_SMM.objectName, objectName) != 0) return false;
            ...
            public GameObject CurrentSplination;
            public GameObject CurrentSplinationCap1;
            public GameObject CurrentSplinationCap2;

            [FormerlySerializedAs("bMaterialOverride")]
            public bool isMaterialOverriden;

            public Material SplinatedMaterial1;
            public Material SplinatedMaterial2;
            public float CapHeightOffset1;
            public float CapHeightOffset2;
            ...
            [FormerlySerializedAs("bToggle")] public bool isToggled = false;
            ...
            [FormerlySerializedAs("StretchBC_LocOffset")]
            public Vector3 stretchedBCLocOffset;

            [FormerlySerializedAs("StretchBC_Size")]
            public Vector3 stretchedBCSize;
            ...
            [FormerlySerializedAs("bBCFlipX")] public bool isBCFlippedX;

            [FormerlySerializedAs("bBCFlipZ")] public bool isBCFlippedZ;
            ...
            [FormerlySerializedAs("RepeatUVType")] public RepeatUVTypeEnum repeatUVType = RepeatUVTypeEnum.None;

            //End objects:
            public GameObject EndCapStart;
            public GameObject EndCapEnd;
            ...
            [FormerlySerializedAs("StartTypeDownOverride")]
            public float startTypeDownOverriden;

            [FormerlySerializedAs("EndTypeDownOverride")]
            public float endTypeDownOverriden;
        ...
        [FormerlySerializedAs("bStraightLineMatchStartEnd")]
        public bool isStraightLineMatchStartEnd;

        [FormerlySerializedAs("bFlipRotation")]
        public bool isFlippingRotation;
        ...
        [FormerlySerializedAs("MasterObjTrans")]
        public Transform masterObjTrans;

        [FormerlySerializedAs("CurrentSplination")]
        public GameObject currentSplination;

        [FormerlySerializedAs("CurrentSplinationCap1")]
        public GameObject currentSplinationCap1;

        [FormerlySerializedAs("CurrentSplinationCap2")]
        public GameObject currentSplinationCap2;

        [FormerlySerializedAs("CurrentSplinationString")]
        public string currentSplinationString = "";

        [FormerlySerializedAs("CurrentSplinationCap1String")]
        public string currentSplinationCap1String = "";

        [FormerlySerializedAs("CurrentSplinationCap2String")]
        public string currentSplinationCap2String = "";

        [FormerlySerializedAs("CapHeightOffset1")]
        public float capHeightOffset1;

        [FormerlySerializedAs("CapHeightOffset2")]
        public float capHeightOffset2;

        [FormerlySerializedAs("MinMaxMod")] public float minMaxMod = 0.002f;

        [FormerlySerializedAs("VertexMatchingPrecision")]
        public float vertexMatchingPrecision = 0.005f;
        ...
        [FormerlySerializedAs("tMesh")] public Mesh mesh;
        ...
        [FormerlySerializedAs("tSpline")] public SplineC spline;
        ...
        public GameObject EndCapStart;
        public GameObject EndCapEnd;
        public GameObject EndCapStartOutput;
        public GameObject EndCapEndOutput;
        public string EndCapStartString = "";
        public string EndCapEndString = "";
        ...
        [FormerlySerializedAs("bCollisionExtrude")]
        public bool isCollisionExtrude;
        ...
        [FormerlySerializedAs("EM")] public SplinatedMeshEditorMaker splinatedMeshEditorMaker = null;
        ...
        public static Vector3 GetVector3Average(Vector3[] _vectors) {
            var tCount = _vectors.Length;
            var mVect = default(Vector3);
            for (var index = 0; index < tCount; index++) mVect += _vectors[index];
            mVect /= tCount;
            return mVect;
        }
        ...
        private static bool FloatsNear(float _near, float _val1, float _val2) {
            if (RootUtils.IsApproximately(_val1, _val2, _near)) return true;

            if (_val1 < _val2 + _near && _val1 > _val2 - _near) return true;
            if (_val2 < _val1 + _near && _val2 > _val1 - _near) return true;
            return false;
        }
        ...
        private static int[] GetCollisionTrisTri(int _meshCount, int _cTriCount, int _cCount) {
            var tCounter = 0;
            var tTris = new int[_cTriCount * 3];
            ...
            tCounter += 1;
            var tMod = -1;
            for (var index = 0; index < _meshCount; index++) {
                tMod = index * 3;
                //Bottom side: ***
        ...
        private static bool IsApproxTwoThirds(ref Vector3 _v1, Vector3 _v2, float _precision = 0.005f) {
            var cCount = 0;
            if (RootUtils.IsApproximately(_v1.x, _v2.x, _precision)) cCount += 1;
            if (RootUtils.IsApproximately(_v1.y, _v2.y, _precision)) cCount += 1;
            if (RootUtils.IsApproximately(_v1.z, _v2.z, _precision)) cCount += 1;
        ...
        private static bool IsApproxWithNeg(ref Vector3 _v1, ref Vector3 _v2) {
            ...
            if (cCount == 2) {
                if (!bXMatch && RootUtils.IsApproximately(_v1.x, _v2.x * -1f, 0.02f))
                    return true;
                if (!bYMatch && RootUtils.IsApproximately(_v1.y, _v2.y * -1f, 0.02f))
                    return true;
                if (!bZMatch && RootUtils.IsApproximately(_v1.z, _v2.z * -1f, 0.02f))
                    return true;
                return false;
            }
        ...
        private static bool V3EqualToNone(Vector3 _v1) {
            if (!RootUtils.IsApproximately(_v1.x, 0f, 0.0001f)) return false;
            if (!RootUtils.IsApproximately(_v1.y, 0f, 0.0001f)) return false;
            if (!RootUtils.IsApproximately(_v1.z, 0f, 0.0001f)) return false;
            return true;
        }
        ...
        private static bool V3EqualNormal(Vector3 _v1, Vector3 _v2) {
            if (!RootUtils.IsApproximately(_v1.x, _v2.x)) return false;
            if (!RootUtils.IsApproximately(_v1.y, _v2.y)) return false;
            if (!RootUtils.IsApproximately(_v1.z, _v2.z)) return false;
            return true;
        }


        /// <summary>
        ///     Returns true if coordinates of _v1 and _v2 do not match with tolerance 0.02f; _isZAxis checks x coordinate
        ///     instead of z
        /// </summary>
        private static bool IsApproxExtruded(ref Vector3 _v1, ref Vector3 _v2, bool _isZAxis) {
            if (!RootUtils.IsApproximately(_v1.y, _v2.y, 0.02f)) return false;

            if (_isZAxis) {
                if (!RootUtils.IsApproximately(_v1.x, _v2.x, 0.02f)) return false;
            }
            else {
                if (!RootUtils.IsApproximately(_v1.z, _v2.z, 0.02f)) return false;
            }
        ...
        private static float GetVHeightAtXY(ref Vector3 _vect1, ref Vector3 _vect2, ref Vector3 _vect3) {
            var tVect2D1 = new Vector2(_vect1.x, _vect1.z);
            var tVect2D2 = new Vector2(_vect2.x, _vect2.z);
            var tVect2D3 = new Vector2(_vect3.x, _vect3.z);

            var tDist1 = Vector2.Distance(tVect2D1, tVect2D3);
            var tDist2 = Vector2.Distance(tVect2D2, tVect2D3);
            var tDistSum = tDist1 + tDist2;

            var CloseTo1 = tDist1 / tDistSum;

            var tVect = (_vect2 - _vect1) * CloseTo1 + _vect1;
