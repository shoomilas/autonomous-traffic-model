...
namespace RoadArchitect {
    public static class Intersections {
        public static GameObject CreateIntersection(SplineN _node1, SplineN _node2) {
            var RoadMod = 10f;
            var SystemObj = _node1.transform.parent.parent.parent.gameObject;
            if (!SystemObj) {
            ...
            var cCount = SystemObj.transform.childCount;
            ...
            var intersectionObject = new GameObject("Inter" + (cCount + 1));
            intersectionObject.transform.parent = InterMaster.transform;
            var roadIntersection = intersectionObject.AddComponent<RoadIntersection>();
            ...
            if (tNode1.isEndPoint) {
                ...
                var isAlreadyNode = false;
                if (tNode1.idOnSpline == 1 || tNode1.idOnSpline == 0) isFirstNode = true;
                if (isFirstNode && tNode1.idOnSpline == 1 && tNode1.spline.nodes[0].isSpecialEndNodeIsStart)
                    isAlreadyNode = true;
                else if (!isFirstNode && tNode1.idOnSpline == tNode1.spline.GetNodeCount() - 2 &&
                         tNode1.spline.nodes[tNode1.spline.GetNodeCount() - 1].isSpecialEndNodeIsEnd)
                    isAlreadyNode = true;
                ...
                if (isFirstNode)
                    ...
                    tPos = tNode1.spline.GetSplineValue(0.999f, true).normalized *
                        (tNode1.spline.road.RoadWidth() * RoadMod) + tNode1.pos;
                ...
                var isFirstNode = false;
                var isAlreadyNode = false;
                if (tNode2.idOnSpline == 1 || tNode2.idOnSpline == 0) isFirstNode = true;
                if (isFirstNode && tNode2.idOnSpline == 1 && tNode2.spline.nodes[0].isSpecialEndNodeIsStart)
                    isAlreadyNode = true;
                else if (!isFirstNode && tNode2.idOnSpline == tNode2.spline.GetNodeCount() - 2 &&
                         tNode2.spline.nodes[tNode2.spline.GetNodeCount() - 1].isSpecialEndNodeIsEnd)
                    isAlreadyNode = true;
                ...
                if (isFirstNode)
                    ...
                    tPos = tNode2.spline.GetSplineValue(0.999f, true).normalized *
                        (tNode2.spline.road.RoadWidth() * RoadMod) + tNode2.pos;
...
#if UNITY_EDITOR
            Undo.RegisterCreatedObjectUndo(intersectionObject, "Created intersection");
#endif
        ...
        public static Vector3[] GetCornerVectorsTest(RoadIntersection _roadIntersection) {
            ...
            var tOffset = 5f;
            var tPos1 = tNode.time - tOffset / tSpline.distance;
            var tPos2 = tNode.time + tOffset / tSpline.distance;
        ...
        public static void CreateIntersection(RoadIntersection _roadIntersection) {
            ...
            var intersectionWidth = _roadIntersection.intersectionWidth * 1.25f;
            var colliders = Physics.OverlapSphere(_roadIntersection.transform.position, intersectionWidth);
            if (colliders == null || colliders.Length < 1) return;
            var roads = new List<Road>();
            foreach (var collider in colliders)
                if (collider.transform.parent) {
                    var tRoad = collider.transform.parent.GetComponent<Road>();
                    if (tRoad)
            ...
            var name = _roadIntersection.transform.name;
            Vector3[] vects;
        ...
        private static void FlattenIntersectionArea(ref List<Road> _roads, RoadIntersection _roadIntersection,
            float _width, out float _height) {
            //Cycle through each road and get all mesh vertices that are within range:
            var tInterPos = _roadIntersection.transform.position;
            var tInterPosY = tInterPos.y;
            foreach (var tRoad in _roads) {
                ...
                var VertCount = Road.vertices.Length;
                var bLeftToggle = true;
                for (var i = 0; i < VertCount; i += 2) {
        ...
        private static Vector3[] GetExtendedVectors(RoadIntersection _roadIntersection, bool _isPrimary = true) {
            ...
            var tOffset = tNode.spline.road.RoadWidth();
            var tOffset2 = tOffset * 0.5f;
            var tPos1 = tNode.time - tOffset / tSpline.distance;
            var tPos2 = tNode.time + tOffset / tSpline.distance;
            var tVect1 = tSpline.GetSplineValue(tPos1);
            var tVect2 = tSpline.GetSplineValue(tPos2);
        ...
        private static bool ClosestPointsOnTwoLines(out Vector3 _closestPointLine1, out Vector3 _closestPointLine2,
            Vector3 _linePoint1, Vector3 _lineVec1, Vector3 _linePoint2, Vector3 _lineVec2) {
            _closestPointLine1 = Vector3.zero;
            _closestPointLine2 = Vector3.zero;

            var a = Vector3.Dot(_lineVec1, _lineVec1);
            var b = Vector3.Dot(_lineVec1, _lineVec2);
            var e = Vector3.Dot(_lineVec2, _lineVec2);

            var d = a * e - b * b;

            //lines are not parallel
            if (d != 0.0f) {
                var r = _linePoint1 - _linePoint2;
                var c = Vector3.Dot(_lineVec1, r);
                var f = Vector3.Dot(_lineVec2, r);
        ...
        private static void CreateIntersectionMeshMain(RoadIntersection _roadIntersection, float _height,
            out Vector3[] _vectors, ref string _name) {
            //Get four points:
            var pVects = GetCornerVectors(_roadIntersection);
            var sVects = GetCornerVectors(_roadIntersection, false);
            _vectors = new Vector3[4];
            var oIntersection = new Vector3(0f, 0f, 0f);
            var oIntersection2 = new Vector3(0f, 0f, 0f); //Unused

            //			bool bIntersection;
            ClosestPointsOnTwoLines(out oIntersection, out oIntersection2, pVects[0], pVects[2] - pVects[0], sVects[0],
                sVects[2] - sVects[0]);
            _vectors[0] = oIntersection;
            _vectors[0].y = _height;

            ClosestPointsOnTwoLines(out oIntersection, out oIntersection2, pVects[0], pVects[2] - pVects[0], sVects[1],
                sVects[3] - sVects[1]);
            _vectors[1] = oIntersection;
            _vectors[1].y = _height;

            ClosestPointsOnTwoLines(out oIntersection, out oIntersection2, pVects[1], pVects[3] - pVects[1], sVects[0],
                sVects[2] - sVects[0]);
            _vectors[2] = oIntersection;
            _vectors[2].y = _height;

            ClosestPointsOnTwoLines(out oIntersection, out oIntersection2, pVects[1], pVects[3] - pVects[1], sVects[1],
                sVects[3] - sVects[1]);
        ...
        private static void CreateIntersectionMeshMainInternal(Vector3[] _verts, GameObject _obj, ref string _name) {
            ...
            for (var index = 0; index < MVL; index++) _verts[index] -= _obj.transform.position;
            ...
            for (var i = 0; i < MVL; i++) normals[i] = -Vector3.forward;
            ...
            if (!MF) MF = _obj.AddComponent<MeshFilter>();
            MF.sharedMesh = tMesh;
            //MeshToFile(MF, RoadEditorUtility.GetBasePath() + "/Mesh/Intersections/" + name +".obj");

            var MC = _obj.GetComponent<MeshCollider>();
            if (MC) Object.DestroyImmediate(MC);
            //if(!MC){ MC = iObj.AddComponent<MeshCollider>(); }
            //MC.sharedMesh = MF.sharedMesh;

            var MR = _obj.GetComponent<MeshRenderer>();
            if (!MR) MR = _obj.AddComponent<MeshRenderer>();
        ...
        private static void CreateIntersectionMeshOuter(RoadIntersection _roadIntersection, Vector3[] _vects,
            ref string _name) {
            var bVects1 = GetExtendedVectors(_roadIntersection);
            var bVects2 = GetExtendedVectors(_roadIntersection, false);
            ...
            var cCount = _roadIntersection.transform.childCount;
            //bool bOuter = false;
        ...
        private static void CreateIntersectionMeshOuterInternal(Vector3[] _verts, GameObject _obj,
            Vector3 _vectorOffset, ref string _name) {
            var tMesh = new Mesh();
            var MVL = 16;
            var triCount = (int)(16f * 1.5f);

            for (var index = 0; index < MVL; index += 4) {
            ...
            var normals = new Vector3[MVL];
            for (var i = 0; i < 4; i++) normals[i] = -Vector3.forward;
            tMesh.normals = normals;
            tMesh.RecalculateNormals();

            var uv = new Vector2[MVL];
            for (var i = 0; i < MVL; i += 4) {
        ...
        private static void InitializeIntersectionObjects(RoadIntersection _roadIntersection) {
            if (_roadIntersection != null) {
                ...
                Object[] allIntersections = Object.FindObjectsOfType<RoadIntersection>();
        ...
        private static void InitializeIntersectionObjectsInternal(RoadIntersection _roadIntersection) {
            ...
            var n1Spline = n1.spline;
            var n2Spline = n2.spline;
            var n3Spline = n3.spline;
            SplineC n4Spline = null;
            if (n4 != null) n4Spline = n4.spline;

            //Get the point:
            var n1Point = GetFourCornerPoint(ref n1Spline, ref n1, _roadIntersection);
            var n2Point = GetFourCornerPoint(ref n2Spline, ref n2, _roadIntersection);
            var n3Point = GetFourCornerPoint(ref n3Spline, ref n3, _roadIntersection);
            var n4Point = new Vector3(0f, 0f, 0f);
            if (n4Spline != null) n4Point = GetFourCornerPoint(ref n4Spline, ref n4, _roadIntersection);
            ...
            if (1 == 1 && _roadIntersection.intersectionType == RoadIntersection.IntersectionTypeEnum.ThreeWay) {
                ...
                if (tList.Count != 3) {
                    Debug.LogError(
                        "InitializeIntersections hashset != 3 connected on three way intersection, real size: " +
                        tList.Count + " on intersection: " + _roadIntersection.transform.name);
                    return;
                }

                ProcessFourCorners(ref n1Point, ref n2Point, _roadIntersection.transform.gameObject,
                    GetLongestSplineDistance(n1Spline, n2Spline));
                ProcessFourCorners(ref n1Point, ref n3Point, _roadIntersection.transform.gameObject,
                    GetLongestSplineDistance(n1Spline, n3Spline));
                ProcessFourCorners(ref n2Point, ref n3Point, _roadIntersection.transform.gameObject,
                    GetLongestSplineDistance(n2Spline, n3Spline));
            }
            else if (_roadIntersection.intersectionType == RoadIntersection.IntersectionTypeEnum.FourWay) {
                //Should be size 3:
                if (tList.Count != 4) {
                    Debug.LogError(
                        "InitializeIntersections hashset != 4 connected on four way intersection, real size: " +
                        tList.Count + " on intersection: " + _roadIntersection.transform.name);
                    return;
                }
                ...
                ProcessFourCorners(ref n1Point, ref n3Point, _roadIntersection.transform.gameObject,
                    GetLongestSplineDistance(n1Spline, n3Spline));
                ProcessFourCorners(ref n1Point, ref n4Point, _roadIntersection.transform.gameObject,
                    GetLongestSplineDistance(n1Spline, n4Spline));
                ProcessFourCorners(ref n2Point, ref n3Point, _roadIntersection.transform.gameObject,
                    GetLongestSplineDistance(n2Spline, n3Spline));
                ProcessFourCorners(ref n2Point, ref n4Point, _roadIntersection.transform.gameObject,
                    GetLongestSplineDistance(n2Spline, n4Spline));
                ProcessFourCorners(ref n3Point, ref n4Point, _roadIntersection.transform.gameObject,
                    GetLongestSplineDistance(n3Spline, n4Spline));
            }
        }
        ...
        private static SplineN GetNodeByID(int _ID) {
            Object[] SplineNodeObjects = Object.FindObjectsOfType<SplineN>();
        ...
        private static Vector3 GetFourCornerPoint(ref SplineC _spline, ref SplineN _node,
            RoadIntersection _roadIntersection) {
            SplineN iNode;
        ...
        private static void ProcessFourCorners(ref Vector3 _n1, ref Vector3 _n2, GameObject _intersectionObject,
            float _splineDistance) {
            ...
            var tAngle = AngleOfTriangle(Side2, Side3, Side1);
        ...
        private static float AngleOfTriangle(float _a, float _b, float _c) {
            var cAng = (_a * _a + _b * _b - _c * _c) / (2 * _a * _b);
            var radAngle = Mathf.Acos(cAng);
            var degAngle = Mathf.Rad2Deg * radAngle;
            return degAngle;
        }


        private static void ProcessTwoCorners(ref GameObject _intersectionObject, ref Vector3 _n1, ref Vector3 _n2,
            float _splineDistance) {
            ...
            //Calculate the angle:
            var v3BA = _n2 - _intersectionObject.transform.position;
            var v3BC = _n1 - _intersectionObject.transform.position;
            ...
            var tStep = 1.25f / _splineDistance;
            var bSuccess = MoveCorner(tStep, 3f, ref tCorner, ref _intersectionObject, v3);
            if (!bSuccess) {
        ...
        private static bool MoveCorner(float _step, float _dist, ref GameObject _corner,
            ref GameObject _intersectionObject, Vector3 _vector) {
            var tStart = 0.05f;
            var isDone = false;
            var spamguard = 0;
            var isRoadHit = false;
            ...
            while (!isDone) {
                ...
                for (var k = 0; k < tCollider.Length; k++)
