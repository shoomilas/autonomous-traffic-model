...
namespace RoadArchitect {
    public class RoadConstructorBufferMaker {
        ...
        public RoadConstructorBufferMaker(Road _road, RoadUpdateTypeEnum _updateType) {
            ...
            isRoadOn = tUpdateType == RoadUpdateTypeEnum.Full || tUpdateType == RoadUpdateTypeEnum.Intersection ||
                       tUpdateType == RoadUpdateTypeEnum.Bridges;
            isTerrainOn = tUpdateType == RoadUpdateTypeEnum.Full || tUpdateType == RoadUpdateTypeEnum.Intersection ||
                          tUpdateType == RoadUpdateTypeEnum.Bridges;
        ...
        private static void SaveMesh(SaveMeshTypeEnum _saveType, Mesh _mesh, Road _road, string _name) {
            if (!_road.isSavingMeshes) return;
            ...
            else if (_saveType == SaveMeshTypeEnum.RoadConn) folderName = Path.Combine(folderName, "RoadConn");

            var path = Path.GetDirectoryName(Application.dataPath);
            path = Path.Combine(path, folderName);
            if (!Directory.Exists(path)) Directory.CreateDirectory(path);

            var roadName = _road.transform.name;
            var finalName = Path.Combine(folderName, sceneName + "-" + roadName + "-" + _name + ".asset");
        ...
        public bool tMeshSkip;
        public bool tMesh_SRSkip;
        public bool tMesh_SLSkip;
        ...
        [FormerlySerializedAs("bRoadOn")] public bool isRoadOn = true;

        [FormerlySerializedAs("bTerrainOn")] public bool isTerrainOn = true;

        [FormerlySerializedAs("bBridgesOn")] public bool isBridgesOn = true;

        [FormerlySerializedAs("bInterseOn")] public bool isInterseOn = true;
        ...
        private void InitGameObjects() {
            ...
            if (road.MainMeshes != null) {
                var MFArray = road.MainMeshes.GetComponentsInChildren<MeshFilter>();
                var MCArray = road.MainMeshes.GetComponentsInChildren<MeshCollider>();

                var MFArrayCount = MFArray.Length;
                var MCArrayCount = MCArray.Length;
                for (var index = MFArrayCount - 1; index > -1; index--) MFArray[index].sharedMesh = null;
                for (var index = MCArrayCount - 1; index > -1; index--) MCArray[index].sharedMesh = null;
        ...
        /// <summary>
        ///     Creates meshes and assigns vertices, triangles and normals. If multithreading enabled, this occurs inbetween
        ///     threaded jobs since unity library can't be used in threads.
        /// </summary>
        public void MeshSetup1() {
            Mesh MeshBuffer = null;

            if (isInterseOn) MeshSetup1IntersectionObjectsSetup();
            ...
            if (isRoadOn) {
                ...
                if (ShoulderR_Vectors.Count < 64000) {
                    ...
                    tMesh_SR = MeshSetup1Helper(ref tMesh_SR, ShoulderR_Vectors.ToArray(), ref tris_ShoulderR,
                        ref normals_ShoulderR);
                    tMesh_SRSkip = false;
                }
                else {
                    tMesh_SRSkip = true;
                ...
                if (ShoulderL_Vectors.Count < 64000) {
                    ...
                    tMesh_SL = MeshSetup1Helper(ref tMesh_SL, ShoulderL_Vectors.ToArray(), ref tris_ShoulderL,
                        ref normals_ShoulderL);
                    tMesh_SLSkip = false;
                }
                else {
                    tMesh_SLSkip = true;
                ...
                if (RoadConnections_verts.Count > 0) {
                    Mesh qMesh = null;
                    for (var index = 0; index < RoadConnections_verts.Count; index++) {
                        qMesh = new Mesh();
                ...
                if ((road.isRoadCutsEnabled || road.isDynamicCutsEnabled) && RoadCuts.Count > 0) {
                    ...
                    var cCount = cut_RoadVectors.Count;
                    for (var index = 0; index < cCount; index++) {
                        tTris = cut_tris[index];
                        tNormals = cut_normals[index];
                        MeshBuffer = new Mesh();
                        tMesh_RoadCuts.Add(MeshSetup1Helper(ref MeshBuffer, cut_RoadVectors[index].ToArray(), ref tTris,
                            ref tNormals));
                        MeshBuffer = new Mesh();
                        tMesh_RoadCuts_world.Add(MeshSetup1Helper(ref MeshBuffer, cut_RoadVectors[index].ToArray(),
                            ref tTris, ref tNormals));
                ...
                if (road.isShoulderCutsEnabled || road.isDynamicCutsEnabled) {
                    int[] tTris = null;
                    Vector3[] tNormals = null;
                    var rCount = cut_ShoulderR_Vectors.Count;
                    for (var index = 0; index < rCount; index++) {
                        tTris = cut_tris_ShoulderR[index];
                        tNormals = cut_normals_ShoulderR[index];
                        MeshBuffer = new Mesh();
                        tMesh_SRCuts.Add(MeshSetup1Helper(ref MeshBuffer, cut_ShoulderR_Vectors[index].ToArray(),
                            ref tTris, ref tNormals));
                        MeshBuffer = new Mesh();
                        tMesh_SRCuts_world.Add(MeshSetup1Helper(ref MeshBuffer, cut_ShoulderR_Vectors[index].ToArray(),
                            ref tTris, ref tNormals));
                        cut_normals_ShoulderR[index] = tNormals;
                        tMesh_SRSkip = true;
                    }

                    if (rCount <= 0) tMesh_SRSkip = false;
                    var lCount = cut_ShoulderL_Vectors.Count;
                    for (var index = 0; index < lCount; index++) {
                        tTris = cut_tris_ShoulderL[index];
                        tNormals = cut_normals_ShoulderL[index];
                        MeshBuffer = new Mesh();
                        tMesh_SLCuts.Add(MeshSetup1Helper(ref MeshBuffer, cut_ShoulderL_Vectors[index].ToArray(),
                            ref tTris, ref tNormals));
                        MeshBuffer = new Mesh();
                        tMesh_SLCuts_world.Add(MeshSetup1Helper(ref MeshBuffer, cut_ShoulderL_Vectors[index].ToArray(),
                            ref tTris, ref tNormals));
                        cut_normals_ShoulderL[index] = tNormals;
                        tMesh_SLSkip = true;
                    }

                    if (lCount <= 0) tMesh_SLSkip = false;
                }
            }

            if (isInterseOn) MeshSetup1IntersectionParts();
        ...
        private void MeshSetup1IntersectionObjectsSetup() {
            var nodeCount = road.spline.GetNodeCount();
            var roadIntersections = new List<RoadIntersection>();
            ...
            foreach (var intersection in roadIntersections) {
                ...
                if (intersection.intersectionStopType == RoadIntersection.iStopTypeEnum.StopSign_AllWay) {
                    IntersectionObjects.CreateStopSignsAllWay(intersection.transform.gameObject);
                }
                else if (intersection.intersectionStopType == RoadIntersection.iStopTypeEnum.TrafficLight1) {
                    IntersectionObjects.CreateTrafficLightBases(intersection.transform.gameObject);
                }
        ...
        private void MeshSetup1IntersectionParts() {
            var mCount = road.spline.GetNodeCount();
            var bHasInter = false;
        ...
        /// <summary>
        ///     Assigns UV and tangents to meshes. If multithreading enabled, this occurs after the last threaded job since unity
        ///     library can't be used in threads.
        /// </summary>
        public void MeshSetup2() {
            ...
            if (isRoadOn) {
                ...
                var markerMaterialsField = road.GetRoadMarkerMaterials();
                var roadMaterialsField = road.GetRoadWorldMaterials();
                // shoulder fields only contain data when isShouldersEnabled
                var shoulderMaterialsField = road.GetShoulderWorldMaterials();
                var shoulderMarkerMaterialsField = road.GetShoulderMarkerMaterials();


                //If road cuts is off, full size UVs:
                if (!road.isRoadCutsEnabled && !road.isDynamicCutsEnabled || RoadCuts == null || RoadCuts.Count <= 0) {
                    if (tMesh != null) {
                        tMesh = MeshSetup2Helper(ref tMesh, uv, tangents, ref road.MeshRoad, shoulderMaterialsField,
                            markerMaterialsField, roadMaterialsField, true);
                        ...
                        // Copy mesh values
                        Array.Copy(tMesh.vertices, ooVerts, ooVerts.Length);
                        Array.Copy(tMesh.triangles, ooTris, ooTris.Length);
                        Array.Copy(tMesh.normals, ooNormals, ooNormals.Length);
                        Array.Copy(uv2, ooUV, ooUV.Length);
                        Array.Copy(tangents2, ooTangents, ooTangents.Length);
                        ...
                        pMesh = MeshSetup2Helper(ref pMesh, uv2, tangents2, ref gObj, shoulderMaterialsField,
                            markerMaterialsField, roadMaterialsField, false);
                    ...
                    for (var i = 0; i < cCount; i++) {
                        ...
                        if (MeshMainBuffer != null) {
                            MeshSetup2HelperRoadCuts(i, ref MeshMainBuffer, cut_uv_world[i], cut_tangents_world[i],
                                ref road.MeshRoad, false, out CreatedMainObj, markerMaterialsField, roadMaterialsField);
                            SaveMesh(SaveMeshTypeEnum.RoadCut, MeshMainBuffer, road, "RoadCut" + i);
                        }
                        ...
                        if (MeshMarkerBuffer != null) {
                            bHasMats = MeshSetup2HelperRoadCuts(i, ref MeshMarkerBuffer, cut_uv[i], cut_tangents[i],
                                ref CreatedMainObj, true, out CreatedMarkerObj, markerMaterialsField,
                                roadMaterialsField);
                            if (bHasMats) {
                                SaveMesh(SaveMeshTypeEnum.RoadCut, MeshMarkerBuffer, road, "RoadCutMarker" + i);
                            }
                            else {
                    ...
                    if (road.MeshRoad != null) {
                        var tMC = road.MeshRoad.GetComponent<MeshCollider>();
                        var tMR = road.MeshRoad.GetComponent<MeshRenderer>();
                        if (tMC != null) Object.DestroyImmediate(tMC);
                        if (tMR != null) Object.DestroyImmediate(tMR);
                    }

                    if (tMesh != null) Object.DestroyImmediate(tMesh);
                }


                //Shoulders:
                if (road.isShouldersEnabled) {
                    if (!road.isShoulderCutsEnabled && !road.isDynamicCutsEnabled || ShoulderCutsL == null ||
                        cut_ShoulderL_Vectors.Count <= 0) {
                        //Right road shoulder:
                        if (tMesh_SR != null) {
                            tMesh_SR = MeshSetup2Helper(ref tMesh_SR, uv_SR, tangents_SR, ref road.MeshShoR,
                                shoulderMaterialsField, markerMaterialsField, roadMaterialsField, false, true);
                            SaveMesh(SaveMeshTypeEnum.Shoulder, tMesh_SR, road, road.MeshShoR.transform.name);
                        }

                        //Left road shoulder:
                        if (tMesh_SL != null) {
                            tMesh_SL = MeshSetup2Helper(ref tMesh_SL, uv_SL, tangents_SL, ref road.MeshShoL,
                                shoulderMaterialsField, markerMaterialsField, roadMaterialsField, false, true);
                        ...
                        for (var index = 0; index < rCount; index++) {
                            ...
                            if (MeshMainBuffer != null) {
                                MeshSetup2HelperCutsShoulder(index, ref MeshMainBuffer, cut_uv_SR_world[index],
                                    cut_tangents_SR_world[index], ref road.MeshShoR, false, false, out CreatedMainObj,
                                    shoulderMarkerMaterialsField, shoulderMaterialsField);
                                SaveMesh(SaveMeshTypeEnum.SCut, MeshMainBuffer, road, "SCutR" + index);
                            }

                            CreatedMarkerObj = null;
                            MeshMarkerBuffer = tMesh_SRCuts[index];
                            if (MeshMarkerBuffer != null) {
                                bHasMats = MeshSetup2HelperCutsShoulder(index, ref MeshMarkerBuffer, cut_uv_SR[index],
                                    cut_tangents_SR[index], ref CreatedMainObj, false, true, out CreatedMarkerObj,
                                    shoulderMarkerMaterialsField, shoulderMaterialsField);
                                if (bHasMats) {
                                    SaveMesh(SaveMeshTypeEnum.SCut, MeshMarkerBuffer, road, "SCutRMarker" + index);
                                }
                                else {
                                    //Destroy if no marker materials:
                                    Object.DestroyImmediate(CreatedMarkerObj);
                        ...
                        for (var index = 0; index < lCount; index++) {
                            ...
                            if (MeshMainBuffer != null) {
                                MeshSetup2HelperCutsShoulder(index, ref MeshMainBuffer, cut_uv_SL_world[index],
                                    cut_tangents_SL_world[index], ref road.MeshShoL, true, false, out CreatedMainObj,
                                    shoulderMarkerMaterialsField, shoulderMaterialsField);
                                SaveMesh(SaveMeshTypeEnum.SCut, MeshMainBuffer, road, "SCutL" + index);
                            }
                            ...
                            if (MeshMarkerBuffer != null) {
                                bHasMats = MeshSetup2HelperCutsShoulder(index, ref MeshMarkerBuffer, cut_uv_SL[index],
                                    cut_tangents_SL[index], ref CreatedMainObj, true, true, out CreatedMarkerObj,
                                    shoulderMarkerMaterialsField, shoulderMaterialsField);
                                if (bHasMats) {
                                    SaveMesh(SaveMeshTypeEnum.SCut, MeshMarkerBuffer, road, "SCutLMarker" + index);
                                }
                                else {
                                    //Destroy if no marker materials:
                                    Object.DestroyImmediate(CreatedMarkerObj);
                    ...
                    Mesh tBuffer = null;
                    var xCount = tMesh_SRCuts_world.Count;
                    ...
                    if (road.MeshShoR != null) Object.DestroyImmediate(road.MeshShoR);

                    if (road.MeshShoL != null) Object.DestroyImmediate(road.MeshShoL);
                }

                var basePath = RoadEditorUtility.GetBasePath();

                for (var index = 0; index < RoadConnections_tangents.Count; index++) {
                    ...
                    var fDist = Vector3.Distance(RoadConnections_verts[index][2], RoadConnections_verts[index][3]);
                    ...
                    vUV[0] = new Vector2(RoadConnections_verts[index][0].x / 5f,
                        RoadConnections_verts[index][0].z / 5f);
                    vUV[1] = new Vector2(RoadConnections_verts[index][1].x / 5f,
                        RoadConnections_verts[index][1].z / 5f);
                    vUV[2] = new Vector2(RoadConnections_verts[index][2].x / 5f,
                        RoadConnections_verts[index][2].z / 5f);
                    vUV[3] = new Vector2(RoadConnections_verts[index][3].x / 5f,
                        RoadConnections_verts[index][3].z / 5f);
                    vMesh.uv = vUV;
                    vMesh.RecalculateNormals();
                    RoadConnections_normals[index] = vMesh.normals;
                    vMesh.tangents =
                        RootUtils.ProcessTangents(vMesh.triangles, vMesh.normals, vMesh.uv, vMesh.vertices);
                    ...
                    SaveMesh(SaveMeshTypeEnum.RoadConn, vMesh, road, "RoadConn" + index);
                }
            }

            if (isInterseOn) MeshSetup2Intersections();

            if (road.MeshiLanes != null) Object.DestroyImmediate(road.MeshiLanes);
            if (road.MeshiLanes0 != null) Object.DestroyImmediate(road.MeshiLanes0);
            if (road.MeshiLanes1 != null) Object.DestroyImmediate(road.MeshiLanes1);
            if (road.MeshiLanes2 != null) Object.DestroyImmediate(road.MeshiLanes2);
            if (road.MeshiLanes3 != null) Object.DestroyImmediate(road.MeshiLanes3);
            if (road.MeshiMainPlates != null) Object.DestroyImmediate(road.MeshiMainPlates);
            if (road.MeshiMarkerPlates != null) Object.DestroyImmediate(road.MeshiMarkerPlates);
            ...
            if (road.isRoadCutsEnabled || road.isShoulderCutsEnabled || road.isDynamicCutsEnabled) {
                var mCount = road.spline.GetNodeCount();
                for (var index = 0; index < mCount; index++) road.spline.nodes[index].UpdateCuts();
            }
        }
        ...
        private void RemoveMainMeshes() {
            //Remove main mesh stuff if necessary:
            if (tMesh_SR != null) Object.DestroyImmediate(tMesh_SR);
            if (tMesh_SL != null) Object.DestroyImmediate(tMesh_SL);

            if (road.MeshShoR != null) {
                var tMC = road.MeshShoR.GetComponent<MeshCollider>();
                var tMR = road.MeshShoR.GetComponent<MeshRenderer>();
                if (tMC != null) Object.DestroyImmediate(tMC);
                if (tMR != null) Object.DestroyImmediate(tMR);
            }

            if (road.MeshShoL != null) {
                var tMC = road.MeshShoL.GetComponent<MeshCollider>();
                var tMR = road.MeshShoL.GetComponent<MeshRenderer>();
                if (tMC != null) Object.DestroyImmediate(tMC);
                if (tMR != null) Object.DestroyImmediate(tMR);
            }
        }
        ...
        private void MeshSetup2Intersections() {
            ...
            MeshFilter MF = null;
            var tCombineDict_Lane0 = new Dictionary<RoadIntersection, List<MeshFilter>>();
            var tCombineDict_Lane1 = new Dictionary<RoadIntersection, List<MeshFilter>>();
            var tCombineDict_Lane2 = new Dictionary<RoadIntersection, List<MeshFilter>>();
            var tCombineDict_Lane3 = new Dictionary<RoadIntersection, List<MeshFilter>>();
            var tCombineDict_MainPlate = new Dictionary<RoadIntersection, List<MeshFilter>>();
            var tCombineDict_MainPlateM = new Dictionary<RoadIntersection, List<MeshFilter>>();
            var uniqueRoadIntersection = new HashSet<RoadIntersection>();

            var tCombineDict_Lane1_Disabled = new Dictionary<RoadIntersection, List<MeshFilter>>();
            var tCombineDict_Lane2_Disabled = new Dictionary<RoadIntersection, List<MeshFilter>>();
            var tCombineDict_Lane2_DisabledActive = new Dictionary<RoadIntersection, List<MeshFilter>>();
            var tCombineDict_Lane2_DisabledActiveR = new Dictionary<RoadIntersection, List<MeshFilter>>();
            var tCombineDict_Lane3_Disabled = new Dictionary<RoadIntersection, List<MeshFilter>>();
            var tCombineDict_Lane1_DisabledActive = new Dictionary<RoadIntersection, List<MeshFilter>>();
            ...
            for (var index = 0; index < vCount; index++) {
                ...
                MF = MeshSetup2IntersectionHelper(ref xMesh, ref tUV, ref tTangents, ref road.MeshiLanes0, "Lane0B",
                    basePath + "/Materials/Markers/InterWhiteLYellowR.mat");
            ...
            vCount = iBLane1s.Count;
            for (var index = 0; index < vCount; index++) {
                var isPrimaryNode = iBLane1s_tID[index].node1 == iBLane1s_nID[index];
                if (!isPrimaryNode &&
                    iBLane1s_tID[index].intersectionType == RoadIntersection.IntersectionTypeEnum.ThreeWay &&
                    (iBLane1s_tID[index].roadType == RoadIntersection.RoadTypeEnum.TurnLane ||
                     iBLane1s_tID[index].roadType == RoadIntersection.RoadTypeEnum.BothTurnLanes) &&
                    !iBLane1s_tID[index].isNode2BLeftTurnLane) {
                    tUV = iBLane1s_uv[index];
                    tTangents = iBLane1s_tangents[index];
                    xMesh = tMesh_iBLanes1[index];
                    MF = MeshSetup2IntersectionHelper(ref xMesh, ref tUV, ref tTangents, ref road.MeshiLanes1,
                        "LaneD1B", basePath + "/Materials/Markers/InterLaneDisabled.mat");
                    ...
                    xMesh = tMesh_iBLanes1[index];
                    MF = MeshSetup2IntersectionHelper(ref xMesh, ref tUV, ref tTangents, ref road.MeshiLanes1,
                        "LaneDA1B", basePath + "/Materials/Markers/InterLaneDisabledOuter.mat");
                    ...
                    tTangents = iBLane1s_tangents[index];
                    xMesh = tMesh_iBLanes1[index];
                    MF = MeshSetup2IntersectionHelper(ref xMesh, ref tUV, ref tTangents, ref road.MeshiLanes1, "Lane1B",
                        basePath + "/Materials/Markers/InterYellowLWhiteR.mat");
            ...
            vCount = iBLane2s.Count;
            for (var index = 0; index < vCount; index++) {
                var bPrimaryNode = iBLane2s_tID[index].node1 == iBLane2s_nID[index];
                if (!bPrimaryNode &&
                    iBLane2s_tID[index].intersectionType == RoadIntersection.IntersectionTypeEnum.ThreeWay &&
                    (iBLane2s_tID[index].roadType == RoadIntersection.RoadTypeEnum.TurnLane ||
                     iBLane2s_tID[index].roadType == RoadIntersection.RoadTypeEnum.BothTurnLanes) &&
                    !iBLane2s_tID[index].isNode2BLeftTurnLane) {
                    tUV = iBLane2s_uv[index];
                    tTangents = iBLane2s_tangents[index];
                    xMesh = tMesh_iBLanes2[index];
                    MF = MeshSetup2IntersectionHelper(ref xMesh, ref tUV, ref tTangents, ref road.MeshiLanes2,
                        "LaneDA2B", basePath + "/Materials/Markers/InterLaneDisabledOuter.mat");
                ...
                else if (!bPrimaryNode &&
                         iBLane2s_tID[index].intersectionType == RoadIntersection.IntersectionTypeEnum.ThreeWay &&
                         iBLane2s_tID[index].roadType == RoadIntersection.RoadTypeEnum.BothTurnLanes &&
                         !iBLane2s_tID[index].isNode2BRightTurnLane) {
                    tUV = iBLane2s_uv[index];
                    tTangents = iBLane2s_tangents[index];
                    xMesh = tMesh_iBLanes2[index];
                    MF = MeshSetup2IntersectionHelper(ref xMesh, ref tUV, ref tTangents, ref road.MeshiLanes2,
                        "LaneDA2B", basePath + "/Materials/Markers/InterLaneDisabledOuterR.mat");
                    ...
                    xMesh = tMesh_iBLanes2[index];
                    MF = MeshSetup2IntersectionHelper(ref xMesh, ref tUV, ref tTangents, ref road.MeshiLanes2,
                        "LaneD2B", basePath + "/Materials/Markers/InterLaneDisabled.mat");
                    ...
                    tTangents = iBLane2s_tangents[index];
                    xMesh = tMesh_iBLanes2[index];
                    MF = MeshSetup2IntersectionHelper(ref xMesh, ref tUV, ref tTangents, ref road.MeshiLanes2, "Lane2B",
                        basePath + "/Materials/Markers/InterWhiteR.mat");
            ...
            vCount = iBLane3s.Count;
            for (var index = 0; index < vCount; index++) {
                var bPrimaryNode = iBLane3s_tID[index].node1 == iBLane3s_nID[index];
                if (!bPrimaryNode &&
                    iBLane3s_tID[index].intersectionType == RoadIntersection.IntersectionTypeEnum.ThreeWay &&
                    iBLane3s_tID[index].roadType == RoadIntersection.RoadTypeEnum.BothTurnLanes &&
                    !iBLane3s_tID[index].isNode2BRightTurnLane) {
                    tUV = iBLane3s_uv[index];
                    tTangents = iBLane3s_tangents[index];
                    xMesh = tMesh_iBLanes3[index];
                    MF = MeshSetup2IntersectionHelper(ref xMesh, ref tUV, ref tTangents, ref road.MeshiLanes3,
                        "LaneD3B", basePath + "/Materials/Markers/InterLaneDisabled.mat");
                    ...
                    tTangents = iBLane3s_tangents[index];
                    xMesh = tMesh_iBLanes3[index];
                    MF = MeshSetup2IntersectionHelper(ref xMesh, ref tUV, ref tTangents, ref road.MeshiLanes3, "Lane3B",
                        basePath + "/Materials/Markers/InterWhiteR.mat");
            ...
            for (var index = 0; index < vCount; index++) {
                ...
                MF = MeshSetup2IntersectionHelper(ref xMesh, ref tUV, ref tTangents, ref road.MeshiLanes0, "Lane0F",
                    basePath + "/Materials/Markers/InterWhiteLYellowR.mat");
            ...
            vCount = iFLane1s.Count;
            for (var index = 0; index < vCount; index++) {
                var bPrimaryNode = iFLane1s_tID[index].node1 == iFLane1s_nID[index];
                if (!bPrimaryNode &&
                    iFLane1s_tID[index].intersectionType == RoadIntersection.IntersectionTypeEnum.ThreeWay &&
                    (iFLane1s_tID[index].roadType == RoadIntersection.RoadTypeEnum.BothTurnLanes ||
                     iFLane1s_tID[index].roadType == RoadIntersection.RoadTypeEnum.TurnLane) &&
                    !iFLane1s_tID[index].isNode2FLeftTurnLane) {
                    tUV = iFLane1s_uv[index];
                    tTangents = iFLane1s_tangents[index];
                    xMesh = tMesh_iFLanes1[index];
                    MF = MeshSetup2IntersectionHelper(ref xMesh, ref tUV, ref tTangents, ref road.MeshiLanes1,
                        "LaneD1F", basePath + "/Materials/Markers/InterLaneDisabled.mat");
                    ...
                    xMesh = tMesh_iFLanes1[index];
                    MF = MeshSetup2IntersectionHelper(ref xMesh, ref tUV, ref tTangents, ref road.MeshiLanes1,
                        "LaneDAR1F", basePath + "/Materials/Markers/InterLaneDisabledOuterR.mat");
                    ...
                    tTangents = iFLane1s_tangents[index];
                    xMesh = tMesh_iFLanes1[index];
                    MF = MeshSetup2IntersectionHelper(ref xMesh, ref tUV, ref tTangents, ref road.MeshiLanes1, "Lane1F",
                        basePath + "/Materials/Markers/InterYellowLWhiteR.mat");
            ...
            vCount = iFLane2s.Count;
            for (var index = 0; index < vCount; index++) {
                var bPrimaryNode = iFLane2s_tID[index].node1 == iFLane2s_nID[index];
                if (!bPrimaryNode &&
                    iFLane2s_tID[index].intersectionType == RoadIntersection.IntersectionTypeEnum.ThreeWay &&
                    (iFLane2s_tID[index].roadType == RoadIntersection.RoadTypeEnum.BothTurnLanes ||
                     iFLane2s_tID[index].roadType == RoadIntersection.RoadTypeEnum.TurnLane) &&
                    !iFLane2s_tID[index].isNode2FLeftTurnLane) {
                    tUV = iFLane2s_uv[index];
                    tTangents = iFLane2s_tangents[index];
                    xMesh = tMesh_iFLanes2[index];
                    MF = MeshSetup2IntersectionHelper(ref xMesh, ref tUV, ref tTangents, ref road.MeshiLanes2,
                        "LaneDA2F", basePath + "/Materials/Markers/InterLaneDisabledOuter.mat");
                    ...
                    tCombineDict_Lane2_DisabledActive[iFLane2s_tID[index]].Add(MF);
                         ...
                         iFLane2s_tID[index].intersectionType == RoadIntersection.IntersectionTypeEnum.ThreeWay &&
                         iFLane2s_tID[index].roadType == RoadIntersection.RoadTypeEnum.BothTurnLanes &&
                         !iFLane2s_tID[index].isNode2FRightTurnLane) {
                    tUV = iFLane2s_uv[index];
                    tTangents = iFLane2s_tangents[index];
                    xMesh = tMesh_iFLanes2[index];
                    MF = MeshSetup2IntersectionHelper(ref xMesh, ref tUV, ref tTangents, ref road.MeshiLanes2,
                        "LaneDAR2F", basePath + "/Materials/Markers/InterLaneDisabledOuterR.mat");
                    ...
                    xMesh = tMesh_iFLanes2[index];
                    MF = MeshSetup2IntersectionHelper(ref xMesh, ref tUV, ref tTangents, ref road.MeshiLanes2,
                        "LaneD2F", basePath + "/Materials/Markers/InterLaneDisabled.mat");
                    ...
                    tTangents = iFLane2s_tangents[index];
                    xMesh = tMesh_iFLanes2[index];
                    MF = MeshSetup2IntersectionHelper(ref xMesh, ref tUV, ref tTangents, ref road.MeshiLanes2, "Lane2F",
                        basePath + "/Materials/Markers/InterWhiteR.mat");
            ...
            vCount = iFLane3s.Count;
            for (var index = 0; index < vCount; index++) {
                var bPrimaryNode = iFLane3s_tID[index].node1 == iFLane3s_nID[index];
                if (!bPrimaryNode &&
                    iFLane3s_tID[index].intersectionType == RoadIntersection.IntersectionTypeEnum.ThreeWay &&
                    iFLane3s_tID[index].roadType == RoadIntersection.RoadTypeEnum.BothTurnLanes &&
                    !iFLane3s_tID[index].isNode2FRightTurnLane) {
                    tUV = iFLane3s_uv[index];
                    tTangents = iFLane3s_tangents[index];
                    xMesh = tMesh_iFLanes3[index];
                    MF = MeshSetup2IntersectionHelper(ref xMesh, ref tUV, ref tTangents, ref road.MeshiLanes3,
                        "LaneD3F", basePath + "/Materials/Markers/InterWhiteR.mat");
                    ...
                    tTangents = iFLane3s_tangents[index];
                    xMesh = tMesh_iFLanes3[index];
                    MF = MeshSetup2IntersectionHelper(ref xMesh, ref tUV, ref tTangents, ref road.MeshiLanes3, "Lane3F",
                        basePath + "/Materials/Markers/InterWhiteR.mat");
            ...
            for (var index = 0; index < vCount; index++) {
                ...
                MF = MeshSetup2IntersectionHelper(ref xMesh, ref tUV, ref tTangents, ref road.MeshiMainPlates,
                    "MainPlateB", basePath + "/Materials/Road1.mat");
                ...
                MF = MeshSetup2IntersectionHelper(ref fMesh, ref tUV, ref tTangents, ref road.MeshiMainPlates,
                    "MainPlateBM", basePath + "/Materials/InterMainPlate1.mat");
            ...
            for (var index = 0; index < vCount; index++) {
                ...
                MF = MeshSetup2IntersectionHelper(ref xMesh, ref tUV, ref tTangents, ref road.MeshiMainPlates,
                    "MainPlateFM", basePath + "/Materials/Road1.mat");
                ...
                MF = MeshSetup2IntersectionHelper(ref tMesh, ref tUV, ref tTangents, ref road.MeshiMainPlates,
                    "MainPlateFM", basePath + "/Materials/InterMainPlate1.mat");
            ...
            for (var index = 0; index < vCount; index++) {
                ...
                if (tNode.isIntersection && tNode.intersection != null && tNode.intersection.node1 == tNode) {
                    ...
                    var cCount = tNode.intersection.transform.childCount;
                    ...
                    for (var j = GOToDelete.Count - 1; j >= 0; j--) Object.DestroyImmediate(GOToDelete[j]);
                    ...
                    if (road.isLightmapped) {
                        ...
                        GameObjectUtility.SetStaticEditorFlags(tCenter, StaticEditorFlags.ContributeGI);
#else
                    ...
                    if (road.isStatic) tCenter.isStatic = true;
                    ...
                    if (road.isLightmapped) {
#if UNITY_2019_2_OR_NEWER
                        GameObjectUtility.SetStaticEditorFlags(tMarker, StaticEditorFlags.ContributeGI);
#else
                    ...
                    if (road.isStatic) tMarker.isStatic = true;

                    bVerts = MF.sharedMesh.vertices;
                    for (var j = 0; j < 4; j++) bVerts[j].y = tNode.intersection.signHeight;


                    var zCount = bVerts.Length;
                    for (var z = 0; z < zCount; z++) bVerts[z] -= tNode.transform.position;
                    ...
                    SaveMesh(SaveMeshTypeEnum.Intersection, MF.sharedMesh, road,
                        tNode.intersection.transform.name + "-" + "CenterMarkers");
                }
            }

            foreach (var KVP in tCombineDict_Lane0) {
                if (!uniqueRoadIntersection.Contains(KVP.Key)) uniqueRoadIntersection.Add(KVP.Key);
                MeshSetup2CombineIntersections(KVP, KVP.Key.transform.name + "-" + "Lane0");
            }

            foreach (var KVP in tCombineDict_Lane1) {
                if (!uniqueRoadIntersection.Contains(KVP.Key)) uniqueRoadIntersection.Add(KVP.Key);
                MeshSetup2CombineIntersections(KVP, KVP.Key.transform.name + "-" + "Lane1");
            }

            foreach (var KVP in tCombineDict_Lane2) {
                if (!uniqueRoadIntersection.Contains(KVP.Key)) uniqueRoadIntersection.Add(KVP.Key);
                MeshSetup2CombineIntersections(KVP, KVP.Key.transform.name + "-" + "Lane2");
            }

            foreach (var KVP in tCombineDict_Lane3) {
                if (!uniqueRoadIntersection.Contains(KVP.Key)) uniqueRoadIntersection.Add(KVP.Key);
                MeshSetup2CombineIntersections(KVP, KVP.Key.transform.name + "-" + "Lane3");
            }

            foreach (var KVP in tCombineDict_MainPlate) {
                if (!uniqueRoadIntersection.Contains(KVP.Key)) uniqueRoadIntersection.Add(KVP.Key);
                MeshSetup2CombineIntersections(KVP, KVP.Key.transform.name + "-" + "TiledExt", true);
            }

            foreach (var KVP in tCombineDict_MainPlateM) {
                if (!uniqueRoadIntersection.Contains(KVP.Key)) uniqueRoadIntersection.Add(KVP.Key);
                MeshSetup2CombineIntersections(KVP, KVP.Key.transform.name + "-" + "StretchExt");
            }

            foreach (var KVP in tCombineDict_Lane1_Disabled) {
                if (!uniqueRoadIntersection.Contains(KVP.Key)) uniqueRoadIntersection.Add(KVP.Key);
                MeshSetup2CombineIntersections(KVP, KVP.Key.transform.name + "-" + "LaneD1");
            }

            foreach (var KVP in tCombineDict_Lane3_Disabled) {
                if (!uniqueRoadIntersection.Contains(KVP.Key)) uniqueRoadIntersection.Add(KVP.Key);
                MeshSetup2CombineIntersections(KVP, KVP.Key.transform.name + "-" + "LaneD3");
            }

            foreach (var KVP in tCombineDict_Lane2_DisabledActive) {
                if (!uniqueRoadIntersection.Contains(KVP.Key)) uniqueRoadIntersection.Add(KVP.Key);
                MeshSetup2CombineIntersections(KVP, KVP.Key.transform.name + "-" + "LaneDA2");
            }

            foreach (var KVP in tCombineDict_Lane2_DisabledActiveR) {
                if (!uniqueRoadIntersection.Contains(KVP.Key)) uniqueRoadIntersection.Add(KVP.Key);
                MeshSetup2CombineIntersections(KVP, KVP.Key.transform.name + "-" + "LaneDAR2");
            }

            foreach (var KVP in tCombineDict_Lane2_Disabled) {
                if (!uniqueRoadIntersection.Contains(KVP.Key)) uniqueRoadIntersection.Add(KVP.Key);
                MeshSetup2CombineIntersections(KVP, KVP.Key.transform.name + "-" + "LaneD2");
            }

            foreach (var KVP in tCombineDict_Lane1_DisabledActive) {
                if (!uniqueRoadIntersection.Contains(KVP.Key)) uniqueRoadIntersection.Add(KVP.Key);
                MeshSetup2CombineIntersections(KVP, KVP.Key.transform.name + "-" + "LaneDA1");
            }

            foreach (var roadIntersection in uniqueRoadIntersection) roadIntersection.UpdateMaterials();
        }


        /// <summary> Combines all intersections in _valuePair into an single mesh </summary>
        private void MeshSetup2CombineIntersections(KeyValuePair<RoadIntersection, List<MeshFilter>> _valuePair,
            string _name, bool _isMainPlates = false) {
            var vCount = _valuePair.Value.Count;
            if (vCount < 1) return;
            _name = road.name + "-" + _name;
            GameObject tCenter = null;
            var cCount = _valuePair.Key.transform.childCount;
            var GOToDelete = new List<GameObject>();
            for (var index = 0; index < cCount; index++) {
            ...
            for (var index = GOToDelete.Count - 1; index >= 0; index--) Object.DestroyImmediate(GOToDelete[index]);

            var CombineCount = vCount;
            ...
            var SpecialVertCount = 0;
            if (tCenter != null) {
                for (var index = 0; index < CombineCount - 1; index++)
                    SpecialVertCount += combine[index].mesh.vertexCount;
                var tMF = tCenter.GetComponent<MeshFilter>();
                var xVerts = tMF.sharedMesh.vertices;
                var xHeight = combine[0].mesh.vertices[combine[0].mesh.vertexCount - 1].y;
                for (var index = 0; index < xVerts.Length; index++) xVerts[index].y = xHeight;
            ...
            for (var index = 0; index < tVerts.Length; index++) {
                tVerts[index] += tVect;
                if (_name.ToLower().EndsWith("-stretchext")) tVerts[index] += new Vector3(0f, 0.01f);
            }
            ...
            MF.sharedMesh.tangents = RootUtils.ProcessTangents(MF.sharedMesh.triangles, MF.sharedMesh.normals,
                MF.sharedMesh.uv, MF.sharedMesh.vertices);
...
#if UNITY_2019_2_OR_NEWER
                GameObjectUtility.SetStaticEditorFlags(tObj, StaticEditorFlags.ContributeGI);
#else
            ...
            if (road.isStatic) tObj.isStatic = true;
            ...
            if (_isMainPlates && tCenter != null) Object.DestroyImmediate(tCenter);
        ...
        private MeshFilter MeshSetup2IntersectionHelper(ref Mesh _mesh, ref Vector2[] _uv, ref Vector4[] _tangents,
            ref GameObject _masterObj, string _name, string _mat, bool _isCollider = false) {
            if (_mesh == null) return null;
            _mesh.uv = _uv;
            _mesh.tangents = _tangents;

#if UNITY_EDITOR
            if (road.isLightmapped) Unwrapping.GenerateSecondaryUVSet(_mesh);
#endif
        ...
        private Mesh MeshSetup2Helper(ref Mesh _mesh, Vector2[] _uv, Vector4[] _tangents, ref GameObject _obj,
            Material[] _shoulderMaterials, Material[] _markerMaterials, Material[] _roadMaterials, bool _isMarker,
            bool _isShoulder = false, bool _isBridge = false) {
            _mesh.uv = _uv;
            _mesh.tangents = _tangents;
            ...
            var MF = _obj.AddComponent<MeshFilter>();
            MF.sharedMesh = _mesh;
            ...
            var MR = _obj.AddComponent<MeshRenderer>();

            if (_isShoulder) {
                if (_shoulderMaterials.Length > 0) MR.materials = _shoulderMaterials;
            }
            else {
                if (_isMarker) {
                    // It should not be necessary to check this, but I let it until I'm sure this does not break anything.
                    if (_markerMaterials.Length > 0) MR.materials = _markerMaterials;
                }
                else {
                    if (_roadMaterials.Length > 0) MR.materials = _roadMaterials;

                    if (MC) MC.sharedMaterial = road.RoadPhysicMaterial;
                }
            }
            ...
            if (road.isLightmapped) {
#if UNITY_2019_2_OR_NEWER
                GameObjectUtility.SetStaticEditorFlags(_obj, StaticEditorFlags.ContributeGI);
#else
            ...
            if (road.isStatic) _obj.isStatic = true;
        ...
        private bool MeshSetup2HelperRoadCuts(int _i, ref Mesh _mesh, Vector2[] _uv, Vector4[] _tangents,
            ref GameObject _masterObj, bool _isMarkers, out GameObject _createdObj, Material[] _markerMaterials,
            Material[] _roadMaterials) {
            if (!_isMarkers) {
                _createdObj = new GameObject("RoadCut" + _i);
                RoadCutNodes[_i].roadCutWorld = _createdObj;
            }
            else {
                _createdObj = new GameObject("Markers" + _i);
            ...
            var MF = _createdObj.AddComponent<MeshFilter>();
            MF.sharedMesh = _mesh;
            ...
            var MR = _createdObj.AddComponent<MeshRenderer>();

            //Disable shadows for road cuts and markers:
            MR.shadowCastingMode = ShadowCastingMode.Off;
            var isUsingMaterials = false;
            ...
            if (!_isMarkers && MC != null) MC.sharedMaterial = road.RoadPhysicMaterial;

#if UNITY_EDITOR
            if (road.isLightmapped) {
#if UNITY_2019_2_OR_NEWER
                GameObjectUtility.SetStaticEditorFlags(_createdObj, StaticEditorFlags.ContributeGI);
#else
            ...
            if (road.isStatic) _createdObj.isStatic = true;
        ...
        private bool MeshSetup2HelperCutsShoulder(int _i, ref Mesh _mesh, Vector2[] _uv, Vector4[] _tangents,
            ref GameObject _masterObj, bool _isLeft, bool _isMarkers, out GameObject _createdObj,
            Material[] _shoulderMarkerMaterials, Material[] _shoulderMaterials) {
            if (_isMarkers) {
                _createdObj = new GameObject("Markers" + _i);
                ...
                if (_isLeft) {
                    _createdObj = new GameObject("SCutL" + _i);
                    _createdObj.transform.position = cut_ShoulderL_VectorsHome[_i];
                    ShoulderCutsLNodes[_i].shoulderCutLWorld = _createdObj;
                }
                else {
                    _createdObj = new GameObject("SCutR" + _i);
                    _createdObj.transform.position = cut_ShoulderR_VectorsHome[_i];
            ...
            MeshCollider MC = null;
            if (road.isUsingMeshColliders) MC = _createdObj.AddComponent<MeshCollider>();
            ...
            if (!_isMarkers && MC != null) MC.sharedMaterial = road.ShoulderPhysicMaterial;
            _createdObj.transform.parent = _masterObj.transform;
