...
namespace RoadArchitect {
    public class SplineI : MonoBehaviour {
        ...
        private void OnDrawGizmos() {
            ...
            var DistanceFromCam = Vector3.Distance(Camera.current.transform.position, nodes[0].pos);

            if (DistanceFromCam > 2048)
                return;
            if (DistanceFromCam > 1024)
                gizmoDrawMeters = 32f;
            ...
            var startI = 0f;
            var endI = 1f;
            if (actionNode.idOnSpline > 3) startI = nodes[actionNode.idOnSpline - 2].time;
            if (actionNode.idOnSpline < nodes.Count - 3) endI = nodes[actionNode.idOnSpline + 2].time;
        ...
        public void DetermineInsertNodes() {
            var nodesCount = spline.nodes.Count;
            if (nodesCount < 2) return;
            SplinePreviewNode previewNode;
            SplineN xNode;
            nodes.Clear();
            var tParam = spline.GetClosestParam(mousePos, false, true);
            ...
            var cCount = nodes.Count;
            if (bEndInsert)
                iStart = cCount;
            else
                for (var index = iStart; index < cCount; index++)
        ...
        public void UpdateActionNode() {
            if (actionNode != null) actionNode.pos = mousePos;
        ...
        private void SetupSplineLength() {
            ...
            var mDistance = 0f;
            var mDistance_NoMod = 0f;
            for (var j = 0; j < nodes.Count; j++) {
                tVect2 = nodes[j].pos;
                if (j > 0) mDistance += Vector3.Distance(tVect1, tVect2);
                tVect1 = tVect2;
            }
        ...
        [FormerlySerializedAs("mNodes")] public List<SplinePreviewNode> nodes = new List<SplinePreviewNode>();

        [FormerlySerializedAs("bClosed")] public bool isClosed;

        public float distance = -1f;

        [FormerlySerializedAs("MousePos")] public Vector3 mousePos = new Vector3(0f, 0f, 0f);

        [FormerlySerializedAs("GSDSpline")] public SplineC spline;

        [FormerlySerializedAs("ActionNode")] public SplinePreviewNode actionNode;

        // Gizmos
        [FormerlySerializedAs("bGizmoDraw")] public bool isDrawingGizmos;

        [FormerlySerializedAs("GizmoDrawMeters")]
