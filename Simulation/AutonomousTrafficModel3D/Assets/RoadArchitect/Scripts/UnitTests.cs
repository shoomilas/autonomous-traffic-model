...
namespace RoadArchitect.Tests {
    ...
    public static class UnitTests {
        ...
        /// <summary>
        ///     WARNING: You should only call this on an empty scene. The RoadArchitect team is not responsbile for loss of
        ///     data or work if this function is called by user.
        /// </summary>
        public static void RoadArchitectUnitTests() {
            CleanupAllTests();

            //Create new road system and turn off updates:
            var roadSystemObject = new GameObject("RoadArchitectSystem1");
            ...
            for (var index = 1; index <= numTests; index++) {
                Debug.Log("Running test " + index);
                double testTime = RunTest(index);
                totalTestTime += testTime;
                Debug.Log("Test " + index + " complete.  Test time: " + testTime + "ms");
            }
        ...
        public static void CleanupTest(int _index) {
            ...
            var roadSystem = GameObject.Find("RoadArchitectSystem" + _index);
        ...
        private static void DestroyTerrainHistory(GameObject _roadSystem) {
            ...
            if (_roadSystem != null) {
                var roads = _roadSystem.GetComponents<Road>();
                foreach (var road in roads) Terraforming.TerrainsReset(road);
            }
        }
        ...
        private static void FlattenTerrains() {
            var allTerrains = Object.FindObjectsOfType<Terrain>();
            foreach (var terrain in allTerrains) terrain.terrainData.SetHeights(0, 0, new float[513, 513]);
        }
        ...
        private static void RoadArchitectUnitTest1() {
            ...
            var maxCount = 18;
            var mod = 100f;
            var vector = new Vector3(50f, 40f, 50f);
            for (var index = 0; index < maxCount; index++)
                //tLocs.Add(xVect + new Vector3(tMod * Mathf.Pow((float)i / ((float)MaxCount * 0.15f), 2f), 1f*((float)i*1.25f), tMod * i));
                nodeLocations.Add(vector + new Vector3(mod * Mathf.Pow(index / (25 * 0.15f), 2f), 0f, mod * index));

            //Get road system create road:
            var testRoad = RoadAutomation.CreateRoadProgrammatically(roadSystem, ref nodeLocations);
        ...
        private static void RoadArchitectUnitTest2() {
            ...
            RoadAutomation.CreateIntersectionsProgrammaticallyForRoad(road1);

            //Now count road intersections, if not 5 throw error
            var intersctionsCount = 0;
            foreach (var node in road1.spline.nodes)
                if (node.isIntersection)
                    intersctionsCount += 1;

            if (intersctionsCount != 5)
                Debug.LogError("Unit Test #2 failed: " + intersctionsCount + " intersections instead of 5.");
        }
        ...
        private static void UnitTestIntersectionHelper(Road _road1, Road _road2,
            RoadIntersection.iStopTypeEnum _iStopType, RoadIntersection.RoadTypeEnum _roadType) {
            ...
            foreach (var node in _road1.spline.nodes)
            foreach (var node2 in _road2.spline.nodes)
                if (RootUtils.IsApproximately(Vector3.Distance(node.transform.position, node2.transform.position), 0f,
                    0.05f)) {
                    nodeInter1 = node;
                    nodeInter2 = node2;
                    break;
            ...
            if (nodeInter1 != null && nodeInter2 != null) {
                var IntersectionsObject = Intersections.CreateIntersection(nodeInter1, nodeInter2);
                var roadIntersction = IntersectionsObject.GetComponent<RoadIntersection>();
        ...
        private static void RoadArchitectUnitTest3() {
            ...
            road2 = RoadAutomation.CreateRoadProgrammatically(roadSystem, ref nodeLocations);
            road2.laneAmount = 4;
            UnitTestIntersectionHelper(road1, road2, RoadIntersection.iStopTypeEnum.TrafficLight1,
            ...
            if (intersectionsCount != 5)
                Debug.LogError("Unit Test #3 failed: " + intersectionsCount + " intersections instead of 5.");
        }
        ...
        private static void RoadArchitectUnitTest4() {
            //Create base road:
            var nodeLocations = new List<Vector3>();
            for (var index = 0; index < 5; index++) nodeLocations.Add(new Vector3(3500f, 90f, 200f + 800f * index));
        ...
        private static void RoadArchitectUnitTest5() {
            //Create base road:
            var nodeLocations = new List<Vector3>();
            for (var index = 0; index < 48; index++) nodeLocations.Add(new Vector3(3000f, 40f, 10f + 79f * index));
            for (var index = 0; index < 35; index++) nodeLocations.Add(new Vector3(2900f - 79f * index, 30f, 3960f));
            for (var index = 0; index < 40; index++) nodeLocations.Add(new Vector3(30, 30f, 3960f - 79f * index));
        ...
        public static void RoadArchitectUnitTest6() {
            ...
            var unitSystem6Object = new GameObject("RoadArchitectSystem6");
            var unitSystem6 = unitSystem6Object.AddComponent<RoadSystem>();
            unitSystem6.isAllowingRoadUpdates = false;

            //Create base road:
            var nodeLocations = new List<Vector3>();
            for (var index = 0; index < 50; index++) nodeLocations.Add(new Vector3(6000f, 40f, 10f + 79f * index));
            for (var index = 0; index < 50; index++) nodeLocations.Add(new Vector3(5900f - 79f * index, 30f, 5950f));
            for (var index = 0; index < 50; index++) nodeLocations.Add(new Vector3(-200f, 30f, 5950f - 79f * index));
            for (var index = 0; index < 50; index++) nodeLocations.Add(new Vector3(-200f + 79f * index, 30f, 10f));
        ...
        public static void RoadArchitectUnitTest7() {
            ...
            var unitSystem7Object = new GameObject("RoadArchitectSystem7");
            var unitSystem7 = unitSystem7Object.AddComponent<RoadSystem>();
            ...
            road2.laneAmount = 2;
            UnitTestIntersectionHelper(road1, road2, RoadIntersection.iStopTypeEnum.None,
                RoadIntersection.RoadTypeEnum.NoTurnLane);
            ...
            road3.laneAmount = 2;
            UnitTestIntersectionHelper(road1, road3, RoadIntersection.iStopTypeEnum.None,
                RoadIntersection.RoadTypeEnum.NoTurnLane);
        ...
        public static void RoadArchitectUnitTest8() {
            ...
            var unitSystem8Object = new GameObject("RoadArchitectSystem8");
            var unitSystem8 = unitSystem8Object.AddComponent<RoadSystem>();
        ...
        public static void RoadArchitectUnitTest9() {
            ...
            var unitSystem9Object = new GameObject("RoadArchitectSystem9");
            var unitSystem9 = unitSystem9Object.AddComponent<RoadSystem>();
            ...
            road4.laneAmount = 2;
            UnitTestIntersectionHelper(road1, road4, RoadIntersection.iStopTypeEnum.None,
                RoadIntersection.RoadTypeEnum.NoTurnLane);
