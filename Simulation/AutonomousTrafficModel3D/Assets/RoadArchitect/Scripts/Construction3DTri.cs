...
namespace RoadArchitect {
    ...
    public class Construction3DTri {
        ...
        public Vector3 LinePlaneIntersection(ref Vector3 _F1) {
            ...
            var dotNumerator = Vector3.Dot(pMiddle - _F1, normal);
            var dotDenominator = Vector3.Dot(Vector3.up.normalized, normal);
        ...
        /// </summary>
        public static void PlaneFrom3Points(out Vector3 _planeNormal, out Vector3 _planePoint, Vector3 _pointA,
            Vector3 _pointB, Vector3 _pointC) {
            _planeNormal = Vector3.zero;
            _planePoint = Vector3.zero;

            //Make two vectors from the 3 input points, originating from point A
            var AB = _pointB - _pointA;
            var AC = _pointC - _pointA;

            //Calculate the normal
            _planeNormal = Vector3.Normalize(Vector3.Cross(AB, AC));

            //Get the points in the middle AB and AC
            var middleAB = _pointA + AB / 2.0f;
            var middleAC = _pointA + AC / 2.0f;

            //Get vectors from the middle of AB and AC to the point which is not on that line.
            var middleABtoC = _pointC - middleAB;
            var middleACtoB = _pointB - middleAC;
        ...
        /// </summary>
        public static bool ClosestPointsOnTwoLines(out Vector3 _closestPointLine1, out Vector3 _closestPointLine2,
        ...
        public static Vector3 SetVectorLength(Vector3 _vector, float _size) {
            ...
            var vectorNormalized = Vector3.Normalize(_vector);
        ...
        public bool Contains2D(ref Vector2 _p) {
            if (Vector2.SqrMagnitude(_p - poly2D[0]) > MaxDistanceSq) return false;
            ...
            for (var index = 0; index < 3; index++) {
                ...
                if (newPoint.x < _p.x == _p.x <= oldPoint.x &&
                    (_p.y - x1.y) * (x2.x - x1.x) < (x2.y - x1.y) * (_p.x - x1.x)) inside = !inside;
                oldPoint = newPoint;
            }
        ...
        public bool Contains2D(ref Vector3 _p) {
            var tVect = new Vector2(_p.x, _p.z);
