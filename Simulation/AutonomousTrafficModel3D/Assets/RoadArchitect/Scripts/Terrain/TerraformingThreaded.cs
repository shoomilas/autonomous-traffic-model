...
namespace RoadArchitect.Threading {
    public static class TerraformingThreaded {
        ...
        */
        ...
        private static void GetTempHeightsCoordinates(ref Vector3 _vector, ref Terraforming.TempTerrainData _TTD,
            out int _x, out int _y) {
            //Get the normalized position of this game object relative to the terrain:
            var tempCoord = _vector - _TTD.TerrainPos;
        ...
        /// <summary> Writes heightmap location of _vector into _x and _y </summary>
        private static void GetTempDetailsCoordinates(ref Vector3 _vector, ref Terraforming.TempTerrainData _TTD,
            out int _x, out int _y) {
            //Get the normalized position of this game object relative to the terrain:
            var tempCoord = _vector - _TTD.TerrainPos;
        ...
        private static void DoRectsDo(ref SplineC _spline, ref Terraforming.TempTerrainData _TTD) {
            var Sep = _spline.road.RoadWidth() * 0.5f;
            var HeightSep = Sep + _spline.road.matchHeightsDistance * 0.5f;
            var TBMList = new List<TerrainBoundsMaker>();
            //List<RoadUtility.Construction3DTri> triList = new List<RoadUtility.Construction3DTri>();
            var TreerectList = new List<Construction2DRect>();
            var tStep = _spline.road.roadDefinition / _spline.distance;
            //tStep *= 0.5f;
            ...
            var fValueMod = _spline.road.roadDefinition / _spline.distance;
            var bIsPastInter = false;
            var tIntStrength = 0f;
            var tIntStrength2 = 0f;
            ...
            var T1SUB = 0f;
            var T2SUB = 0f;
            var bIntStr1_Full = false;
            var bIntStr1_FullPrev = false;
            var bIntStr1_FullNext = false;
            var bIntStr2_Full = false;
            var bIntStr2_FullPrev = false;
            var bIntStr2_FullNext = false;
            var tVect3 = default(Vector3);
            var tXYs = new List<int[]>();
            var TreeClearDist = _spline.road.clearTreesDistance;
            if (TreeClearDist < _spline.road.RoadWidth()) TreeClearDist = _spline.road.RoadWidth();
            Construction2DRect tRect = null;
            var tGrade = 0f;
            for (var index = StartMin; index < FinalMax; index += tStep)
                if (_spline.road.isHeightModificationEnabled) {
                    ...
                    tIntStrength = _spline.IntersectionStrength(ref tVect1, ref tIntHeight, ref roadIntersection,
                        ref bIsPastInter, ref index, ref xNode);
                    ...
                    tIntStrength2 = _spline.IntersectionStrength(ref tVect2, ref tIntHeight2, ref roadIntersection,
                        ref bIsPastInter, ref index, ref xNode);
                    ...
                    if (tIntStrength >= 0f) {
                        ...
                        if (RootUtils.IsApproximately(tIntStrength, 1f)) {
                            T1SUB = tIntHeight;
                            bIntStr1_Full = true;
                            ...
                            if (!RootUtils.IsApproximately(tIntStrength, 0f))
                    ...
                    if (tIntStrength2 >= 0f || RootUtils.IsApproximately(tIntStrength2, 0f)) {
                        ...
                        if (RootUtils.IsApproximately(tIntStrength, 1f)) {
                            ...
                            bIntStr2_Full = false;
                            if (!RootUtils.IsApproximately(tIntStrength2, 0f))
                        ...
                        if (bIntStr2_Full && !bIntStr2_FullPrev) {
                            ...
                            if (tNext + tStep < 1f) {
                                tVect3 = _spline.GetSplineValue(tNext + tStep);
                                tIntStrength2 = _spline.IntersectionStrength(ref tVect3, ref tIntHeight2,
                                    ref roadIntersection, ref bIsPastInter, ref index, ref xNode);
                            }
                            else {
                                tIntStrength2 = 0f;
                            }
                    ...
                    tXYs.Add(CreateTris(fValue1, fValue2, ref tVect1, ref POS1, ref tVect2, ref POS2, Sep, ref TBMList,
                        ref T1SUB, ref T2SUB, ref _TTD, HeightSep));

                    //Details and trees:
                    tRect = SetDetailCoords(index, ref tVect1, ref POS1, ref tVect2, ref POS2,
                        _spline.road.clearDetailsDistance, TreeClearDist, ref _TTD, ref _spline);
                    if (_spline.road.isTreeModificationEnabled && tRect != null) TreerectList.Add(tRect);
                }
                else {
                    if (index > 1f) break;
                    tNext = index + tStep;
            ...
            if (_spline.road.isTreeModificationEnabled && TreerectList != null && TreerectList.Count > 0) {
                var tCount = _TTD.TreeSize;
                var jCount = TreerectList.Count;
            ...
            if (!_spline.road.isHeightModificationEnabled) return;
            ...
            var k = 0;
            int[] tXY = null;
            var tXYsCount = tXYs.Count;
            var isBridge = false;
            var isTunnel = false;
            for (var index = StartMin; index < FinalMax; index += tStep) {
                if (TBMList.Count > 0) {
                    if (TBMList[0].MaxI < index) CleanupTris(index, ref TBMList);
                }
                else {
                    break;
                }
                ...
                if (k < tXYsCount) {
                    tXY = tXYs[k];
                    tFloat = ProcessCoordinateGrabber(ref index, ref _spline, ref _TTD, ref TBMList, ref tXY, isBridge,
                        isTunnel);
        ...
        private static void CleanupTris(float _currentI, ref List<TerrainBoundsMaker> _terrainBoundsList) {
            var mCount = _terrainBoundsList.Count;
            ...
            for (var index = 0; index < mCount; index++)
                if (_terrainBoundsList[index].MaxI < _currentI)
                    ...
                    break;
            if (LastIndexToRemove >= 0) _terrainBoundsList.RemoveRange(0, LastIndexToRemove);
            //			
        ...
        private static int[] CreateTris(float _i, float _i2, ref Vector3 _vect1, ref Vector3 _POS1, ref Vector3 _vect2,
            ref Vector3 _POS2, float _sep, ref List<TerrainBoundsMaker> _list, ref float _T1SUB, ref float _T2SUB,
            ref Terraforming.TempTerrainData _TTD, float _heightSep) {
            var lVect1 = _vect1 + new Vector3(_sep * -_POS1.normalized.z, 0, _sep * _POS1.normalized.x);
            var rVect1 = _vect1 + new Vector3(_sep * _POS1.normalized.z, 0, _sep * -_POS1.normalized.x);
            var lVect2 = _vect2 + new Vector3(_sep * -_POS2.normalized.z, 0, _sep * _POS2.normalized.x);
            var rVect2 = _vect2 + new Vector3(_sep * _POS2.normalized.z, 0, _sep * -_POS2.normalized.x);
            ...
            var lVect1far = _vect1 + new Vector3(_heightSep * -_POS1.normalized.z, 0, _heightSep * _POS1.normalized.x);
            var rVect1far = _vect1 + new Vector3(_heightSep * _POS1.normalized.z, 0, _heightSep * -_POS1.normalized.x);
            var lVect2far = _vect2 + new Vector3(_heightSep * -_POS2.normalized.z, 0, _heightSep * _POS2.normalized.x);
            var rVect2far = _vect2 + new Vector3(_heightSep * _POS2.normalized.z, 0, _heightSep * -_POS2.normalized.x);
            ...
            TBM.constructRect = new Construction2DRect(new Vector2(lVect1far.x, lVect1far.z),
                new Vector2(rVect1far.x, rVect1far.z), new Vector2(rVect2far.x, rVect2far.z),
                new Vector2(lVect2far.x, lVect2far.z));
            //tRect.MinI = i;
            //tRect.MaxI = i2;
            ...
            var Min = Mathf.Min(Xs);
            var Max = Mathf.Max(Xs);
            Xs[0] = Min - 2;
            Xs[2] = Max + 2;
            Min = Mathf.Min(Ys);
            Max = Mathf.Max(Ys);
        ...
        private static Construction2DRect SetDetailCoords(float _param, ref Vector3 _vect1, ref Vector3 _POS1,
            ref Vector3 _vect2, ref Vector3 POS2, float _sep, float _treeSep, ref Terraforming.TempTerrainData _TTD,
            ref SplineC _spline) {
            ...
            var isInBridge = _spline.IsInBridgeTerrain(_param);
            var isInTunnel = _spline.IsInTunnelTerrain(_param);
            int x2, y2, x3, y3;
            ...
            var tDiff1 = _TTD.heights[x2, y2] * _TTD.TerrainSize.y - _vect1.y;
            ...
            if (x3 == -1) return null;
            if (y3 == -1) return null;
            var tDiff2 = _TTD.heights[x3, y3] * _TTD.TerrainSize.y - _vect2.y;
            ...
            if (_spline.road.isTreeModificationEnabled) {
                ...
                if (isInBridge && !isQuit) {
                    ...
                    if (tDiff1 > _spline.road.clearTreesDistanceHeight) isQuit = true;
                    if (tDiff2 > _spline.road.clearTreesDistanceHeight) isQuit = true;
                }

                if (isInTunnel && !isQuit) {
                    if (tDiff1 < 0f) {
                        if (tDiff1 * -1f > _spline.road.clearTreesDistanceHeight) isQuit = true;
                    }
                    else {
                        if (tDiff1 > _spline.road.clearTreesDistanceHeight * 0.1f) isQuit = true;
                    }

                    if (tDiff2 < 0f) {
                        if (tDiff2 * -1f > _spline.road.clearTreesDistanceHeight) isQuit = true;
                    }
                    else {
                        if (tDiff2 > _spline.road.clearTreesDistanceHeight * 0.1f) isQuit = true;
                    }
                }
                ...
                if (!isQuit) {
                    ...
                    tRect = new Construction2DRect(new Vector2(lVect1far.x, lVect1far.z),
                        new Vector2(rVect1far.x, rVect1far.z), new Vector2(rVect2far.x, rVect2far.z),
                        new Vector2(lVect2far.x, lVect2far.z));
                }
            }
            ...
            if (_spline.road.isDetailModificationEnabled) {
                if (isInBridge || isInTunnel) {
                    ...
                    if (tDiff1 > _spline.road.clearDetailsDistanceHeight) isQuit = true;
                    if (tDiff2 > _spline.road.clearDetailsDistanceHeight) isQuit = true;

                    if (isQuit) return tRect;
                }
                ...
                if (MinX >= _TTD.DetailMaxIndex) MinX = _TTD.DetailMaxIndex - 1;
                if (MinY >= _TTD.DetailMaxIndex) MinY = _TTD.DetailMaxIndex - 1;
                if (MaxX >= _TTD.DetailMaxIndex) MaxX = _TTD.DetailMaxIndex - 1;
                if (MaxY >= _TTD.DetailMaxIndex) MaxY = _TTD.DetailMaxIndex - 1;
        ...
        private static float ProcessCoordinateGrabber(ref float _param, ref SplineC _spline,
            ref Terraforming.TempTerrainData _TTD, ref List<TerrainBoundsMaker> _terrainList, ref int[] _XY,
            bool _isBridge, bool _isTunnel) {
            var MinX = _XY[0];
            var MinY = _XY[1];
            var MaxX = _XY[2];
            var MaxY = _XY[3];

            if (MinX >= _TTD.TerrainMaxIndex) MinX = _TTD.TerrainMaxIndex - 1;
            if (MinY >= _TTD.TerrainMaxIndex) MinY = _TTD.TerrainMaxIndex - 1;
            if (MaxX >= _TTD.TerrainMaxIndex) MaxX = _TTD.TerrainMaxIndex - 1;
            if (MaxY >= _TTD.TerrainMaxIndex) MaxY = _TTD.TerrainMaxIndex - 1;
            ...
            for (var k = MinY; k <= MaxY; k++)
                if (_TTD.tHeights[index, k] != true) {
                    if (_TTD.cX.Length <= _TTD.Count) break;

                    xVect = ConvertTerrainCoordToWorldVect(index, k, _TTD.heights[index, k], ref _TTD);
                    AdjustedTerrainVectTri(ref _param, out isAdjusted, out tHeight, ref xVect, ref _terrainList,
                        _isBridge, _isTunnel);
            ...
            if (_isBridge && RootUtils.IsApproximately(tReturnFloat, 0f, 0.0001f))
                tReturnFloat = _spline.GetSplineValue(_param).y;

            return tReturnFloat;
        }


        private static Vector3 ConvertTerrainCoordToWorldVect(int _x, int _y, float _height,
            ref Terraforming.TempTerrainData _TTD) {
            //Get the normalized position of this game object relative to the terrain:
            var x1 = _x / (_TTD.HM - 1f);
            x1 = x1 * _TTD.TerrainSize.x;

            var z1 = _y / (_TTD.HM - 1f);
            z1 = z1 * _TTD.TerrainSize.z;

            var y1 = _height * _TTD.TerrainSize.y;
        ...
        private static void AdjustedTerrainVectTri(ref float _param, out bool _isAdjusted, out float _height,
            ref Vector3 _vect, ref List<TerrainBoundsMaker> _terrainList, bool _isBridge, bool _isTunnel) {
            var OrigHeight = _vect.y;
            var mCount = _terrainList.Count;
            var tCount = 0;
            Construction3DTri tTri;
            TerrainBoundsMaker TBM;
            _isAdjusted = false;
            _height = 0f;
            var t2D = new Vector2(_vect.x, _vect.z);
            for (var index = 0; index < mCount; index++) {
                TBM = _terrainList[index];
                if (_param < TBM.MinI) return;
                if (_param > TBM.MaxI) continue;
        ...
        public class TerrainBoundsMaker {
            [FormerlySerializedAs("tRect")] public Construction2DRect constructRect;
