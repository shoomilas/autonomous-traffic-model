...
namespace RoadArchitect {
    ...
    public class RoadIntersectionEditor : Editor {
        ...
        private static readonly string[] rTypeDescriptions =
        {
            "No turn lanes",
            "Left turn lane only",
        ...
        private static readonly string[] rTypeDescriptions_3Way =
        {
            "No turn lanes",
            "Left turn lane only"
        };
        ...
        private static readonly string[] iStopTypeEnumDescriptions =
        {
            "Stop signs",
            "Traffic lights",
            "None"
//		"Traffic lights #2"
        ...
        private static readonly string[] iTrafficLightSequenceTypeDesc =
        {
            "Fixed time",
            "Other"
        };
        ...
        public void OnSceneGUI() {
            ...
            if (GUI.changed) EditorUtility.SetDirty(intersection);
        }
        ...
        public override void OnInspectorGUI() {
            ...
            {
                var road1 = intersection.node1.spline.transform.parent.GetComponent<Road>();
                var road2 = intersection.node2.spline.transform.parent.GetComponent<Road>();
            ...
            if (GUILayout.Button("Update intersection", loadButton)) TriggerRoadUpdate(true);

            //Option: Auto update:
            isAutoUpdatingIntersection.boolValue =
                EditorGUILayout.Toggle("Auto-update:", intersection.isAutoUpdatingIntersection);
            ...
            if (intersection.intersectionType == RoadIntersection.IntersectionTypeEnum.ThreeWay)
                roadType.enumValueIndex = EditorGUILayout.Popup((int)intersection.roadType, rTypeDescriptions_3Way);
            else
                roadType.enumValueIndex = EditorGUILayout.Popup((int)intersection.roadType, rTypeDescriptions);

            //Option: Left yield on green:
            if (intersection.roadType != RoadIntersection.RoadTypeEnum.NoTurnLane) {
                isLeftTurnYieldOnGreen.boolValue =
                    EditorGUILayout.Toggle("Left yield on green: ", intersection.isLeftTurnYieldOnGreen);
            ...
            intersectionStopType.enumValueIndex = EditorGUILayout.Popup("Intersection stop type:",
                (int)intersection.intersectionStopType, iStopTypeEnumDescriptions);


            if (intersection.intersectionStopType == RoadIntersection.iStopTypeEnum.TrafficLight1 ||
                intersection.intersectionStopType == RoadIntersection.iStopTypeEnum.TrafficLight2) {
                //Option: Traffic light timing type:
                lightType.enumValueIndex = EditorGUILayout.Popup("Traffic light timing:", (int)intersection.lightType,
                    iTrafficLightSequenceTypeDesc);
                ...
                if (intersection.lightType == RoadIntersection.LightTypeEnum.Timed) {
                    ...
                    EditorGUILayout.BeginHorizontal();
                    fixedTimeRegularLightLength.floatValue = EditorGUILayout.Slider("Green length: ",
                        intersection.fixedTimeRegularLightLength, 0.1f, 180f);
                    ...
                    if (intersection.roadType != RoadIntersection.RoadTypeEnum.NoTurnLane) {
                        EditorGUILayout.BeginHorizontal();
                        fixedTimeLeftTurnLightLength.floatValue = EditorGUILayout.Slider("Left turn only length: ",
                            intersection.fixedTimeLeftTurnLightLength, 0.1f, 180f);
                        if (GUILayout.Button(refreshButtonTexture, imageButton, GUILayout.Width(16f)))
                    ...
                    EditorGUILayout.BeginHorizontal();
                    fixedTimeAllRedLightLength.floatValue = EditorGUILayout.Slider("All red length: ",
                        intersection.fixedTimeAllRedLightLength, 0.1f, 180f);
                    if (GUILayout.Button(refreshButtonTexture, imageButton, GUILayout.Width(16f)))
                        fixedTimeAllRedLightLength.floatValue = 1f;
                    EditorGUILayout.EndHorizontal();

                    EditorGUILayout.BeginHorizontal();
                    fixedTimeYellowLightLength.floatValue = EditorGUILayout.Slider("Yellow light length: ",
                        intersection.fixedTimeYellowLightLength, 0.1f, 180f);
                    if (GUILayout.Button(refreshButtonTexture, imageButton, GUILayout.Width(16f)))
            ...
            if (intersection.intersectionStopType == RoadIntersection.iStopTypeEnum.TrafficLight1 ||
                ...
                isTrafficPoleStreetLight.boolValue =
                    EditorGUILayout.Toggle("Street lights: ", intersection.isTrafficPoleStreetLight);
                ...
                isRegularPoleAlignment.boolValue =
                    EditorGUILayout.Toggle("Normal pole alignment: ", intersection.isRegularPoleAlignment);
                ...
                EditorGUILayout.EndHorizontal();
                isLightsEnabled.boolValue =
                    EditorGUILayout.Toggle("  Point lights enabled: ", intersection.isLightsEnabled);

                //Options: Street point light options:
                if (intersection.isTrafficPoleStreetLight) {
                    //Option: Street light range:
                    EditorGUILayout.BeginHorizontal();
                    streetLightRange.floatValue = EditorGUILayout.Slider("  Street light range: ",
                        intersection.streetLightRange, 1f, 128f);
                    if (GUILayout.Button(refreshButtonTexture, imageButton, GUILayout.Width(16f)))
                    ...
                    EditorGUILayout.BeginHorizontal();
                    streetLightIntensity.floatValue = EditorGUILayout.Slider("  Street light intensity: ",
                        intersection.streetLightIntensity, 0f, 8f);
                    if (GUILayout.Button(refreshButtonTexture, imageButton, GUILayout.Width(16f)))
                    ...
                    EditorGUILayout.BeginHorizontal();
                    streetLightColor.colorValue =
                        EditorGUILayout.ColorField("  Street light color: ", intersection.streetLightColor);
                    if (GUILayout.Button(refreshButtonTexture, imageButton, GUILayout.Width(16f)))
                ...
                if (isShowingTLSense) {
                    EditorGUILayout.BeginVertical("box");
                    EditorGUILayout.LabelField(
                        "Increasing this value will increase the scaling sensitivity relative to the size of the intersection. Higher scaling value = bigger traffic lights at further distances. Default value is 170.");
                    GUILayout.Space(4f);
                    EditorGUILayout.BeginHorizontal();
                    GUILayout.Button(refreshButtonTexture, imageButton, GUILayout.Width(16f));
                    EditorGUILayout.LabelField(" = Resets settings to default.");
            ...
            if (GUILayout.Button(refreshButtonTexture, imageButton, GUILayout.Width(16f))) gradeMod.floatValue = 0.375f;
            EditorGUILayout.EndHorizontal();
            isGradeCorrect = EditorGUILayout.Foldout(isGradeCorrect, status);
            if (isGradeCorrect) {
                EditorGUILayout.BeginVertical("box");
                EditorGUILayout.LabelField(
                    "If using extreme road grades immediately surrounding the intersection, terrain height matching errors may occur at the point of road expansion leading to the intersection.");
                EditorGUILayout.LabelField(
            ...
            EditorUtilities.DrawLine();
            isUsingDefaultMaterials.boolValue =
                EditorGUILayout.Toggle("Use default materials:", intersection.isUsingDefaultMaterials);
            isShowingDefaultMatHelp = EditorGUILayout.Foldout(isShowingDefaultMatHelp, status);
            if (isShowingDefaultMatHelp) {
                EditorGUILayout.BeginVertical("box");
                EditorGUILayout.LabelField(
                    "Checking this option will reset all materials for this intersection and use the default intersection materials that come with this addon.");
                EditorGUILayout.EndVertical();
            }
            ...
            if (intersection.markerCenter1 != null) {
                ...
                if (intersection.markerCenter2 != null &&
                    GUILayout.Button(deleteButtonTexture, imageButton, GUILayout.Width(16f)))
            ...
            if (intersection.markerCenter2 != null && intersection.markerCenter1 != null) {
                ...
                if (intersection.markerCenter3 != null &&
                    GUILayout.Button(deleteButtonTexture, imageButton, GUILayout.Width(16f)))
            ...
            if (isShowingMarkerCenter) {
                EditorGUILayout.BeginVertical("box");
                EditorGUILayout.LabelField(
                    "Center marker materials require transparent shaders. Covers the center area of the intersection. Displayed in order #1 on bottom to #4 on top. Combine with transparent shaders which accept shadows to allow for easy marking.");
                DoDefaultHelpMat();
                DoDeleteHelpMat();
                EditorGUILayout.EndVertical();
            }
            ...
            if (intersection.markerExtStretch1 != null) {
                ...
                if (intersection.markerExtStretch2 != null &&
                    GUILayout.Button(deleteButtonTexture, imageButton, GUILayout.Width(16f)))
            ...
            if (intersection.markerExtStretch2 != null && intersection.markerExtStretch1 != null) {
                ...
                if (intersection.markerExtStretch3 != null &&
                    GUILayout.Button(deleteButtonTexture, imageButton, GUILayout.Width(16f)))
            ...
            if (isShowingMarkerStretch) {
                EditorGUILayout.BeginVertical("box");
                EditorGUILayout.LabelField(
                    "Fitted marker materials require transparent shaders. Covers the exterior area of the intersection with the UV's stretched to match at a 1:1 ratio. Should be use for intersection markings and any visual effects like dirt." +
                    helpText1);
                DoDefaultHelpMat();
                DoDeleteHelpMat();
                EditorGUILayout.EndVertical();
            ...
            if (intersection.markerExtTiled1 != null) {
                ...
                if (intersection.markerExtTiled2 != null &&
                    GUILayout.Button(deleteButtonTexture, imageButton, GUILayout.Width(16f)))
            ...
            if (intersection.markerExtTiled2 != null && intersection.markerExtTiled1 != null) {
                ...
                if (intersection.markerExtTiled3 != null &&
                    GUILayout.Button(deleteButtonTexture, imageButton, GUILayout.Width(16f)))
            ...
            if (isShowingMarkerTiled) {
                EditorGUILayout.BeginVertical("box");
                EditorGUILayout.LabelField(
                    "Covers the exterior area of the intersection with the UV's tiled matching world coordinates. Tiled and used for road pavement textures. UV coordinates will match up seamlessly with road pavement." +
                    helpText1);
                DoDefaultHelpMat();
                DoDeleteHelpMat();
                EditorGUILayout.EndVertical();
            ...
            if (intersection.lane0Mat1 != null &&
                GUILayout.Button(deleteButtonTexture, imageButton, GUILayout.Width(16f))) intersection.lane0Mat1 = null;
            EditorGUILayout.EndHorizontal();
            if (intersection.lane0Mat1 != null) {
                EditorGUILayout.BeginHorizontal();
                EditorGUILayout.PropertyField(lane0Mat2, new GUIContent("Lane section 0 mat #2:"));
                if (intersection.lane0Mat2 != null &&
                    GUILayout.Button(deleteButtonTexture, imageButton, GUILayout.Width(16f)))
            ...
            if (intersection.lane1Mat1 != null &&
                GUILayout.Button(deleteButtonTexture, imageButton, GUILayout.Width(16f))) intersection.lane1Mat1 = null;
            EditorGUILayout.EndHorizontal();
            if (intersection.lane1Mat1 != null) {
                EditorGUILayout.BeginHorizontal();
                EditorGUILayout.PropertyField(lane1Mat2, new GUIContent("Lane section 1 mat #2:"));
                if (intersection.lane1Mat2 != null &&
                    GUILayout.Button(deleteButtonTexture, imageButton, GUILayout.Width(16f)))
            ...
            if (intersection.roadType == RoadIntersection.RoadTypeEnum.BothTurnLanes ||
                intersection.roadType == RoadIntersection.RoadTypeEnum.TurnLane) {
                EditorGUILayout.BeginHorizontal();
                ...
                if (intersection.lane2Mat1 != null) {
                    ...
                    if (intersection.lane2Mat2 != null &&
                        GUILayout.Button(deleteButtonTexture, imageButton, GUILayout.Width(16f)))
            ...
            if (intersection.roadType == RoadIntersection.RoadTypeEnum.BothTurnLanes) {
                ...
                if (intersection.lane3Mat1 != null) {
                    ...
                    if (intersection.lane3Mat2 != null &&
                        GUILayout.Button(deleteButtonTexture, imageButton, GUILayout.Width(16f)))
            ...
            if (isShowingLanes) {
                EditorGUILayout.BeginVertical("box");
                EditorGUILayout.LabelField(
                    "Covers individual lane sections in the intersection. Used for high-definition line marking.");
                EditorGUILayout.LabelField(
                ...
                if (intersection.roadType == RoadIntersection.RoadTypeEnum.BothTurnLanes) {
                    ...
                    EditorGUILayout.LabelField(
                        "Lane section 1 = Right outgoing traffic lane. Use lane texture which matches the number of lanes used on the road system with a white right line.");
                }
            ...
            //Set change bools:
            var isTrafficLightPoleColorToggled = false;
            var isPointLightsToggled = false;
            var isGizmoChanged = false;
            var isMaterialChanged = false;
            var isRoadUpdate = false;

            if (GUI.changed) {
                //Option: Gizmo:
                if (isDrawingGizmo.boolValue != intersection.isDrawingGizmo) isGizmoChanged = true;
                ...
                //Option: Left yield on green:
                if (isLeftTurnYieldOnGreen.boolValue != intersection.isLeftTurnYieldOnGreen) isRoadUpdate = true;

                //Option: Intersection stop type:
                if (intersectionStopType.enumValueIndex != (int)intersection.intersectionStopType) isRoadUpdate = true;

                //Option: Traffic light poles:
                if (isTrafficPoleStreetLight.boolValue != intersection.isTrafficPoleStreetLight) isRoadUpdate = true;
                ...
                //Option: Normal pole alignment:
                if (isRegularPoleAlignment.boolValue != intersection.isRegularPoleAlignment) isRoadUpdate = true;

                //Option: Point lights enabled:
                if (isLightsEnabled.boolValue != intersection.isLightsEnabled) isPointLightsToggled = true;

                //Option: Street light range:
                if (!RootUtils.IsApproximately(streetLightRange.floatValue, intersection.streetLightRange))
                    isPointLightsToggled = true;

                //Option: Street light intensity:
                if (!RootUtils.IsApproximately(streetLightIntensity.floatValue, intersection.streetLightIntensity))
                    isPointLightsToggled = true;

                //Option: Street light color:
                if (streetLightColor.colorValue != intersection.streetLightColor) isPointLightsToggled = true;

                //Option: Use default materials:
                if (isUsingDefaultMaterials.boolValue != intersection.isUsingDefaultMaterials) isMaterialChanged = true;
                ...
                if (markerExtTiled3 == null || markerExtTiled3.objectReferenceValue == null)
                    intersection.markerExtTiled3 = null;
                if (lane0Mat1 == null || lane0Mat1.objectReferenceValue == null) intersection.lane0Mat1 = null;
                if (lane0Mat2 == null || lane0Mat2.objectReferenceValue == null) intersection.lane0Mat2 = null;
                if (lane1Mat1 == null || lane1Mat1.objectReferenceValue == null) intersection.lane1Mat1 = null;
                if (lane1Mat2 == null || lane1Mat2.objectReferenceValue == null) intersection.lane1Mat2 = null;
                if (lane2Mat1 == null || lane2Mat1.objectReferenceValue == null) intersection.lane2Mat1 = null;
                if (lane2Mat2 == null || lane2Mat2.objectReferenceValue == null) intersection.lane2Mat2 = null;
                if (lane3Mat1 == null || lane3Mat1.objectReferenceValue == null) intersection.lane3Mat1 = null;
                if (lane3Mat2 == null || lane3Mat2.objectReferenceValue == null) intersection.lane3Mat2 = null;

                if (intersection.markerCenter1 != null &&
                    markerCenter1.objectReferenceValue != intersection.markerCenter1) isMaterialChanged = true;
                if (intersection.markerCenter2 != null &&
                    markerCenter2.objectReferenceValue != intersection.markerCenter2) isMaterialChanged = true;
                if (intersection.markerCenter3 != null &&
                    markerCenter3.objectReferenceValue != intersection.markerCenter3) isMaterialChanged = true;
                if (intersection.markerExtStretch1 != null &&
                    markerExtStretch1.objectReferenceValue != intersection.markerExtStretch1) isMaterialChanged = true;
                if (intersection.markerExtStretch2 != null &&
                    markerExtStretch2.objectReferenceValue != intersection.markerExtStretch2) isMaterialChanged = true;
                if (intersection.markerExtStretch3 != null &&
                    markerExtStretch3.objectReferenceValue != intersection.markerExtStretch3) isMaterialChanged = true;
                if (intersection.markerExtTiled1 != null &&
                    markerExtTiled1.objectReferenceValue != intersection.markerExtTiled1) isMaterialChanged = true;
                if (intersection.markerExtTiled2 != null &&
                    markerExtTiled2.objectReferenceValue != intersection.markerExtTiled2) isMaterialChanged = true;
                if (intersection.markerExtTiled3 != null &&
                    markerExtTiled3.objectReferenceValue != intersection.markerExtTiled3) isMaterialChanged = true;
                ...
                if (isGizmoChanged) {
                    var meshRenderers = intersection.transform.GetComponentsInChildren<MeshRenderer>();
                    var count = meshRenderers.Length;
                    for (var i = 0; i < count; i++)
                        //EditorUtility.SetSelectedWireframeHidden(tMRs[i], !tInter.bDrawGizmo);
                        EditorUtility.SetSelectedRenderState(meshRenderers[i],
                            intersection.isDrawingGizmo
                                ? EditorSelectedRenderState.Wireframe
                                : EditorSelectedRenderState.Hidden);
                    SceneView.RepaintAll();
                }

                if (isPointLightsToggled) intersection.TogglePointLights(intersection.isLightsEnabled);

                if (isTrafficLightPoleColorToggled) intersection.ToggleTrafficLightPoleColor();

                if (isMaterialChanged) {
                    intersection.UpdateMaterials();
                    if (intersection.isUsingDefaultMaterials) intersection.ResetMaterialsAll();
                }

                if (isRoadUpdate) TriggerRoadUpdate();
        ...
        private void TriggerRoadUpdate(bool _isForced = false) {
            if (!_isForced && !intersection.isAutoUpdatingIntersection) return;

            if (intersection != null && intersection.node1 != null && intersection.node2 != null)
                if (!intersection.node1.spline.road.isEditorConstructing &&
                    !intersection.node2.spline.road.isEditorConstructing) {
        ...
        private bool isShowingMarkerStretch;
        private bool isShowingMarkerTiled;
        private bool isShowingMarkerCenter;
        private bool isShowingTLSense;
        private bool isShowingTLPole;
        private bool isShowingLightHelp;
        private bool isShowingLanes;
        private bool isGradeCorrect;
        private bool isShowingDefaultMatHelp;
        private bool isShowingHelpLeftTurnGreen;
        private readonly string status = "Show help";
        ...
        private const string helpText1 =
            "Each material added is rendered on top of the previous. Combine with transparent shaders which accept shadows to allow for easy marking.";
