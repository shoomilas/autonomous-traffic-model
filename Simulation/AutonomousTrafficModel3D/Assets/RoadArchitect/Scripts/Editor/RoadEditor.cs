...
namespace RoadArchitect {
    ...
    public class RoadEditor : Editor {
        private static readonly string[] RoadMaterialDropdownEnumDesc =
        {
            "Asphalt",
            "Dirt",
            "Brick",
        ...
        private static readonly string[] tempEnumDescriptions =
        {
            "Two",
            "Four",
            "Six"
        ...
        public void OnSceneGUI() {
            ...
            if (Selection.Contains(road.transform.gameObject) && Selection.objects.Length > 1) SetSelectionToRoad();
            ...
            if (Selection.activeGameObject == road.transform.gameObject) {
                    ...
                    if (current.control) {
                        ...
                        if (Physics.Raycast(worldRay, out hitInfo)) {
                            ...
                            if (road.spline && road.spline.previewSpline) {
                                ...
                                if (road.spline.previewSpline.nodes == null ||
                                    road.spline.previewSpline.nodes.Count < 1) road.spline.Setup();
                        ...
                        if (Physics.Raycast(worldRay, out hitInfo)) {
                            ...
                            if (road.spline && road.spline.previewSplineInsert) {
                                if (road.spline.previewSplineInsert.nodes == null ||
                                    road.spline.previewSplineInsert.nodes.Count < 1)
                ...
                if (road.spline.previewSpline) road.spline.previewSpline.isDrawingGizmos = false;
            }


            if (current.shift && road.spline.previewSpline != null) road.spline.previewSpline.isDrawingGizmos = false;
            var isUsed = false;
            if (current.control) {
            ...
            if (GUI.changed) EditorUtility.SetDirty(road);
        }
        ...
        public override void OnInspectorGUI() {
            ...
            if (GUILayout.Button("Update road", loadButton)) road.isUpdateRequired = true;
            ...
            road.newNodePreviewColor =
                EditorGUILayout.ColorField("Gizmo new Node preview Color: ", road.newNodePreviewColor);
            ...
            else if (tLanesEnum == tempEnum.Six) lanesAmount.intValue = 6;

            //Option: Lane and road width:
            EditorGUILayout.BeginHorizontal();
            laneWidth.floatValue = EditorGUILayout.FloatField("Lane width:", road.laneWidth);
            if (GUILayout.Button(refreshButtonTexture, imageButton, GUILayout.Width(16f))) laneWidth.floatValue = 5f;
            ...
            if (road.isUsingDefaultMaterials) {
                var Old = (int)road.roadMaterialDropdown;
                roadMaterialDropdown.enumValueIndex = EditorGUILayout.Popup("Road material: ",
                    (int)road.roadMaterialDropdown, RoadMaterialDropdownEnumDesc, GUILayout.Width(250f));
            ...
            EditorGUILayout.BeginHorizontal();
            matchTerrainSubtraction.floatValue =
                EditorGUILayout.Slider("Terrain subtraction: ", road.matchTerrainSubtraction, 0.01f, 1f);
            if (GUILayout.Button(refreshButtonTexture, imageButton, GUILayout.Width(16f)))
            ...
            EditorGUILayout.BeginHorizontal();
            magnitudeThreshold.floatValue =
                EditorGUILayout.Slider("Spline magnitude limit: ", road.magnitudeThreshold, 128f, 8192f);
            if (GUILayout.Button(refreshButtonTexture, imageButton, GUILayout.Width(16f)))
                magnitudeThreshold.floatValue = 300f;
            EditorGUILayout.EndHorizontal();

            //Option: Height modification
            isHeightModificationEnabled.boolValue =
                EditorGUILayout.Toggle("Height modification: ", road.isHeightModificationEnabled);

            //Option: Active detail removal
            isDetailModificationEnabled.boolValue =
                EditorGUILayout.Toggle("Active detail removal: ", road.isDetailModificationEnabled);

            //Option: Active tree removal
            isTreeModificationEnabled.boolValue =
                EditorGUILayout.Toggle("Active tree removal: ", road.isTreeModificationEnabled);

            //Option: heights width
            if (road.isHeightModificationEnabled) {
                EditorGUILayout.BeginHorizontal();
                matchHeightsDistance.floatValue =
                    EditorGUILayout.Slider("Heights match width: ", road.matchHeightsDistance, 0.01f, 512f);
                if (GUILayout.Button(refreshButtonTexture, imageButton, GUILayout.Width(16f)))
            ...
            if (road.isDetailModificationEnabled) {
                EditorGUILayout.BeginHorizontal();
                clearDetailsDistance.floatValue =
                    EditorGUILayout.Slider("Details clear width: ", road.clearDetailsDistance, 0.01f, 512f);
                if (GUILayout.Button(refreshButtonTexture, imageButton, GUILayout.Width(16f)))
                ...
                EditorGUILayout.BeginHorizontal();
                clearDetailsDistanceHeight.floatValue = EditorGUILayout.Slider("Details clear height: ",
                    road.clearDetailsDistanceHeight, 0.01f, 512f);
                if (GUILayout.Button(refreshButtonTexture, imageButton, GUILayout.Width(16f)))
            ...
            if (road.isTreeModificationEnabled) {
                EditorGUILayout.BeginHorizontal();
                clearTreesDistance.floatValue =
                    EditorGUILayout.Slider("Trees clear width: ", road.clearTreesDistance, 0.01f, 512f);
                if (GUILayout.Button(refreshButtonTexture, imageButton, GUILayout.Width(16f)))
                    clearTreesDistance.floatValue = 30f;
                EditorGUILayout.EndHorizontal();

                EditorGUILayout.BeginHorizontal();
                clearTreesDistanceHeight.floatValue = EditorGUILayout.Slider("Trees clear height: ",
                    road.clearTreesDistanceHeight, 0.01f, 512f);
                if (GUILayout.Button(refreshButtonTexture, imageButton, GUILayout.Width(16f)))
            ...
            isSavingTerrainHistoryOnDisk.boolValue =
                EditorGUILayout.Toggle(road.isSavingTerrainHistoryOnDisk, GUILayout.Width(50f));
            EditorGUILayout.EndHorizontal();
            ...
            if (!road.isDynamicCutsEnabled) {
                ...
                if (road.isShouldersEnabled)
                    isShoulderCutsEnabled.boolValue =
                        EditorGUILayout.Toggle("Auto split shoulders: ", road.isShoulderCutsEnabled);
            }
            else {
                EditorGUILayout.BeginHorizontal();
            ...
            editorCameraMetersPerSecond.floatValue =
                EditorGUILayout.Slider("Camera meters/sec:", road.EditorCameraMetersPerSecond, 1f, 512f);
            if (GUILayout.Button(refreshButtonTexture, imageButton, GUILayout.Width(16f)))
                editorCameraMetersPerSecond.floatValue = 60f;
            EditorGUILayout.EndHorizontal();

            //Option: Editor camera auto rotate:
            isEditorCameraRotated.boolValue =
                EditorGUILayout.Toggle("Camera auto rotate: ", road.isEditorCameraRotated);
            if (road.editorPlayCamera == null) road.EditorCameraSetSingle();
            road.editorPlayCamera =
                (Camera)EditorGUILayout.ObjectField("Editor play camera:", road.editorPlayCamera, typeof(Camera), true);
            EditorGUILayout.BeginHorizontal();
            ...
            if (road.isEditorCameraMoving) {
                if (GUILayout.Button("Pause", GUILayout.Width(70f))) road.isEditorCameraMoving = false;
            }
            else {
                if (GUILayout.Button("Play", GUILayout.Width(70f))) road.isEditorCameraMoving = true;
            }
            ...
            if (isResetingTH)
                EditorGUILayout.LabelField(
                    "WARNING: This option can't be undone! Only reset the terrain history if you have changed terrain resolution data such as heightmap or detail resolutions. A rare event may occur when editing and compiling this addon's scripts that a terrain history reset may be necessary. Treat this reset as a last resort.",
                    warningLabelStyle);
            GUILayout.Space(6f);
            ...
            EditorGUILayout.LabelField("Total nodes: " + road.MostRecentNodeCount);
            EditorGUILayout.EndVertical();

            var isGizmoChanged = false;
            var isLaneChanged = false;
            var isTerrainHistoryChanged = false;
            var isEditorCameraSpeedChanged = false;
            ...
            if (GUI.changed) {
                ...
                if (!RootUtils.IsApproximately(editorCameraMetersPerSecond.floatValue, road.EditorCameraMetersPerSecond,
                    0.001f)) isEditorCameraSpeedChanged = true;
                ...
                //Option: Editor camera meters per sec
                if (isEditorCameraSpeedChanged) road.ChangeEditorCameraMetersPerSec();

                //Update road:
                if (isNeedingRoadUpdate) road.spline.TriggerSetup();
        ...
        private void RenderRoadMaterials() {
            ...
            //Option: Set mats to defaults:
            isUsingDefaultMats = false;
            if (GUILayout.Button(refreshButtonTexture, imageButton, GUILayout.Width(16f))) isUsingDefaultMats = true;
            EditorGUILayout.EndHorizontal();


            //Option: Use default materials:
            isUsingDefaultMaterials.boolValue =
                EditorGUILayout.Toggle("Use default materials:", road.isUsingDefaultMaterials);

            EditorGUILayout.BeginHorizontal();
            EditorGUILayout.PropertyField(roadMaterial1, new GUIContent("  Mat #1: "));
            if (GUILayout.Button(deleteButtonText, imageButton, GUILayout.Width(16f))) road.RoadMaterial1 = null;
            EditorGUILayout.EndHorizontal();
            ...
            if (road.RoadMaterial1 != null) {
                ...
                if (GUILayout.Button(deleteButtonText, imageButton, GUILayout.Width(16f))) road.RoadMaterial2 = null;
                EditorGUILayout.EndHorizontal();
        ...
        private void RenderRoadMarkerMaterials() {
            ...
            if (GUILayout.Button(deleteButtonText, imageButton, GUILayout.Width(16f))) road.RoadMaterialMarker1 = null;
            EditorGUILayout.EndHorizontal();
        ...
        private void RenderMaterialsHelpDialog() {
            ...
            if (isShowingMaterialsHelp) {
                ...
                GUILayout.Space(4f);
                EditorGUILayout.LabelField(
                    "The physical material for road will be used on all road colliders. The physical material for shoulder will be used on all shoulder colliders. If using road and or shoulder cuts, you can specficy unique physics materials which will be used instead of the default physics materials.");
                GUILayout.Space(4f);
                EditorGUILayout.LabelField(
        ...
        private void RenderCutsHelpDialog() {
            ...
            if (isShowingCutsHelp) {
                ...
                GUILayout.Space(4f);
                EditorGUILayout.LabelField(
                    "Choosing split options will split the road/shoulder up into pieces mirroring the locations of nodes.");
                GUILayout.Space(4f);
                EditorGUILayout.LabelField(
                    "Splitting allows for more detailed and flexible road texturing options such as passing sections, other different road lines per section, road debris and more.");
                GUILayout.Space(4f);
                EditorGUILayout.LabelField(
                    "Choosing split options may also speed up mesh collider collision calculations if bounds calculations are involved.");
                GUILayout.Space(4f);

                EditorGUILayout.LabelField("Which splitting option to choose?", EditorStyles.boldLabel);
                EditorGUILayout.LabelField(
                    "Choose no splitting if you desire a single material set for this entire road and your game experiences no collison processing slowdowns from one large mesh collider. This option will create less game objects than automatic and manual splitting.");
                GUILayout.Space(4f);
                EditorGUILayout.LabelField(
                ...
                GUILayout.Space(4f);
                EditorGUILayout.LabelField(
                    "Example hill: Goal is to have double yellow no passing lines on a two lane road but only while the road is near or on the hill. " +
                ...
                GUILayout.Space(4f);
                EditorGUILayout.LabelField(
                    "Example mountains: In the mountains, the road is curvy and the grade is high. " +
        ...
        private void RenderTerrainHelpDialog() {
            ...
            if (isShowingTerrainHelp) {
                ...
                EditorGUILayout.LabelField(
                    "This value, in meters, will be subtracted from the terrain match height to prevent z-fighting.");
                GUILayout.Space(4f);
                EditorGUILayout.LabelField("Spline magnitude limit: ", EditorStyles.boldLabel);
                EditorGUILayout.LabelField(
                    "Limits the magnitude of the spline nodes. Lower limit is better for typical roads with node seperation of around 100 to 300 meters. Higher limits will allow for less tension when using very spread out nodes.");
                GUILayout.Space(4f);
                ...
                EditorGUILayout.LabelField(
                    "Enables or disables active detail removal. Memory intensive on large terrains with large amounts of details. Recommended to not use this option and instead remove details and trees via splat maps with other addons.");
                GUILayout.Space(4f);
                EditorGUILayout.LabelField("Active tree removal:", EditorStyles.boldLabel);
                EditorGUILayout.LabelField(
                    "Enables or disables active tree removal. Memory intensive on large terrains with large amounts of trees. Recommended to not use this option and instead remove details and trees via splat maps with other addons.");
                GUILayout.Space(4f);
                EditorGUILayout.LabelField("Heights match width:", EditorStyles.boldLabel);
                EditorGUILayout.LabelField(
                    "The distance to the left and right of the road in which terrain heights will be matched to the road.");
                GUILayout.Space(4f);
                EditorGUILayout.LabelField("Details clear width:", EditorStyles.boldLabel);
                EditorGUILayout.LabelField(
                    "The distance between the road and detail, width wise, in which details will be removed.");
                GUILayout.Space(4f);
                EditorGUILayout.LabelField("Details clear height:", EditorStyles.boldLabel);
                EditorGUILayout.LabelField(
                    "The distance between the road and detail, height wise, in which details will be removed.");
                GUILayout.Space(4f);
                EditorGUILayout.LabelField("Tree clear width:", EditorStyles.boldLabel);
                EditorGUILayout.LabelField(
                    "The distance between the road and tree, width wise, in which trees will be removed.");
                GUILayout.Space(4f);
                EditorGUILayout.LabelField("Tree clear height:", EditorStyles.boldLabel);
                EditorGUILayout.LabelField(
                    "The distance between the road and tree, height wise, in which trees will be removed.");
                GUILayout.Space(4f);
                EditorGUILayout.LabelField("Store terrain history separate from scene:", EditorStyles.boldLabel);
                EditorGUILayout.LabelField(
                    "If enabled, stores the terrain history immediately on disk after use, saving memory while in editor.");
                GUILayout.Space(4f);
                EditorGUILayout.LabelField("Terrain history size:", EditorStyles.boldLabel);
                EditorGUILayout.LabelField(
                    "Shows the size, in kilobytes, of the terrain history in memory or on disk.");
                GUILayout.Space(4f);
                EditorGUILayout.BeginHorizontal();
                GUILayout.Button(refreshButtonTexture, imageButton, GUILayout.Width(16f));
                EditorGUILayout.LabelField(" = Resets settings to default.");
        ...
        private void RenderRoadHelpDialog() {
            ...
            if (isShowingRoadHelp) {
                ...
                EditorGUILayout.LabelField(
                    "Enable or disable most gizmos for this road. Disable mesh collider gizmos via the unity menu if necessary or desired.",
                ...
                EditorGUILayout.LabelField(
                    "When enabled will use default materials for the road system, allowing certain aspects of generation to automatically determine the correct materials to utilize.");
                EditorGUILayout.EndVertical();
                GUILayout.Space(4f);
                EditorGUILayout.BeginVertical();
        ...
        private void TriggerRoadUpdate() {
            if (road != null) road.isUpdateRequired = true;
        }


        private void SetSelectionToRoad() {
            var objects = new GameObject[1];
        ...
        private bool isShowingCutsHelp;
        private bool isShowingMaterialsHelp;
        private bool isShowingRoadHelp;
        private bool isShowingTerrainHelp;
        private bool isShowingCameraHelp;
        private bool isResetingTH;
        ...
        private GUIStyle maybeButton;
        private Texture2D warningLabelBG;
        private Texture2D loadBtnBG;
        private Texture2D loadBtnBGGlow;
        private Texture refreshButtonTexture;
        private Texture deleteButtonText;
        private Texture refreshButtonTextureReal;

        //Buffers:
        private bool isNeedingRoadUpdate;
        private bool isUsingDefaultMats;
        private bool isApplyingMaterialsCheck;
        private bool isApplyingMatsCheck;
