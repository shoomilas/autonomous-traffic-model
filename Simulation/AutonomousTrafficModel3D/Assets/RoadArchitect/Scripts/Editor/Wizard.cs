    ...
    public class Wizard : EditorWindow {
        ...
        private void DoGUI() {
            ...
            if (thisNode.isBridgeStart) {
                xWindowType = (WindowTypeEnum)EditorGUILayout.Popup("Category: ", (int)tWindowType,
                    WindowTypeDescBridge, GUILayout.Width(312f));
            }
            else {
                ...
                SxWindowType = (WindowTypeEnumShort)EditorGUILayout.Popup("Category: ", (int)StWindowType,
                    WindowTypeDesc, GUILayout.Width(312f));
            ...
            if ((((yMax) * spacingHeight) + 25) > position.height) {
                scrollPos = GUI.BeginScrollView(new Rect(0, 25, position.width - 10, position.height - 30), scrollPos,
                    new Rect(0, 0, (yMod * spacingWidth) + 25, (((yMax) * spacingHeight) + 50)));
                isScrolling = true;
            ...
            for (int i = 0; i < objectCount; i++) {
                ...
                if (isClicked) {
                    if (tWindowType == WindowTypeEnum.Extrusion) {
                        SplinatedMeshMaker SMM = thisNode.AddSplinatedObject();
                        SMM.SetDefaultTimes(thisNode.isEndPoint, thisNode.time, thisNode.nextTime, thisNode.idOnSpline,
                            thisNode.spline.distance);
                        ...
                        EdgeObjectMaker EOM = thisNode.AddEdgeObject();
                        EOM.SetDefaultTimes(thisNode.isEndPoint, thisNode.time, thisNode.nextTime, thisNode.idOnSpline,
                            thisNode.spline.distance);
                        ...
                        thisNode.LoadWizardObjectsFromLibrary(objectList[i].fileName, objectList[i].isDefault,
                            objectList[i].isBridge);
                    }
        ...
        public static void GetGroupListing(out string[] _names, out string[] _paths, int _lanes,
        ...
        private void LoadObjs(ref string[] _names, ref string[] _paths, bool _isDefault = false) {
            ...
            for (int i = 0; i < namesCount; i++) {
                ...
                if (tWindowType == WindowTypeEnum.Extrusion) {
                    SplinatedMeshMaker.SplinatedMeshLibraryMaker SLM =
                        RootUtils.LoadXML<SplinatedMeshMaker.SplinatedMeshLibraryMaker>(ref path);
                    if (SLM == null) {
                        continue;
                    }
                    ...
                    EdgeObjectMaker.EdgeObjectLibraryMaker ELM =
                        RootUtils.LoadXML<EdgeObjectMaker.EdgeObjectLibraryMaker>(ref path);
                    if (ELM == null) {
                        continue;
                    }
                ...
                if (_isDefault && tWindowType == WindowTypeEnum.Extrusion) {
                    if (tO.displayName.Contains("GSDKRailCurvedR")) {
                        ...
                        tO.desc =
                            "Federal spec cement KRailing (also known as Jersey Barriers). Variant with curved ends for right shoulder.";
                    }
                    else if (tO.displayName.Contains("GSDKRailCurvedL")) {
                        tO.displayName = "KRail Curved Left";
                        tO.desc =
                            "Federal spec cement KRailing (also known as Jersey Barriers). Variant with curved ends for left shoulder.";
                    }
