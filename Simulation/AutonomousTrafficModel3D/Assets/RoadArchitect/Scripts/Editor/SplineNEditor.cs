...
namespace RoadArchitect {
    ...
    public class SplineNEditor : Editor {
        ...
        public void OnSceneGUI() {
            ...
            if (!node.isEditorSelected) node.isEditorSelected = true;
            ...
            if (controlID != node.GetHashCode()) node.isEditorSelected = false;
            ...
            if (Event.current.type == EventType.MouseUp && Event.current.button == 0) {
                Object[] nodeObjects = FindObjectsOfType<SplineN>();
                foreach (SplineN xNode in nodeObjects)
                    if (Vector3.Distance(xNode.transform.position, node.transform.position) < 2f) {
                        if (xNode == node) continue;
                        if (node.isSpecialEndNode || xNode.isSpecialEndNode) continue;
                        ...
                        if (xNode.isIntersection) continue;
                        if (xNode.isNeverIntersect) continue;
                        if (node.isEndPoint && xNode.isEndPoint) continue;
                ...
                if (!isMouseDragProcessed) {
                    ...
                    if (node.IsLegitimate() && node.spline.road.isMaxGradeEnabled) node.EnsureGradeValidity();
                    TriggerRoadUpdate();
                    isUsed = true;
                }
            ...
            //Enforce maximum road grade:
            if (isMouseDragProcessed) {
                var vChangeChecker = node.transform.position;
        ...
        private void Init() {
            ...
            var halfRoadWidth = node.spline.road.RoadWidth() * 0.5f;
            ...
            if (node.spline.road.isShouldersEnabled) {
                HorizMatchSubTypeDescriptions[3] = "Match shoulder left edge: -" +
                                                   (halfRoadWidth + node.spline.road.shoulderWidth).ToString("F1") +
                                                   " meters";
                HorizMatchSubTypeDescriptions[5] = "Match shoulder right edge: " +
                                                   (halfRoadWidth + node.spline.road.shoulderWidth).ToString("F1") +
                                                   " meters";
            }
            else {
                HorizMatchSubTypeDescriptions[2] =
                    "Match shoulder left edge: -" + halfRoadWidth.ToString("F1") + " meters";
                HorizMatchSubTypeDescriptions[4] =
                    "Match shoulder right edge: " + halfRoadWidth.ToString("F1") + " meters";
            }
        ...
        public override void OnInspectorGUI() {
            ...
            if (node.idOnSpline > 0 && node.idOnSpline < node.spline.GetNodeCount() - 1 && !node.isIntersection &&
                !node.isSpecialEndNode) {
            ...
            if (!node.isEndPoint) {
                ...
                if (isBridgeDisplayed) EditorUtilities.DrawLine();
            }

            #endregion


            if (Selection.objects.Length == 1 && Selection.objects[0] is SplineN ||
                node.specialNodeCounterpart == null && !node.isSpecialRoadConnPrimary) {
                //Do extrusion and edge objects overview:
                DoExtAndEdgeOverview();
            }
            else {
                EditorGUILayout.BeginHorizontal();
                if (GUILayout.Button("Road objects")) Selection.objects = new Object[1] { node.specialNodeCounterpart };
            ...
            if (node.isSpecialRoadConnPrimary) {
                ...
                if (GUILayout.Button("Update road connection")) {
                    var node1 = node.originalConnectionNodes[0];
                    var node2 = node.originalConnectionNodes[1];
                    node.specialNodeCounterpart.BreakConnection();
                    node.spline.road.UpdateRoad();
                    node1.spline.ActivateEndNodeConnection(node1, node2);
                }

                if (GUILayout.Button("Break road connection")) node.specialNodeCounterpart.BreakConnection();
                ...
                if (node.specialNodeCounterpart != null)
                    EditorGUILayout.LabelField(node.specialNodeCounterpart.spline.road.transform.name + " to " +
                                               node.specialNodeCounterpart.specialNodeCounterpart.spline.road.transform
                                                   .name);
                EditorGUILayout.LabelField(
                    "To break this road connection, click the \"Break road connection\" button.");
                EditorUtilities.DrawLine();
            }
        ...
        private void DoExtAndEdgeOverview() {
            ...
            if (GUILayout.Button("Save group", EditorStyles.miniButton, GUILayout.Width(108f)) ||
                GUILayout.Button(saveButtonTexture, imageButton, GUILayout.Width(16f))) {
                var saveWindow = EditorWindow.GetWindow<SaveWindow>();
                if (node.isBridge)
            ...
            if (GUILayout.Button("Open Wizard", loadButton, GUILayout.Width(128f))) {
                ...
                var wizard = EditorWindow.GetWindow<Wizard>();
                if (sceneRect.x < 0) sceneRect.x = 0f;
                if (sceneRect.y < 0) sceneRect.y = 0f;
            ...
            if (isEdgeObjectHelp) {
                ...
                EditorGUILayout.LabelField("");
                EditorGUILayout.EndHorizontal();
                EditorGUILayout.LabelField("Railings, bridge pieces, center dividers and other connected objects.",
                ...
                EditorGUILayout.LabelField("");
                EditorGUILayout.EndHorizontal();
                EditorGUILayout.LabelField("Signs, street lights, bridge pillars and other unconnected road objects.",
                    EditorStyles.miniLabel);
                EditorGUILayout.EndVertical();
                ...
                EditorGUILayout.LabelField("= Saves object config to library for use on other nodes.",
                    EditorStyles.miniLabel);
                EditorGUILayout.EndHorizontal();
                EditorGUILayout.EndVertical();
            ...
            EditorGUILayout.BeginHorizontal();
            EditorGUILayout.LabelField("");
            if (GUILayout.Button("Add custom extrusion object", EditorStyles.miniButton)) node.AddSplinatedObject();
            EditorGUILayout.EndHorizontal();
            GUILayout.Space(4f);
            EditorGUILayout.BeginHorizontal();
            EditorGUILayout.LabelField("");
            if (GUILayout.Button("Add custom edge object", EditorStyles.miniButton)) node.AddEdgeObject();
            EditorGUILayout.EndHorizontal();
        ...
        public void DoSplineObjects() {
            if (!node.CanSplinate()) return;
            if (node.SplinatedObjects == null) node.SplinatedObjects = new List<SplinatedMeshMaker>();
            ...
            for (var index = 0; index < node.SplinatedObjects.Count; index++) {
                currentCount += 1;
                SMM = node.SplinatedObjects[index];
                if (SMM.splinatedMeshEditorMaker == null)
                    SMM.splinatedMeshEditorMaker = new SplinatedMeshMaker.SplinatedMeshEditorMaker();
                ...
                if (SMM.isRequiringUpdate) SMM.Setup(true);


                EditorGUILayout.BeginHorizontal();

                SMM.isToggled = EditorGUILayout.Foldout(SMM.isToggled, "#" + currentCount + ": " + SMM.objectName);

                if (GUILayout.Button(extrudeButtonTexture, imageButton, GUILayout.Width(32f))) { }

                if (GUILayout.Button(refreshButtonTexture, imageButton, GUILayout.Width(16f))) SMM.Setup();
                if (GUILayout.Button(saveButtonTexture, imageButton, GUILayout.Width(16f))) {
                    var saveDialog = EditorWindow.GetWindow<SaveWindow>();
                ...
                //Game object (prefab):
                SMM.splinatedMeshEditorMaker.CurrentSplination =
                    (GameObject)EditorGUILayout.ObjectField("Prefab:", SMM.currentSplination, typeof(GameObject),
                        false);

                //Game object (prefab start cap):
                SMM.splinatedMeshEditorMaker.CurrentSplinationCap1 =
                    (GameObject)EditorGUILayout.ObjectField("Prefab start cap:", SMM.currentSplinationCap1,
                        typeof(GameObject), false);
                //Prefab start cap height offset:
                if (SMM.currentSplinationCap1 != null)
                    SMM.splinatedMeshEditorMaker.CapHeightOffset1 =
                        EditorGUILayout.FloatField("  Height offset:", SMM.capHeightOffset1);

                //Game object (prefab end cap):
                SMM.splinatedMeshEditorMaker.CurrentSplinationCap2 =
                    (GameObject)EditorGUILayout.ObjectField("Prefab end cap:", SMM.currentSplinationCap2,
                        typeof(GameObject), false);
                //Prefab end cap height offset:
                if (SMM.currentSplinationCap2 != null)
                    SMM.splinatedMeshEditorMaker.CapHeightOffset2 =
                        EditorGUILayout.FloatField("  Height offset:", SMM.capHeightOffset2);

                //Material overrides:
                SMM.splinatedMeshEditorMaker.isMaterialOverriden =
                    EditorGUILayout.Toggle("Material override: ", SMM.isMaterialOverriden);
                if (SMM.isMaterialOverriden) {
                    SMM.splinatedMeshEditorMaker.SplinatedMaterial1 =
                        (Material)EditorGUILayout.ObjectField("Override mat #1: ", SMM.SplinatedMaterial1,
                            typeof(Material), false);
                    SMM.splinatedMeshEditorMaker.SplinatedMaterial2 =
                        (Material)EditorGUILayout.ObjectField("Override mat #2: ", SMM.SplinatedMaterial2,
                            typeof(Material), false);
                }

                //Axis:
                SMM.splinatedMeshEditorMaker.Axis = (Splination.AxisTypeEnum)EditorGUILayout.Popup("Extrusion axis: ",
                    (int)SMM.Axis, TheAxisDescriptionsSpline, GUILayout.Width(250f));

                //Start time:
                if (SMM.StartTime < node.minSplination) SMM.StartTime = node.minSplination;
                if (SMM.EndTime > node.maxSplination) SMM.EndTime = node.maxSplination;
                EditorGUILayout.BeginHorizontal();

                SMM.splinatedMeshEditorMaker.StartTime = EditorGUILayout.Slider("Start param: ", SMM.StartTime,
                    node.minSplination, node.maxSplination - 0.01f);
                ...
                //End time:
                EditorGUILayout.BeginHorizontal();
                SMM.splinatedMeshEditorMaker.EndTime =
                    EditorGUILayout.Slider("End param: ", SMM.EndTime, SMM.StartTime, node.maxSplination);
                ...
                if (node.IsStraight()) {
                    if (!SMM.isStretched) {
                        SMM.splinatedMeshEditorMaker.isStretched =
                            EditorGUILayout.Toggle("Straight line stretch:", SMM.isStretched);
                    }
                    else {
                        EditorGUILayout.BeginVertical("box");
                        SMM.splinatedMeshEditorMaker.isStretched =
                        ...
                        //Stretch_UVThreshold:
                        SMM.splinatedMeshEditorMaker.stretchedUVThreshold =
                            EditorGUILayout.Slider("UV stretch threshold:", SMM.stretchUVThreshold, 0.01f, 0.5f);

                        //UV repeats:
                        SMM.splinatedMeshEditorMaker.repeatUVType = (RepeatUVTypeEnum)EditorGUILayout.Popup(
                            "UV stretch axis: ", (int)SMM.RepeatUVType, RepeatUVTypeDescriptionsSpline,
                ...
                SMM.splinatedMeshEditorMaker.isMatchingTerrain =
                    EditorGUILayout.Toggle("Match ground: ", SMM.isMatchingTerrain);

                //Vector min/max threshold: 
                EditorGUILayout.BeginHorizontal();
                SMM.splinatedMeshEditorMaker.MinMaxMod =
                    EditorGUILayout.Slider("Vertex min/max threshold: ", SMM.minMaxMod, 0.01f, 0.2f);
                if (GUILayout.Button(defaultButtonTexture, imageButton, GUILayout.Width(16f)))
                ...
                EditorGUILayout.BeginHorizontal();
                SMM.splinatedMeshEditorMaker.VertexMatchingPrecision =
                    EditorGUILayout.Slider("Vertex matching precision: ", SMM.vertexMatchingPrecision, 0f, 0.01f);
                if (GUILayout.Button(defaultButtonTexture, imageButton, GUILayout.Width(16f)))
                ...
                if (!SMM.isStretched)
                    SMM.splinatedMeshEditorMaker.repeatUVType = (RepeatUVTypeEnum)EditorGUILayout.Popup(
                        "UV repeat axis: ", (int)SMM.RepeatUVType, RepeatUVTypeDescriptionsSpline,
                        GUILayout.Width(250f));

                if (SMM.isMatchingRoadDefinition) {
                    EditorGUILayout.BeginVertical("TextArea");
                    EditorGUILayout.BeginHorizontal();
                    SMM.splinatedMeshEditorMaker.isMatchingRoadDefinition =
                        EditorGUILayout.Toggle("Match road definition: ", SMM.isMatchingRoadDefinition);
                    if (GUILayout.Button(defaultButtonTexture, imageButton, GUILayout.Width(16f)))
                        SMM.splinatedMeshEditorMaker.isMatchingRoadDefinition = false;
                    EditorGUILayout.EndHorizontal();
                    if (SMM.isMatchingRoadDefinition) {
                        EditorGUILayout.LabelField(
                            "  Only use this option if object length doesn't match the road definition.",
                            EditorStyles.miniLabel);
                        EditorGUILayout.LabelField("  Matching road definition requires a UV repeat type.",
                            EditorStyles.miniLabel);
                        EditorGUILayout.LabelField(
                            "  If the material fails to scale properly, try flipping the Y rotation.",
                            EditorStyles.miniLabel);
                    }

                    //Flip rotation option:
                    SMM.splinatedMeshEditorMaker.isFlippedRotation =
                ...
                EditorGUILayout.BeginVertical("box");
                EditorGUILayout.BeginHorizontal();
                SMM.splinatedMeshEditorMaker.VerticalRaise =
                    EditorGUILayout.Slider("Vertical raise magnitude:", SMM.verticalRaise, -512f, 512f);
                if (GUILayout.Button(defaultButtonTexture, imageButton, GUILayout.Width(16f)))
                ...
                horizMatching = (HorizMatchingDefaultsEnum)EditorGUILayout.Popup((int)horizMatching,
                    HorizMatchSubTypeDescriptions, GUILayout.Width(100f));
                if (horizMatching != HorizMatchingDefaultsEnum.None) {
                    if (horizMatching == HorizMatchingDefaultsEnum.MatchCenter)
                        ...
                        SMM.splinatedMeshEditorMaker.HorizontalSep =
                            (node.spline.road.RoadWidth() / 2 + node.spline.road.shoulderWidth) * -1;
                        ...
                        SMM.splinatedMeshEditorMaker.HorizontalSep =
                            node.spline.road.RoadWidth() / 2 + node.spline.road.shoulderWidth;
                    horizMatching = HorizMatchingDefaultsEnum.None;
                }

                EditorGUILayout.BeginHorizontal();
                SMM.splinatedMeshEditorMaker.HorizontalSep = EditorGUILayout.Slider("Horiz offset magnitude:",
                    SMM.splinatedMeshEditorMaker.HorizontalSep, -1f * horizRoadMax, horizRoadMax);
                if (GUILayout.Button(defaultButtonTexture, imageButton, GUILayout.Width(16f)))
                    SMM.splinatedMeshEditorMaker.HorizontalSep = 0f;
                EditorGUILayout.EndHorizontal();
                ...
                EditorGUILayout.BeginHorizontal();
                SMM.splinatedMeshEditorMaker.HorizontalCurve =
                    EditorGUILayout.CurveField("Curve: ", SMM.horizontalCurve);
                if (GUILayout.Button(defaultButtonTexture, imageButton, GUILayout.Width(16f)))
                ...
                EditorGUILayout.BeginVertical("box");
                SMM.splinatedMeshEditorMaker.isVerticalCutoff =
                    EditorGUILayout.Toggle("Height cutoff enabled:", SMM.isVerticalCutoff);
                if (SMM.isVerticalCutoff) {
                    SMM.splinatedMeshEditorMaker.isVerticalCutoffMatchingZero =
                        EditorGUILayout.Toggle("Match spline height:", SMM.isVerticalCutoffMatchingZero);
                    SMM.splinatedMeshEditorMaker.isVerticalCutoffDownwards =
                        EditorGUILayout.Toggle("Cut direction toggle:", SMM.isVerticalCutoffDownwards);
                    SMM.splinatedMeshEditorMaker.VerticalCutoff =
                        EditorGUILayout.Slider("Height cut offset: ", SMM.VerticalCutoff, -50f, 50f);
                    SMM.splinatedMeshEditorMaker.isVerticalMeshCutoffOppositeDir =
                        EditorGUILayout.Toggle("Opposite dir mesh cut:", SMM.isVerticalMeshCutoffOppositeDir);
                    SMM.splinatedMeshEditorMaker.VerticalMeshCutoffOffset =
                        EditorGUILayout.Slider("Mesh cut offset: ", SMM.VerticalMeshCutoffOffset, -5f, 5f);
                }
                ...
                if (SMM.isStartDown) {
                    SMM.splinatedMeshEditorMaker.isStartTypeDownOverriden =
                        EditorGUILayout.Toggle("Override start down value: ", SMM.isStartTypeDownOverriden);
                    if (SMM.isStartTypeDownOverriden)
                        SMM.splinatedMeshEditorMaker.startTypeDownOverriden =
                            EditorGUILayout.Slider("Downward movement: ", SMM.startTypeDownOverride, -10f, 10f);
                }

                if (SMM.isEndDown) {
                    SMM.splinatedMeshEditorMaker.isEndTypeDownOverriden =
                        EditorGUILayout.Toggle("Override end down value: ", SMM.isEndTypeDownOverriden);
                    if (SMM.isEndTypeDownOverriden)
                        SMM.splinatedMeshEditorMaker.endTypeDownOverriden =
                            EditorGUILayout.Slider("Downward movement: ", SMM.endTypeDownOverride, -10f, 10f);
                }
                ...
                SMM.splinatedMeshEditorMaker.isEndCapCustomMatchingStart =
                    EditorGUILayout.Toggle("Match objects to ends:", SMM.isEndCapCustomMatchStart);

                //End objects match ground:
                SMM.splinatedMeshEditorMaker.isEndObjectsMatchingGround =
                    EditorGUILayout.Toggle("Force origins to ground:", SMM.isEndObjectsMatchingGround);
                ...
                endObjectAdd =
                    (EndObjectsDefaultsEnum)EditorGUILayout.Popup((int)endObjectAdd, EndObjectsDefaultsEnumDesc);
                ...
                SMM.splinatedMeshEditorMaker.EndCapStart =
                    (GameObject)EditorGUILayout.ObjectField("Prefab:", SMM.EndCapStart, typeof(GameObject), false);
                if (SMM.EndCapStart != null) {
                    SMM.splinatedMeshEditorMaker.EndCapCustomOffsetStart =
                        EditorGUILayout.Vector3Field("Position offset:", SMM.EndCapCustomOffsetStart);
                    SMM.splinatedMeshEditorMaker.EndCapCustomRotOffsetStart =
                        EditorGUILayout.Vector3Field("Rotation offset:", SMM.EndCapCustomRotOffsetStart);
                }
                ...
                SMM.splinatedMeshEditorMaker.EndCapEnd =
                    (GameObject)EditorGUILayout.ObjectField("Prefab:", SMM.EndCapEnd, typeof(GameObject), false);
                if (SMM.EndCapEnd != null) {
                    SMM.splinatedMeshEditorMaker.EndCapCustomOffsetEnd =
                        EditorGUILayout.Vector3Field("Position offset:", SMM.EndCapCustomOffsetEnd);
                    SMM.splinatedMeshEditorMaker.EndCapCustomRotOffsetEnd =
                        EditorGUILayout.Vector3Field("Rotation offset:", SMM.EndCapCustomRotOffsetEnd);
                }
                ...
                EditorGUILayout.BeginVertical("box");
                SMM.splinatedMeshEditorMaker.collisionType = (CollisionTypeEnum)EditorGUILayout.Popup(
                    "Collision type: ", (int)SMM.CollisionType, TheCollisionTypeEnumDescSpline, GUILayout.Width(320f));
                //Mesh collison convex option
                if (SMM.CollisionType != CollisionTypeEnum.None &&
                    SMM.CollisionType != CollisionTypeEnum.BoxCollision) {
                    SMM.splinatedMeshEditorMaker.isCollisionConvex =
                        EditorGUILayout.Toggle(" Convex: ", SMM.isCollisionConvex);
                    SMM.splinatedMeshEditorMaker.isCollisionTriggered =
                        EditorGUILayout.Toggle(" Trigger: ", SMM.isCollisionTrigger);
                }

                if (SMM.CollisionType == CollisionTypeEnum.SimpleMeshTriangle ||
                    SMM.CollisionType == CollisionTypeEnum.SimpleMeshTrapezoid)
                    SMM.splinatedMeshEditorMaker.isSimpleCollisionAutomatic =
                        EditorGUILayout.Toggle(" Automatic simple collision: ", SMM.isSimpleCollisionAutomatic);
                ...
                if (!SMM.isSimpleCollisionAutomatic) {
                    if (SMM.CollisionType == CollisionTypeEnum.SimpleMeshTriangle) {
                        ...
                        SMM.splinatedMeshEditorMaker.CollisionTriBL.x = EditorGUILayout.Slider(" x-axis: ",
                            SMM.CollisionTriBL.x, SMM.mMinX - 5f, SMM.mMaxX + 5f);
                        SMM.splinatedMeshEditorMaker.CollisionTriBL.y = EditorGUILayout.Slider(" y-axis: ",
                            SMM.CollisionTriBL.y, SMM.mMinY - 5f, SMM.mMaxY + 5f);
                        SMM.splinatedMeshEditorMaker.CollisionTriBL.z = EditorGUILayout.Slider(" z-axis: ",
                            SMM.CollisionTriBL.z, SMM.mMinZ - 5f, SMM.mMaxZ + 5f);

                        EditorGUILayout.LabelField("Bottom right:");
                        SMM.splinatedMeshEditorMaker.CollisionTriBR.x = EditorGUILayout.Slider(" x-axis: ",
                            SMM.CollisionTriBR.x, SMM.mMinX - 5f, SMM.mMaxX + 5f);
                        SMM.splinatedMeshEditorMaker.CollisionTriBR.y = EditorGUILayout.Slider(" y-axis: ",
                            SMM.CollisionTriBR.y, SMM.mMinY - 5f, SMM.mMaxY + 5f);
                        SMM.splinatedMeshEditorMaker.CollisionTriBR.z = EditorGUILayout.Slider(" z-axis: ",
                            SMM.CollisionTriBR.z, SMM.mMinZ - 5f, SMM.mMaxZ + 5f);

                        EditorGUILayout.LabelField("Top:");
                        SMM.splinatedMeshEditorMaker.CollisionTriT.x = EditorGUILayout.Slider(" x-axis: ",
                            SMM.CollisionTriT.x, SMM.mMinX - 5f, SMM.mMaxX + 5f);
                        SMM.splinatedMeshEditorMaker.CollisionTriT.y = EditorGUILayout.Slider(" y-axis: ",
                            SMM.CollisionTriT.y, SMM.mMinY - 5f, SMM.mMaxY + 5f);
                        SMM.splinatedMeshEditorMaker.CollisionTriT.z = EditorGUILayout.Slider(" z-axis: ",
                            SMM.CollisionTriT.z, SMM.mMinZ - 5f, SMM.mMaxZ + 5f);
                    }
                    else if (SMM.CollisionType == CollisionTypeEnum.SimpleMeshTrapezoid) {
                        SMM.splinatedMeshEditorMaker.CollisionBoxBL =
                            EditorGUILayout.Vector3Field(" Bottom left:", SMM.CollisionBoxBL);
                        SMM.splinatedMeshEditorMaker.CollisionBoxBR =
                            EditorGUILayout.Vector3Field(" Bottom right:", SMM.CollisionBoxBR);
                        SMM.splinatedMeshEditorMaker.CollisionBoxTL =
                            EditorGUILayout.Vector3Field(" Top left:", SMM.CollisionBoxTL);
                        SMM.splinatedMeshEditorMaker.CollisionBoxTR =
                            EditorGUILayout.Vector3Field(" Top right:", SMM.CollisionBoxTR);
                    }
                }

                if (SMM.CollisionType == CollisionTypeEnum.BoxCollision) {
                    SMM.splinatedMeshEditorMaker.stretchedBCLocOffset =
                        EditorGUILayout.Vector3Field("Box collider center offset:", SMM.boxColliderOffset);
                    SMM.splinatedMeshEditorMaker.isBCFlippedX =
                        EditorGUILayout.Toggle("Flip center X:", SMM.isBoxColliderFlippedOnX);
                    SMM.splinatedMeshEditorMaker.isBCFlippedZ =
                        EditorGUILayout.Toggle("Flip center Z:", SMM.isBoxColliderFlippedOnZ);


                    SMM.splinatedMeshEditorMaker.isStretchedSize =
                        EditorGUILayout.Toggle("Box collider size edit:", SMM.isStretchedSize);
                    if (SMM.isStretchedSize)
                        SMM.splinatedMeshEditorMaker.stretchedBCSize =
                            EditorGUILayout.Vector3Field("Size:", SMM.boxColliderSize);
                    else
                ...
                if (SMM.splinatedMeshEditorMaker.isFlippedRotation != SMM.isFlippingRotation)
                    SMM.splinatedMeshEditorMaker.isFlippedRotation = EditorGUILayout.Toggle("Flip Y rotation: ",
                        SMM.splinatedMeshEditorMaker.isFlippedRotation);
                else
                    SMM.splinatedMeshEditorMaker.isFlippedRotation =
                        EditorGUILayout.Toggle("Flip Y rotation: ", SMM.isFlippingRotation);
                ...
                EditorGUILayout.BeginVertical("box");
                SMM.splinatedMeshEditorMaker.isExactSplination =
                    EditorGUILayout.Toggle("Directional extrusion: ", SMM.isExactSplination);
        ...
        public void UpdateSplineObjectsOnUndo() {
            ...
            for (var index = node.transform.childCount - 1; index >= 0; index--)
                DestroyImmediate(node.transform.GetChild(index).gameObject);
        ...
        public void DoEdgeObjects() {
            ...
            if (node.EdgeObjects == null) node.EdgeObjects = new List<EdgeObjectMaker>();
            count = node.EdgeObjects.Count;

            EOM = null;

            for (var index = 0; index < node.EdgeObjects.Count; index++) {
                EOM = node.EdgeObjects[index];
                if (EOM.edgeMaker == null) EOM.edgeMaker = new EdgeObjectMaker.EdgeObjectEditorMaker();
                ...
                if (EOM.isRequiringUpdate) EOM.Setup();
                EOM.isRequiringUpdate = false;

                EditorGUILayout.BeginHorizontal();

                EOM.isToggled = EditorGUILayout.Foldout(EOM.isToggled, "#" + currentCount + ": " + EOM.objectName);

                if (GUILayout.Button(edgeButtonTexture, imageButton, GUILayout.Width(32f))) { }

                if (GUILayout.Button(refreshButtonTexture, imageButton, GUILayout.Width(16f))) EOM.Setup();
                if (GUILayout.Button(saveButtonTexture, imageButton, GUILayout.Width(16f))) {
                    var saveWindow = EditorWindow.GetWindow<SaveWindow>();
                ...
                //Edge object:
                EOM.edgeMaker.edgeObject =
                    (GameObject)EditorGUILayout.ObjectField("Edge object: ", EOM.edgeObject, typeof(GameObject), false);
                ...
                EOM.edgeMaker.isMaterialOverriden =
                    EditorGUILayout.Toggle("Material override: ", EOM.isMaterialOverriden);
                ...
                if (EOM.isMaterialOverriden) {
                    if (EOM.isEdgeSignLabel) EditorGUILayout.TextField("Material search term: ", EOM.edgeSignLabel);

                    EOM.edgeMaker.edgeMaterial1 = (Material)EditorGUILayout.ObjectField("Override mat #1: ",
                        EOM.edgeMaterial1, typeof(Material), false);
                    EOM.edgeMaker.edgeMaterial2 = (Material)EditorGUILayout.ObjectField("Override mat #2: ",
                        EOM.edgeMaterial2, typeof(Material), false);
                }
                ...
                if (EOM.isSingle) {
                    ...
                    if (EOM.isCombinedMesh)
                        EOM.edgeMaker.isCombinedMeshCollider =
                            EditorGUILayout.Toggle("Combined mesh collider: ", EOM.isCombinedMeshCollider);
                }
                ...
                if (EOM.edgeMaker.isSingle != EOM.isSingle) {
                    ...
                    if (EOM.edgeMaker.isSingle) EOM.edgeMaker.isCombinedMesh = false;
                }

                if (EOM.isSingle) {
                    EOM.edgeMaker.singlePosition =
                        EditorGUILayout.Slider("Single location: ", EOM.singlePosition, node.time, 1f);

                    if (node.isBridgeStart && node.isBridgeMatched) {
                        EOM.edgeMaker.singleOnlyBridgePercent =
                            EditorGUILayout.Slider("Bridge %: ", EOM.singleOnlyBridgePercent, 0f, 1f);
                        if (!RootUtils.IsApproximately(EOM.singleOnlyBridgePercent,
                            EOM.edgeMaker.singleOnlyBridgePercent, 0.001f)) {
                            EOM.edgeMaker.singleOnlyBridgePercent =
                                Mathf.Clamp(EOM.edgeMaker.singleOnlyBridgePercent, 0f, 1f);
                            var dist = EOM.edgeMaker.singleOnlyBridgePercent *
                ...
                EOM.edgeMaker.isMatchingTerrain =
                    EditorGUILayout.Toggle("Match ground height: ", EOM.isMatchingTerrain);
                ...
                EOM.edgeMaker.isStartMatchingRoadDefinition =
                    EditorGUILayout.Toggle("Match road definition: ", EOM.isStartMatchRoadDefinition);
                if (EOM.isStartMatchRoadDefinition) {
                    EOM.edgeMaker.startMatchRoadDef =
                        EditorGUILayout.Slider("Position fine tuning: ", EOM.startMatchRoadDef, 0f, 1f);
                ...
                if (!EOM.isSingle) {
                    if (EOM.edgeMaker.startTime < node.minSplination) EOM.edgeMaker.startTime = node.minSplination;
                    if (EOM.edgeMaker.endTime > node.maxSplination) EOM.edgeMaker.endTime = node.maxSplination;


                    #region "Start param"

                    EditorGUILayout.BeginHorizontal();
                    EOM.edgeMaker.startTime =
                        EditorGUILayout.Slider("Start param: ", EOM.startTime, node.minSplination, EOM.endTime);
                    ...
                    EditorGUILayout.BeginHorizontal();
                    EOM.edgeMaker.endTime =
                        EditorGUILayout.Slider("End param: ", EOM.endTime, EOM.startTime, node.maxSplination);
                ...
                EditorGUILayout.BeginHorizontal();
                EOM.edgeMaker.verticalRaise =
                    EditorGUILayout.Slider("Vertical raise magnitude:", EOM.verticalRaise, -512f, 512f);
                if (GUILayout.Button(defaultButtonTexture, imageButton, GUILayout.Width(16f)))
                ...
                if (horizMatching != HorizMatchingDefaultsEnum.None) {
                    if (horizMatching == HorizMatchingDefaultsEnum.MatchCenter) {
                        ...
                        if (node.spline.road.isShouldersEnabled)
                            EOM.edgeMaker.horizontalSep =
                                (node.spline.road.RoadWidth() * 0.5f + node.spline.road.shoulderWidth) * -1;
                        else
                            EOM.edgeMaker.horizontalSep = node.spline.road.RoadWidth() * 0.5f * -1;
                    }
                    else if (horizMatching == HorizMatchingDefaultsEnum.MatchRoadRight) {
                        ...
                        if (node.spline.road.isShouldersEnabled)
                            EOM.edgeMaker.horizontalSep =
                                node.spline.road.RoadWidth() * 0.5f + node.spline.road.shoulderWidth;
                        else
                            EOM.edgeMaker.horizontalSep = node.spline.road.RoadWidth() * 0.5f;
                    }
                ...
                if (!RootUtils.IsApproximately(EOM.edgeMaker.horizontalSep, EOM.horizontalSep))
                    EOM.edgeMaker.horizontalSep =
                        Mathf.Clamp(EOM.edgeMaker.horizontalSep, -1f * horizRoadMax, horizRoadMax);


                EditorGUILayout.BeginHorizontal();
                EOM.edgeMaker.horizontalSep = EditorGUILayout.Slider("Horiz offset magnitude:",
                    EOM.edgeMaker.horizontalSep, -1f * horizRoadMax, horizRoadMax);
                if (GUILayout.Button(defaultButtonTexture, imageButton, GUILayout.Width(16f)))
                    EOM.edgeMaker.horizontalSep = 0f;
                if (!RootUtils.IsApproximately(EOM.edgeMaker.horizontalSep, EOM.horizontalSep))
                    EOM.edgeMaker.horizontalSep =
                ...
                if (EOM.horizontalSep < 0f) {
                    EOM.edgeMaker.isOncomingRotation =
                        EditorGUILayout.Toggle("Auto rotate oncoming objects: ", EOM.isOncomingRotation);
                ...
                EOM.edgeMaker.isRotationAligning =
                    EditorGUILayout.Toggle("Align objects to road rotation: ", EOM.isRotationAligning);
                ...
                EditorGUILayout.BeginVertical("box");
                EditorGUILayout.BeginHorizontal();
                var scale = EditorGUILayout.Slider("Custom scale: ", EOM.customScale.x, 1f, 10f);
        ...
        private bool VectorDiff(Vector3 _vect1, Vector3 _vect2) {
            if (!RootUtils.IsApproximately(_vect1.x, _vect2.x, 0.0001f)) return true;
            if (!RootUtils.IsApproximately(_vect1.y, _vect2.y, 0.0001f)) return true;
            if (!RootUtils.IsApproximately(_vect1.z, _vect2.z, 0.0001f)) return true;
            return false;
        }


        private bool VectorEquals(ref Vector3 _v1, ref Vector3 _v2) {
            if (!RootUtils.IsApproximately(_v1.x, _v2.x, 0.001f)) return false;
            if (!RootUtils.IsApproximately(_v1.y, _v2.y, 0.001f)) return false;
            if (!RootUtils.IsApproximately(_v1.z, _v2.z, 0.001f)) return false;
            return true;
        }
        ...
        public bool isSplinatedObjectHelp;
        public bool isEdgeObjectHelp;
        private bool isMouseDragProcessed = true;
        private bool isRemovingAll;
        private float horizRoadMax;

        private SplineN node1;
        private SplineN node2;
        private bool isCreatingIntersection;


        #region "Button icons"

        private Texture deleteButtonTexture;
        private Texture copyButtonTexture;
        private Texture saveButtonTexture;
        private Texture loadButtonTexture;
        private Texture extrudeButtonTexture;
        private Texture edgeButtonTexture;
        ...
        private Texture2D textAreaBG;
        private Texture2D loadButtonBGGlow;
        private Texture2D manualBG;

        #endregion


        public bool isLoadingEOS;
        public int loadingEOSIndex;
        public List<string> loadingEOSNames;
        public List<string> loadingEOSPaths;

        public bool isLoadingEOM;
        public int loadingEOMIndex;
        public List<string> loadingEOMNames;
        public List<string> loadingEOMPaths;

        //Checkers:
        private SplinatedMeshMaker SMM;
        ...
        private readonly BridgeBottomBaseDefaultsEnum BridgeBottomBaseQuickAdd = BridgeBottomBaseDefaultsEnum.None;
        ...
        private EdgeObjectMaker EOM;
        private GUIStyle imageButton;
        private GUIStyle loadButton;
        private GUIStyle manualButton;
        private GUIStyle guiButton;

        private bool isSceneRectSet;
        private Rect sceneRect;

        private bool isInitialized;
        private bool isGizmosEnabled;

        // Bridge
        private bool isBridgeStart;
        private bool isBridgeEnd;
        private bool isRoadCut;
        ...
        private static readonly string[] EndObjectsDefaultsEnumDesc =
        {
            "Quick add",
            "WarningSign1",
            "WarningSign2",
        ...
        private static readonly string[] TheAxisDescriptionsSpline =
        {
            "X axis",
            "Z axis"
        };
        ...
        private static readonly string[] RepeatUVTypeDescriptionsSpline =
        {
            "None",
            "X axis",
            "Y axis"
        ...
        private static readonly string[] TheCollisionTypeEnumDescSpline =
        {
            "None",
            "Simple triangle",
            "Simple trapezoid",
        ...
        private void BridgeAddTopBase(float _horizSep = 0f, float _vertRaise = -0.01f, string _mat = "",
            bool _isOverridenPrefab = false, string _overridePrefab = "") {
            var basePath = RoadEditorUtility.GetBasePath();

            if (_mat == "") _mat = basePath + "/Materials/Concrete2.mat";
            ...
            if (_isOverridenPrefab) tBridgeTopBaseToAdd = _overridePrefab;

            SMM.objectName = tName;
            SMM.currentSplination = AssetDatabase.LoadAssetAtPath<GameObject>(tBridgeTopBaseToAdd);
        ...
        private void BridgeAddBottomBase(float _horizSep = 0f, float _vertRaise = -1.01f, string _mat = "",
            bool _isOverridenPrefab = false, string _overridePrefab = "") {
            var basePath = RoadEditorUtility.GetBasePath();
            ...
            if (_isOverridenPrefab) tBridgeBottomBaseToAdd = _overridePrefab;

            SMM.currentSplination = AssetDatabase.LoadAssetAtPath<GameObject>(tBridgeBottomBaseToAdd);
            SMM.horizontalSep = _horizSep;
            SMM.verticalRaise = _vertRaise;
            SMM.isMaterialOverriden = true;
            SMM.objectName = tName;
        ...
        private void ExtrusionQuickAdd(bool _isHorizOverriden = false, float _horizSep = 0f,
            bool _isVertOverriden = false, float _vertRaise = 0f) {
            try {
                ExtrusionQuickAddDo();
            }
        ...
        private void ExtrusionQuickAddDo() {
            if (SMMQuickAdd == SMMDefaultsEnum.KRail) node.SplinatedObjectQuickAdd("KRail");
        }


        private void ExtrudeHelper(string _path, string _name, float DefaultHoriz,
            Splination.AxisTypeEnum _axisType = Splination.AxisTypeEnum.Z, bool _isHorizOverriden = false,
            float _horizSep = 0f, bool _isVertOverriden = false, float _vertRaise = 0f, bool _isFlippingRot = false) {
            SMM = node.AddSplinatedObject();
            SMM.currentSplination = AssetDatabase.LoadAssetAtPath<GameObject>(_path);
            ...
            if (_isVertOverriden) {
                ...
                if (node.isBridgeStart) SMM.verticalRaise = -0.01f;
            }
        ...
        private void TriggerRoadUpdate() {
            if (node != null) node.spline.road.isUpdateRequired = true;
        }
