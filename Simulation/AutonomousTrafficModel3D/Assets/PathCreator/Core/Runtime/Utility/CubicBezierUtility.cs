...
namespace PathCreation.Utility {
    ...
    public static class CubicBezierUtility {
        ...
        public static Bounds CalculateSegmentBounds(Vector3 p0, Vector3 p1, Vector3 p2, Vector3 p3) {
            ...
            var extremePointTimes = ExtremePointTimes(p0, p1, p2, p3);
            foreach (var t in extremePointTimes) minMax.AddValue(EvaluateCurve(p0, p1, p2, p3, t));
        ...
        private static IEnumerable<float> StationaryPointTimes(float a, float b, float c) {
            ...
            if (a != 0) {
                ...
                if (discriminant >= 0) {
                    ...
                    var t1 = (-b + s) / (2 * a);
                    if (t1 >= 0 && t1 <= 1) times.Add(t1);

                    if (discriminant != 0) {
                        var t2 = (-b - s) / (2 * a);

                        if (t2 >= 0 && t2 <= 1) times.Add(t2);
                    }
                }
            }
