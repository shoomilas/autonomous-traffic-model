...
namespace PathCreation {
    ...
    public class BezierPath {
        const int defaultNormalsAngle = 90;
        private const int defaultPerAnchorNormalsAngle = 90;
        private const PathSpace defaultSpace = PathSpace.xz;
        ...
        [SerializeField] [HideInInspector] private float globalNormalsAngle = defaultNormalsAngle;
        ...
        public BezierPath(Vector3 centre, bool isClosed = false, PathSpace space = PathSpace.xyz) {
            ...
            perAnchorNormalsAngle = new List<float> {defaultPerAnchorNormalsAngle, defaultPerAnchorNormalsAngle};
        ...
        public BezierPath(IEnumerable<Vector3> points, bool isClosed = false, PathSpace space = PathSpace.xyz) {
            ...
            if (pointsArray.Length < 2) {
                ...
                perAnchorNormalsAngle = new List<float>(new float[] {defaultPerAnchorNormalsAngle, defaultPerAnchorNormalsAngle});

                for (var i = 2; i < pointsArray.Length; i++) {
                    AddSegmentToEnd(pointsArray[i]);
                    perAnchorNormalsAngle.Add(defaultPerAnchorNormalsAngle);
                }
            }
        ...
        public void SetPoint(int i, Vector3 localPosition, bool suppressPathModifiedEvent = false) {
            ...
            if (!suppressPathModifiedEvent) NotifyPathModified();
        }
        ...
        public void AddSegmentToEnd(Vector3 anchorPos) {
            ...
            if (controlMode == ControlMode.Automatic) AutoSetAllAffectedControlPoints(points.Count - 1);
        ...
        public void AddSegmentToStart(Vector3 anchorPos) {
            ...
            if (controlMode == ControlMode.Automatic) AutoSetAllAffectedControlPoints(0);
            NotifyPathModified();
        }
        ...
        public void SplitSegment(Vector3 anchorPos, int segmentIndex, float splitTime) {
            ...
            if (controlMode == ControlMode.Automatic) {
                ...
                points.InsertRange(segmentIndex * 3 + 2,
                    new[] {splitSegment[0][2], splitSegment[1][0], splitSegment[1][1]});
                ...
                if (controlMode == ControlMode.Mirrored) {
                    var avgDst = ((splitSegment[0][2] - anchorPos).magnitude +
            ...
            var numAngles = perAnchorNormalsAngle.Count;
        ...
        public void DeleteSegment(int anchorIndex) {
            ...
            if (NumSegments > 2 || !isClosed && NumSegments > 1) {
                ...
                if (controlMode == ControlMode.Automatic) AutoSetAllControlPoints();
        ...
        public void MovePoint(int i, Vector3 pointPos, bool suppressPathModifiedEvent = false) {
            if (space == PathSpace.xy)
                pointPos.z = 0;
            else if (space == PathSpace.xz) pointPos.y = 0;
        ...
        public Bounds CalculateBoundsWithTransform(Transform transform) {
            ...
            for (var i = 0; i < NumSegments; i++) {
                ...
                for (var j = 0; j < p.Length; j++) p[j] = MathUtility.TransformPoint(p[j], transform, space);

                minMax.AddValue(p[0]);
                minMax.AddValue(p[3]);

                var extremePointTimes = CubicBezierUtility.ExtremePointTimes(p[0], p[1], p[2], p[3]);
                foreach (var t in extremePointTimes) minMax.AddValue(CubicBezierUtility.EvaluateCurve(p, t));
            }
        ...
        public void ResetNormalAngles() {
            for (var i = 0; i < perAnchorNormalsAngle.Count; i++) perAnchorNormalsAngle[i] = defaultPerAnchorNormalsAngle;
            globalNormalsAngle = defaultNormalsAngle;
            NotifyPathModified();
        }
        ...
        public Bounds PathBounds
        {
            get
            {
                if (!boundsUpToDate) UpdateBounds();
                return bounds;
            }
        }
        ...
        private void UpdateToNewPathSpace(PathSpace previousSpace) {
            ...
            if (previousSpace == PathSpace.xyz) {
                ...
                for (var i = 0; i < NumPoints; i++)
                    if (space == PathSpace.xy) {
                        var x = minBoundsSize == boundsSize.x ? points[i].z : points[i].x;
                        var y = minBoundsSize == boundsSize.y ? points[i].z : points[i].y;
                        points[i] = new Vector3(x, y, 0);
                    }
                    else if (space == PathSpace.xz) {
                        var x = minBoundsSize == boundsSize.x ? points[i].y : points[i].x;
                        var z = minBoundsSize == boundsSize.z ? points[i].y : points[i].z;
                ...
                if (space != PathSpace.xyz)
                    for (var i = 0; i < NumPoints; i++) // from xz to xy
                        if (space == PathSpace.xy)
                            points[i] = new Vector3(points[i].x, points[i].z, 0);
                        // from xy to xz
                        else if (space == PathSpace.xz) points[i] = new Vector3(points[i].x, 0, points[i].y);
            }
        ...
        private void UpdateClosedState() {
            if (isClosed) {
                ...
                var lastAnchorSecondControl = points[points.Count - 1] * 2 - points[points.Count - 2];
                ...
                if (controlMode != ControlMode.Mirrored && controlMode != ControlMode.Automatic) {
                    ...
                    var dstBetweenStartAndEndAnchors = (points[points.Count - 1] - points[0]).magnitude;
            ...
            if (controlMode == ControlMode.Automatic) AutoSetStartAndEndControls();
