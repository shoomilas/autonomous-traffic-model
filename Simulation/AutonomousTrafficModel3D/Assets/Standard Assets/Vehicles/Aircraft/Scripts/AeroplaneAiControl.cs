    ...
    public class AeroplaneAiControl : MonoBehaviour {
        ...
        private void FixedUpdate() {
            if (m_Target != null) {
                ...
                var changePitch = targetAnglePitch - m_AeroplaneController.PitchAngle;
                ...
                var desiredRoll = Mathf.Clamp(targetAngleYaw, -m_MaxRollAngle * Mathf.Deg2Rad,
                    m_MaxRollAngle * Mathf.Deg2Rad);
                float yawInput = 0;
                float rollInput = 0;
                if (!m_TakenOff) {
                    // If the planes altitude is above m_TakeoffHeight we class this as taken off
                    if (m_AeroplaneController.Altitude > m_TakeoffHeight) m_TakenOff = true;
                }
                else {
                ...
                var currentSpeedEffect = 1 + m_AeroplaneController.ForwardSpeed * m_SpeedEffect;
